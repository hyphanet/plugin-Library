...
class SpiderIndexURIs {
  ...
  synchronized FreenetURI loadSSKURIs() {
    if (privURI == null) {
      ...
      try {
        ...
        privURI = new FreenetURI(br.readLine()).setDocName("index.yml"); // Else InsertableClientSSK
                                                                         // doesn't like it.
      ...
      if (privURI == null) {
        InsertableClientSSK key =
            InsertableClientSSK.createRandom(pr.getNode().random, "index.yml");
  ...
  synchronized FreenetURI getPrivateUSK() {
    return loadSSKURIs().setKeyType("USK").setDocName(SpiderIndexUploader.INDEX_DOCNAME)
  ...
  synchronized FreenetURI getPublicUSK() {
    loadSSKURIs();
    return pubURI.setKeyType("USK").setDocName(SpiderIndexUploader.INDEX_DOCNAME)
        .setSuggestedEdition(getLastUploadedEdition());
  }
