...
class SpiderIndexURIs {
    ...
    synchronized FreenetURI loadSSKURIs() {
        if (privURI == null) {
            ...
            try {
                ...
                privURI = new FreenetURI(br.readLine()).setDocName(
                    "index.yml");    // Else InsertableClientSSK doesn't like it.
    ...
    synchronized FreenetURI getPrivateUSK() {
        return loadSSKURIs().setKeyType("USK").setDocName(
            SpiderIndexUploader.INDEX_DOCNAME).setSuggestedEdition(edition);
    }
    ...
    synchronized FreenetURI getPublicUSK() {
        ...
        return pubURI.setKeyType("USK").setDocName(
            SpiderIndexUploader.INDEX_DOCNAME).setSuggestedEdition(getLastUploadedEdition());
    }
