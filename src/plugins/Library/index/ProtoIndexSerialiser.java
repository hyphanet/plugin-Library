...
public class ProtoIndexSerialiser
    implements Archiver<ProtoIndex>, Serialiser.Composite<Archiver<Map<String, Object>>>, // TODO
                                                                                          // NORM
                                                                                          // make
                                                                                          // this a
                                                                                          // LiveArchiver
  ...
  /*
   * FIXME HIGH: Parallelism in fetching multiple words for the same query. A single serialiser
   * means when we fetch two words for the same query, and they both end up in the same bucket, we
   * get an AssertionError when we fetch the bucket twice in ProgressTracker.addPullProgress. So the
   * solution, for the time being, is simply to use two separate serialisers.
   */
  ...
  public static ProtoIndexSerialiser forIndex(FreenetURI uri, short priorityClass) {
    ...
    // Library.makeArchiver(ProtoIndexComponentSerialiser.yamlrw, MIME_TYPE, 0x80 *
    // ProtoIndex.BTREE_NODE_MIN);
  ...
  public static ProtoIndexSerialiser forIndex(File prefix) {
    ...
    // if (srl == null) {
    // srl_cls.put(File.class, srl = new ProtoIndexSerialiser(new FileArchiver<Map<String,
    // Object>>(ProtoIndexComponentSerialiser.yamlrw, true, FILE_EXTENSION)));
    // }
    // return srl;

    // One serialiser per application. See comments above re srl_cls.
    return new ProtoIndexSerialiser(new FileArchiver<Map<String, Object>>(
        ProtoIndexComponentSerialiser.yamlrw, true, FILE_EXTENSION, "", "", prefix));
  }
  ...
  /* @Override **/ public void pull(PullTask<ProtoIndex> task) throws TaskAbortException {
    ...
    if (task.meta instanceof FreenetURI) { // if not FreenetURI, skip this silently so we can test
                                           // on local files
  ...
  /* @Override **/ public void push(PushTask<ProtoIndex> task) throws TaskAbortException {
    PushTask<Map<String, Object>> serialisable =
        new PushTask<Map<String, Object>>(trans.app(task.data));
  ...
  public static class IndexTranslator implements Translator<ProtoIndex, Map<String, Object>> {
    ...
    Translator<SkeletonBTreeMap<String, SkeletonBTreeSet<TermEntry>>, Map<String, Object>> ttrans =
        new SkeletonBTreeMap.TreeTranslator<String, SkeletonBTreeSet<TermEntry>>(null,
            new ProtoIndexComponentSerialiser.TreeMapTranslator<String, SkeletonBTreeSet<TermEntry>>(
                null));
    ...
    Translator<SkeletonBTreeMap<URIKey, SkeletonBTreeMap<FreenetURI, URIEntry>>, Map<String, Object>> utrans =
        new SkeletonBTreeMap.TreeTranslator<URIKey, SkeletonBTreeMap<FreenetURI, URIEntry>>(null,
            new ProtoIndexComponentSerialiser.TreeMapTranslator<URIKey, SkeletonBTreeMap<FreenetURI, URIEntry>>(
                null));
     ...
     ** {@inheritDoc}
     **
     ** Note: the resulting map will contain the insert SSK URI under the key {@code insID}. The
     * intention is for push methods to remove this and add it to the task metadata. '''Failure to
     * do this could result in the insert URI being published'''.
     **
     ** FIXME NORM maybe make this more secure, eg. wrap it in a UnserialisableWrapper or something
     * that makes YAML throw an exception if it is accidentally passed to it.
     */
    ...
    /* @Override **/ public ProtoIndex rev(Map<String, Object> map) throws DataFormatException {
      ...
      if (magic == ProtoIndex.serialVersionUID) {
        try {
          ...
          // FIXME yaml idiocy??? It seems to give a Long if the number is big enough to need one,
          ...
          return cmpsrl.setSerialiserFor(new ProtoIndex(reqID, name, ownerName, ownerEmail,
              totalPages, modified, extra, utab, ttab));
          ...
          throw new DataFormatException("Unrecognised format ID", e, map.get("serialFormatUID"),
              map, "serialFormatUID");
