...
final public class ProtoIndex implements Index {
    ...
    /* final */
    public static int         BTREE_NODE_MIN = 0x400;
    ...
    protected ProtoIndex(FreenetURI id, String n, String owner, String ownerEmail, long pages,
                         Date m, Map<String, Object> x,
    ...
    public class getTermEntriesHandler extends AbstractExecution<Set<TermEntry>>
        ...
        public String getStatus() {
            ...
            return (cur == null)
                   ? "Starting next stage..." : cur.getSubject() + ": " + cur.getStatus();
        }
        ...
        public Progress getCurrentProgress() {
            return (last != null)
                   ...
                   : (current_tracker == null)
                     ? null : current_tracker.getPullProgressFor(current_meta);
        }
            ...
            try {
                ...
                for (;;) {
                    try {
                        ...
                        trackers.put(current_meta = d.getValue(),
                                     current_tracker =
                                         ((Serialiser.Trackable) p.getSerialiser()).getTracker());
                        p.inflate(d.getKey());
                    }
                }
                ...
                if (root == null) {
                    ...
                    throw new TaskAbortException("Index does not contain term " + subject,
                                                 new Exception("Index does not contain term " +
                                                     subject));
                }
                ...
                if (totalPages > 0) {
                    ...
                    if (multiplier < 0) {
                        Logger.error(this,
                                     "Negative multiplier!: " + multiplier + " total = " + total +
                                     " specific = " + root.size());
                        multiplier = 1.0;
                    } else {
                    ...
                    return new Iterator<TermEntry>() {
                        ...
                        public TermEntry next() {
                            ...
                            if ((t instanceof TermPageEntry) && (relAdjustment != 1.0)) {
                                ...
                                // Adjust relevance
                                return new TermPageEntry((TermPageEntry) t,
                                                         (float) (relAdjustment * t.rel));
                            } else {
                                return t;
                            }
                        }
