...
final public class ProtoIndex implements Index {
  ...
  /**
   * FIXME DEFAULT FAN-OUT: Yaml has quite high overheads which are dependant on the size of the
   * document being processed. This is potentially a problem on the client side as well as here. In
   * the long term our fan-out will be on the order of hundreds anyway because we will include the
   * metadata in the layer above. For now, a relatively low fan-out (1024 not 4096) will give
   * reasonable performance without causing such problems. Note that this also determines the size
   * of bins for Packer.
   */
  ...
  /**
   * Total number of indexed pages. Currently this is just passed in or set(), and used for rank
   * calculations. In future we might get it from utab.
   */
  ...
  public ProtoIndex(FreenetURI id, String n, String owner, String ownerEmail, long pages) {
    this(id, n, owner, ownerEmail, pages, new Date(), new HashMap<String, Object>(),
        ...
        new SkeletonBTreeMap<String, SkeletonBTreeSet<TermEntry>>(
            BTREE_NODE_MIN)/*
                            * , //filtab = new SkeletonPrefixTreeMap<Token, TokenFilter>(new
                            * Token(), TKTAB_MAX)
                            */
    );
  }

  protected ProtoIndex(FreenetURI id, String n, String owner, String ownerEmail, long pages, Date m,
      Map<String, Object> x, SkeletonBTreeMap<URIKey, SkeletonBTreeMap<FreenetURI, URIEntry>> u,
  ...
  private Map<String, Execution<Set<TermEntry>>> getTermEntriesProgress =
      new HashMap<String, Execution<Set<TermEntry>>>();
  ...
  public class getTermEntriesHandler extends AbstractExecution<Set<TermEntry>>
      implements Runnable, ChainedProgress {
    ...
    public ProgressParts getParts() throws TaskAbortException {
      ...
      int estimate = (result != null) ? ProgressParts.TOTAL_FINALIZED
          : Math.max(ttab.heightEstimate() + 1, known);
    ...
    /* @Override **/ public Progress getCurrentProgress() {
      return last != null ? last
          : current_tracker == null ? null : current_tracker.getPullProgressFor(current_meta);
    }
      ...
      try {
        ...
        for (;;) {
          try {
            ...
            trackers.put(current_meta = d.getValue(),
        ...
        if (root == null) {
          ...
          throw new TaskAbortException("Index does not contain term " + subject,
              new Exception("Index does not contain term " + subject));
        }
        ...
        if (totalPages > 0) {
          ...
          if (multiplier < 0) {
            Logger.error(this, "Negative multiplier!: " + multiplier + " total = " + total
                + " specific = " + root.size());
            multiplier = 1.0;
          } else
    ...
    private Set<TermEntry> wrapper(final SkeletonBTreeSet<TermEntry> root,
        final double relAdjustment) {
