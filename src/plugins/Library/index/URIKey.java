...
public class URIKey extends BytePrefixKey<URIKey> {
    ...
    public static byte[] getNodeRoutingKey(FreenetURI u) throws java.net.MalformedURLException {
        try {
            return ((ClientKey) BaseClientKey.getBaseKey(u.isUSK()
                    ? u.sskForUSK() : u)).getNodeKey().getRoutingKey();
        } catch (ClassCastException e) {
            throw new UnsupportedOperationException(
                "Could not get the node routing key for FreenetURI " + u +
                ". Only CHK/SSK/USK/KSKs are supported.");
        }
    }
