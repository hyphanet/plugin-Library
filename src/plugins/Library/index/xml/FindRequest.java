...
public class FindRequest extends AbstractExecution<Set<TermEntry>>
    implements Comparable<Execution>, ChainedProgress, Execution<Set<TermEntry>> {
  private Set<TermPageEntry> resultnotfinished;
  private SubProgress currentProgress;

  public enum Stages {
    UNSTARTED("Not Started"), FETCHROOT("Fetching Index Root"), FETCHSUBINDEX(
        "Fetching Subindex"), PARSE("Parsing Subindex"), DONE("Done");
  ...
  /**
   * Moves the unfinished result so that the Progress is marked as finsihed TODO get rid of this way
   * of doing it
   */
  ...
  public ProgressParts getParts() throws TaskAbortException {
    ...
    return ProgressParts.normalise(stage, (stage < end - 1) ? stage + 1 : end, end,
  ...
  /**
   * Set the stage number, between 0 & 4 inclusive TODO : now stage is not used in Progress make
   * this an enum
   * 
  ...
  /**
   * Update the ProgressParts of the currentProgress of all the FindRequests in the List with the
   * contents of the ClientEvent
   * 
  ...
  class SubProgress implements Progress { // TODO finish this
    ...
    public void partsFromEvent(ClientEvent ce) {
      ...
      if (ce instanceof SplitfileProgressEvent) {
        ...
        parts = ProgressParts.normalise(spe.succeedBlocks, spe.minSuccessfulBlocks,
            spe.minSuccessfulBlocks,
            spe.finalizedTotal ? ProgressParts.TOTAL_FINALIZED : ProgressParts.ESTIMATE_UNKNOWN);
      } else if (ce instanceof SendingToNetworkEvent || ce instanceof ExpectedMIMEEvent
          || ce instanceof ExpectedFileSizeEvent || ce instanceof SplitfileCompatibilityModeEvent
          || ce instanceof ExpectedHashesEvent) {
        // Ignore.
        return;
      } else {
        parts = ProgressParts.normalise(0, 0);
        Logger.error(this, "Fetch progress will not update due to unrecognised ClientEvent : "
            + ce.getClass().getName());
      }
    }
