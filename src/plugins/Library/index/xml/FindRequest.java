...
public class FindRequest extends AbstractExecution<Set<TermEntry>>
    ...
    public enum Stages {
        UNSTARTED("Not Started"), FETCHROOT("Fetching Index Root"),
        FETCHSUBINDEX("Fetching Subindex"), PARSE("Parsing Subindex"), DONE("Done");
...
//          Logger.normal(this, "Setting parts in "+this+" from event "+ce.getClass());
            if (ce instanceof SplitfileProgressEvent) {
                ...
                parts = ProgressParts.normalise(spe.succeedBlocks, spe.minSuccessfulBlocks,
                        ...
                        spe.finalizedTotal
                        ? ProgressParts.TOTAL_FINALIZED : ProgressParts.ESTIMATE_UNKNOWN);
            } else if ((ce instanceof SendingToNetworkEvent) ||
                       (ce instanceof ExpectedMIMEEvent) ||
                       (ce instanceof ExpectedFileSizeEvent) ||
                       (ce instanceof SplitfileCompatibilityModeEvent) ||
                ...
                Logger.error(this,
                             "Fetch progress will not update due to unrecognised ClientEvent : " +
                             ce.getClass().getName());
            }
        }
