...
public class LibrarianHandler extends DefaultHandler {
   ...
   * 
   * @param requests the requests wanting to be resolved by this LibrarianHandler, results are
  ...
  public void startElement(String nameSpaceURI, String localName, String rawName, Attributes attrs)
    ...
    if (elt_name.equals("keywords"))
      processingWord = true;
    /*
     * looks for the word in the given subindex file if the word is found then the parser fetches
     * the corresponding fileElements
     */
    ...
    if (processingWord && wordMatches != null) {
      ...
      for (FindRequest match : wordMatches) {
        ...
        if (termpositions != null && termpositions.size() > 0 && inFileWordCount > 0) {
          ...
          // if(logMINOR) Logger.minor(this, "Set relevance of "+pageEntry.getTitle()+" to
          // "+pageEntry.rel+" - "+pageEntry.toString());
        }

        TermPageEntry pageEntry =
            new TermPageEntry(match.getSubject(), relevance, inFileURI, inFileTitle, termpositions);
