...
public class XMLIndex implements Index, ClientGetCallback, RequestClient {
    ...
    public XMLIndex(String baseURI, long edition, PluginRespirator pr, URLUpdateHook hook,
        ...
        if ( !Util.isValid(baseURI)) {
            throw new InvalidSearchException(baseURI +
                    " is neither a valid file nor valid Freenet URI");
        }
...
//      Logger.normal(this, "Fetch successful " + toString());
    ...
    public void onFailure(FetchException e, ClientGetter state, ObjectContainer container) {
        ...
        if ((fetchFailures < 20) && (e.newURI != null)) {
            try {
                ...
                if ((origEdition != -1) && (e.newURI.getEdition() < origEdition)) {
                    Logger.error(this,
                                 "Redirect to earlier edition?!?!?!?: " + e.newURI.getEdition() +
                                 " from " + origEdition);
                } else {
                    if (logMINOR) {
        ...
        for (FindRequest findRequest : waitingOnMainIndex) {
            findRequest.setError(new TaskAbortException("Failure fetching rootindex of " +
                    toString(), e));
        }
     ...
     */
    private synchronized void startFetch(boolean retry)
            throws FetchException, MalformedURLException {
        if (( !retry) &&
                ((fetchStatus != FetchStatus.UNFETCHED) && (fetchStatus != FetchStatus.FAILED))) {
            return;
        }
...
//              Logger.normal(this, "Updated with event : "+ce.getDescription());
        ...
        SubIndex(String indexuri, String filename) {
            ...
            if (pr != null) {
                hlsc = pr.getNode().clientCore.makeClient(
                    RequestStarter.INTERACTIVE_PRIORITY_CLASS, false, false);
                ...
                while (waitingOnSubindex.size() > 0) {
                    if ((fetchStatus == FetchStatus.UNFETCHED) ||
                        ...
                        try {
                            ...
                            String msg = indexuri + filename + " could not be opened: " +
                                         e.toString();
            ...
            synchronized (parsingSubindex) {
                ...
                try {
                        ...
                        if (mode == MODE_SEEKING_DECLARATION) {
                            if (declarationPtr == declarationBuf.length) {
                                throw new TaskAbortException(
                                    "Could not split up XML: declaration too long",
                            ...
                            if (prefixPtr != prefixBuffer.length) {
                                ...
                                if (Fields.byteArrayEqual(filesPrefix, prefixBuffer) ||
                                    ...
                                    System.arraycopy(prefixBuffer, 1, prefixBuffer, 0,
                                                     prefixBuffer.length - 1);
                            ...
                            if (endPtr != endBuffer.length) {
                                ...
                                if (Fields.byteArrayEqual(filesEnd, endBuffer)) {
                                    ...
                                    System.arraycopy(endBuffer, 1, endBuffer, 0,
                                                     endBuffer.length - 1);
                    ...
                    if (mode != MODE_COPYING_REST) {
                        throw new TaskAbortException("Could not split up XML: Last mode was " +
                                                     mode, new Exception("bad xml"));
                    }
        ...
        Map<String, ArrayList<FileMatch>> idToFileMatches = new HashMap<String,
                                                                ArrayList<FileMatch>>();
        ...
        class StageTwoHandler extends DefaultHandler {
            ...
            public void startElement(String nameSpaceURI, String localName, String rawName,
                    ...
                    throws SAXException {
                if ((requests.size() == 0) &&
                        ((wordMatches == null) || (wordMatches.size() == 0))) {
                    return;
                }
                    ...
                    try {
                        ...
                        if (requests != null) {
                            ...
                            if (wordMatches != null) {
                                ...
                                thisWordMatch =
                                    new WordMatch(new ArrayList<FindRequest>(wordMatches),
                                                  inWordFileCount);
                            }
                        }
                    } catch (Exception e) {
            ...
            public void endElement(String namespaceURI, String localName, String qName) {
                if (processingWord && (wordMatches != null) && qName.equals("file")) {
                    ...
                    FileMatch            thisFile  = new FileMatch(id, termpositions,
                                                         thisWordMatch);
            ...
            public void startElement(String nameSpaceURI, String localName, String rawName,
                ...
                if (elt_name.equals("file")) {
                    try {
                            ...
                            for (FileMatch match : matches) {
                                ...
                                if (l >= 3) {
                                    try {
                                        ...
                                        Logger.error(this,
                                                     "Index Format not compatible " + e.toString(),
                                                     e);
                                    }
                                    ...
                                    try {
                                        ...
                                        if (wordCountString != null) {
                                            wordCount =
                                                Integer.parseInt(attrs.getValue("wordCount"));
                                        }
                                    } catch (Exception e) {
                                ...
                                for (FindRequest req : match.word.searches) {
                                    ...
                                    if (logDEBUG) {
                                        Logger.debug(
                                            this,
                                            "termcount " +
                                            ((match.termpositions == null)
                                             ? 0
                                             : match.termpositions.size()) + " filewordcount = " +
                                                 wordCount);
                                    }

                                    if ((match.termpositions != null) &&
                                            (match.termpositions.size() > 0) && (wordCount > 0)) {
                                        relevance = (float) (match.termpositions.size() /
                                                             (float) wordCount);

                                        if ((totalFileCount > 0) &&
                                                (match.word.inWordFileCount > 0)) {
                                            relevance *=
                                                Math.log((float) totalFileCount /
                                                         (float) match.word.inWordFileCount);
                                        }

                                        if (logDEBUG) {
                                            Logger.debug(this,
                                                         "Set relevance of " + title + " to " +
                                                         relevance + " - " + key);
                                        }
                                    }

                                    TermPageEntry pageEntry = new TermPageEntry(req.getSubject(),
                                                                  relevance, new FreenetURI(key),
                                                                  title, match.termpositions);
                        ...
                        Logger.error(
                            this,
                            "File id and key could not be retrieved. May be due to format clash",
                            e);
                    }
                }
            }
