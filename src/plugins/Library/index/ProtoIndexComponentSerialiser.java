...
public class ProtoIndexComponentSerialiser {
     ...
     */
    final protected static Translator<URIKey, String> utab_keys_ktr = new Translator<URIKey,
                                                                          String>() {
    ...
    protected ProtoIndexComponentSerialiser(int fmtid,
        ...
        ttab_data       = new BTreePacker<String, SkeletonBTreeSet<TermEntry>,
                EntryGroupSerialiser<String,
                                     SkeletonBTreeSet<TermEntry>>>(new EntryGroupSerialiser<String,
                                         SkeletonBTreeSet<TermEntry>>(leaf_arx, null,
                                             new SkeletonBTreeSet.TreeTranslator<TermEntry,
                                                 TermEntry>(null, term_data_mtr) {
            @Override
        ...
        utab_data =
            new BTreePacker<URIKey, SkeletonBTreeMap<FreenetURI, URIEntry>,
                            EntryGroupSerialiser<URIKey,
                                SkeletonBTreeMap<FreenetURI,
                                    URIEntry>>>(new EntryGroupSerialiser<URIKey,
                                        SkeletonBTreeMap<FreenetURI, URIEntry>>(leaf_arx, null,
                                            new SkeletonBTreeMap.TreeTranslator<FreenetURI,
                                                URIEntry>(null, null) {
            @Override
     ...
     */
    public SkeletonBTreeMap<FreenetURI, URIEntry> setSerialiserFor(SkeletonBTreeMap<FreenetURI,
                            URIEntry> entries) {
        BTreeNodeSerialiser<FreenetURI, URIEntry> uri_keys =
            new BTreeNodeSerialiser<FreenetURI, URIEntry>(
                "uri entries", leaf_arx,
                entries.makeNodeTranslator(
                    null,
                    null)    // no translator needed as FreenetURI and URIEntry are both directly serialisable by YamlReaderWriter
            );
    ...
    public static class BTreeNodeSerialiser<K, V>
            ...
            implements Archiver<SkeletonBTreeMap<K, V>.SkeletonNode>,
                       Serialiser.Translate<SkeletonBTreeMap<K, V>.SkeletonNode,
                           Map<String,
                               Object>>, Serialiser.Composite<LiveArchiver<Map<String, Object>,
                                   SimpleProgress>> {
        ...
        public BTreeNodeSerialiser(String n, LiveArchiver<Map<String, Object>, SimpleProgress> s,
            ...
            super(new ProgressTracker<SkeletonBTreeMap<K, V>.SkeletonNode,
                                      SimpleProgress>(SimpleProgress.class));
            subsrl = s;
            trans  = t;
            name   = n;
        }
        ...
        /* @Override* */
        public Translator<SkeletonBTreeMap<K, V>.SkeletonNode,
                          Map<String, Object>> getTranslator() {
            return trans;
        }
        ...
        /* @Override* */
        public void pullLive(PullTask<SkeletonBTreeMap<K, V>.SkeletonNode> task, SimpleProgress p)
            ...
            try {
                ...
                PullTask<Map<String, Object>> serialisable = new PullTask<Map<String,
                                                                 Object>>(ghost.getMeta());
        ...
        /* @Override* */
        public void pushLive(PushTask<SkeletonBTreeMap<K, V>.SkeletonNode> task, SimpleProgress p)
                throws TaskAbortException {
            p.enteredSerialiser();
     ...
     * @author infinity0
     */
    public static class BTreePacker<K, V,
            S extends IterableSerialiser<Map<K, V>> & Serialiser.Trackable<Map<K, V>>>
        ...
        protected void preprocessPushBins(Map<K, PushTask<V>> tasks,
            ...
            for (Map.Entry<K, PushTask<V>> en : tasks.entrySet()) {
                try {
                    ...
                    p.setSubProgress(ProgressTracker.makePushProgressIterable(subsrl.getTracker(),
                            bintasks));
                    p.setEstimate(ProgressParts.TOTAL_FINALIZED);
    ...
    public static class EntryGroupSerialiser<K, V>
        ...
        /* @Override* */
        public void pullLive(PullTask<Map<K, V>> task, SimpleProgress p) throws TaskAbortException {
            p.enteredSerialiser();
            ...
            try {
                ...
                try {
                    ...
                    throw new DataFormatException("Exception in converting data", e, t.data, null,
                                                  null);
                }
        ...
        /* @Override* */
        public void pushLive(PushTask<Map<K, V>> task, SimpleProgress p) throws TaskAbortException {
            p.enteredSerialiser();
            ...
            try {
                ...
                PushTask<Map<String, Object>> t = new PushTask<Map<String, Object>>(conv,
                                                      task.meta);
    ...
    public static class DummySerialiser<K, T>
            implements IterableSerialiser<T>, MapSerialiser<K, T> {
