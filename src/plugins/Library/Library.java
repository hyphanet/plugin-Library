...
final public class Library implements URLUpdateHook {
  ...
  public static final String DEFAULT_INDEX_SITE =
      BOOKMARK_PREFIX + "liberty-of-information" + " " + BOOKMARK_PREFIX + "free-market-free-people"
          + " " + BOOKMARK_PREFIX + "gotcha" + " " + BOOKMARK_PREFIX + "wanna" + " "
          + BOOKMARK_PREFIX + "wanna.old" + " " + BOOKMARK_PREFIX + "gogo";
  ...
  /**
   * Method to setup Library class so it has access to PluginRespirator, and load bookmarks TODO
   * pull bookmarks from disk
   */
  private Library(PluginRespirator pr) {
    ...
    if (persistentFile.canRead()) {
      try {
        ObjectInputStream is = new ObjectInputStream(new FileInputStream(persistentFile)); // These
                                                                                           // are
                                                                                           // annoying
                                                                                           // but
                                                                                           // it's
        ...
        Logger.error(this,
            "Moved LibraryPersistent contents into database and securely deleted old file.");
        migrated = true;
    ...
    for (Map.Entry<String, String> entry : bookmarks.entrySet()) {
      ...
      if (uri.isUSK()) {
        ...
        callback.ret = uskManager.subscribeContent(u, callback, false,
            pr.getHLSimpleClient().getFetchContext(),
            RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, rcBulk);
      }
    }
    if (bookmarks.isEmpty() || needNewWanna || !bookmarks.containsKey("gotcha")
      ...
      if (!bookmarks.containsKey("liberty-of-information"))
        addBookmark("liberty-of-information",
            "USK@5YCPzcs60uab6VSdiKcyvo-G-r-ga2UWCyOzWHaPoYE,a1zEPCf0qkUFQxftFZK5xmxYdxt0JErDb2aJgwG8s~4,AQACAAE/index.yml/25");
      if (!bookmarks.containsKey("free-market-free-people"))
        addBookmark("free-market-free-people",
            "USK@X4lMQ51bXPSicAgbR~XdFzDyizYHYrvzStdeUrIFhes,0ze4TAqd~RdAMZMsshybHZFema3ZP3id4sgN3H8969g,AQACAAE/index.yml/4");
      if (!bookmarks.containsKey("gotcha"))
        addBookmark("gotcha",
            "USK@zcAnAgT-xp5LnnK28-Lc7Qt-GU7pNKnVdkmU4-HCCBc,s2jiTh8~O9MtnGdVqJqgnKGrXrosK8rArcZ8A49hprY,AQACAAE/index.yml/6");
      if (!bookmarks.containsKey("wanna.old"))
        addBookmark("wanna.old",
            "USK@5hH~39FtjA7A9~VXWtBKI~prUDTuJZURudDG0xFn3KA,GDgRGt5f6xqbmo-WraQtU54x4H~871Sho9Hz6hC-0RA,AQACAAE/Search/25/index.xml");
      if (!bookmarks.containsKey("freenetindex"))
        addBookmark("freenetindex",
            "USK@US6gHsNApDvyShI~sBHGEOplJ3pwZUDhLqTAas6rO4c,3jeU5OwV0-K4B6HRBznDYGvpu2PRUuwL0V110rn-~8g,AQACAAE/freenet-index/5/index.xml");
      if (!bookmarks.containsKey("gogo"))
        addBookmark("gogo",
            "USK@shmVvDhwivG1z1onSA5efRl3492Xyoov52hrC0tF6uI,wpMhseMpFHPLpXYbV8why1nfBXf2XdSQkzVaemFOEsA,AQACAAE/index.yml/51");
      if (!bookmarks.containsKey("wanna"))
        addBookmark("wanna",
            "USK@gxuHPaicqxlpPPagBKPVPraZ4bwLdMYBc5vipkWGh3E,08ExdmvZzB8Hfi6H6crbiuCd2~ikWDIpJ8dvr~tLp7k,AQACAAE/index.yml/82");
      migrated = true;
  ...
  public synchronized void saveState() {
    ...
    try {
      pr.putStore(store);
      if (logMINOR)
        Logger.minor(this, "Stored state to database");
    } catch (PersistenceDisabledException e) {
      // Not much we can do...
    }
  }
  ...
  /*
   * FIXME Parallel fetch from the same index is not currently supported. This means that the only
   * reliable way to do e.g. an intersection search is to run two completely separate fetches.
   * Otherwise we get assertion errors in e.g. BTreePacker.preprocessPullBins from
   * TaskInProgressException's we can't handle from ProgressTracker.addPullProgress, when the terms
   * are in the same bucket. We should make it possible to search for multiple terms in the same
   * btree, but for now, turning off caching is the only viable option.
   */
  ...
  /**
   ** Get the index type giving a {@code FreenetURI}. This must not contain a metastring (end with
   * "/") or be a USK.
   */
  public Class<?> getIndexType(FreenetURI indexuri) throws FetchException {
    if (indexuri.lastMetaString() != null
        && indexuri.lastMetaString().equals(XMLIndex.DEFAULT_FILE))
      return XMLIndex.class;
    if (indexuri.lastMetaString() != null
        && indexuri.lastMetaString().equals(ProtoIndex.DEFAULT_FILE))
    ...
    HighLevelSimpleClient hlsc =
        core.makeClient(RequestStarter.INTERACTIVE_PRIORITY_CLASS, false, false);

    List<FreenetURI> uris = Arrays.asList(indexuri, indexuri.pushMetaString(""),
   ...
   * 
   * /** Get the index type giving a {@code FreenetURI}. This should have been passed through {@link
   * KeyExplorerUtils#sanitizeURI(List, String)} at some point - ie. it must not contain a
   * metastring (end with "/") or be a USK. / public Class<?> getIndexType(FreenetURI uri) throws
   * FetchException, IOException, MetadataParseException, LowLevelGetException,
   * KeyListenerConstructionException { GetResult getresult = KeyExplorerUtils.simpleGet(pr, uri);
   * byte[] data = BucketTools.toByteArray(getresult.getData());
   * 
   * if (getresult.isMetaData()) { try { Metadata md = Metadata.construct(data);
   * 
   * if (md.isArchiveManifest()) { if (md.getArchiveType() == ARCHIVE_TYPE.TAR) { return
   * getIndexTypeFromManifest(uri, false, true);
   * 
   * } else if (md.getArchiveType() == ARCHIVE_TYPE.ZIP) { return getIndexTypeFromManifest(uri,
   * true, false);
   * 
   * } else { throw new UnsupportedOperationException("not implemented - unknown archive manifest");
   * }
   * 
   * } else if (md.isSimpleManifest()) { return getIndexTypeFromManifest(uri, false, false); }
   * 
   * return getIndexTypeFromSimpleMetadata(md);
   * 
   * } catch (MetadataParseException e) { throw new RuntimeException(e); } } else { throw new
   * UnsupportedOperationException("Found data instead of metadata; I do not have enough intelligence to decode this."
   * ); } }
   * 
   * public Class<?> getIndexTypeFromSimpleMetadata(Metadata md) { String mime = md.getMIMEType();
   * if (mime.equals(ProtoIndex.MIME_TYPE)) { //return "YAML index"; return ProtoIndex.class; } else
   * if (mime.equals(XMLIndex.MIME_TYPE)) { //return "XML index"; return XMLIndex.class; } else {
   * throw new UnsupportedOperationException("Unknown mime-type for index"); } }
   * 
   * public Class<?> getIndexTypeFromManifest(FreenetURI furi, boolean zip, boolean tar) throws
   * FetchException, IOException, MetadataParseException, LowLevelGetException,
   * KeyListenerConstructionException {
   * 
   * boolean automf = true, deep = true, ml = true; Metadata md = null;
   * 
   * if (zip) md = KeyExplorerUtils.zipManifestGet(pr, furi); else if (tar) md =
   * KeyExplorerUtils.tarManifestGet(pr, furi, ".metadata"); else { md =
   * KeyExplorerUtils.simpleManifestGet(pr, furi); if (ml) { md =
   * KeyExplorerUtils.splitManifestGet(pr, md); } }
   * 
   * if (md.isSimpleManifest()) { // a subdir HashMap<String, Metadata> docs = md.getDocuments();
   * Metadata defaultDoc = md.getDefaultDocument();
   * 
   * if (defaultDoc != null) { //return "(default doc method) " +
   * getIndexTypeFromSimpleMetadata(defaultDoc); return getIndexTypeFromSimpleMetadata(defaultDoc);
   * }
   * 
   * if (docs.containsKey(ProtoIndex.DEFAULT_FILE)) { //return "(doclist method) YAML index"; return
   * ProtoIndex.class; } else if (docs.containsKey(XMLIndex.DEFAULT_FILE)) { //return
   * "(doclist method) XML index"; return XMLIndex.class; } else { throw new
   * UnsupportedOperationException("Could not find a supported index in the document-listings for "
   * + furi.toString()); } }
   * 
   * throw new UnsupportedOperationException("Parsed metadata but did not reach a simple manifest: "
   * + furi.toString()); }
   */
  public Class<?> getIndexType(File f) {
    ...
    throw new UnsupportedOperationException(
        "Could not determine default index file under the path given: " + f);
  }
  ...
  public Object getAddressTypeFromString(String indexuri) {
    try {
      ...
      if (file.canRead()) {
        ...
        throw new UnsupportedOperationException(
            "Could not recognise index type from string: " + indexuri);
      }
    }
  }

  /**
   * Add a new bookmark, TODO there should be a separate version for untrusted adds from freesites
   * which throws some Security Exception
   * 
  ...
  public String addBookmark(String name, String uri) {
    ...
    if (!isSame) {
      ...
      callback.ret = uskManager.subscribeContent(uskNew, callback, false,
          pr.getHLSimpleClient().getFetchContext(),
          RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, rcBulk);
    }
    return name;
  ...
  private class BookmarkCallback implements USKRetrieverCallback, USKCallback {
    ...
    public void onFound(USK origUSK, long edition, FetchResult data) {
      data.asBucket().free();
      if (logMINOR)
        Logger.minor(this, "Bookmark " + bookmarkName + " : fetching edition " + edition);
    }
    ...
    public void onFoundEdition(long l, USK key, ClientContext context, boolean metadata,
      ...
      if (newKnownGood) {
        ...
        if (logMINOR)
          Logger.minor(this, "Bookmark " + bookmarkName + " new last known good edition " + l
              + " uri is now " + uri);
  ...
  /**
   * Returns a set of Index objects one for each of the uri's specified gets an existing one if its
   * there else makes a new one
   *
   * @param indexuris list of index specifiers separated by spaces
   * @return Set of Index objects
   */
  public final ArrayList<Index> getIndices(String indexuris)
   ...
   */
  /**
   * Returns an Index object for the uri specified, gets an existing one if its there else makes a
   * new one
   *
   ...
   * @return Index object
   */
  public final Index getIndex(String indexuri, String origIndexName)
      throws InvalidSearchException, TaskAbortException {
    ...
    try {
      ...
      if (indextype == ProtoIndex.class) {
        ...
        ProtoIndexSerialiser.forIndex(indexkey, RequestStarter.INTERACTIVE_PRIORITY_CLASS)
            .pull(task);
        index = task.data;
      ...
      throw new TaskAbortException("Failed to fetch index " + indexuri + " : " + e, e, true); // can
                                                                                              // retry
      /*
       * KEYEXPLORER } catch (IOException e) { throw new TaskAbortException("Failed to fetch index "
       * + indexuri, e, true); // can retry
       * 
       * } catch (LowLevelGetException e) { throw new TaskAbortException("Failed to fetch index " +
       * indexuri, e, true); // can retry
       * 
       * } catch (KeyListenerConstructionException e) { throw new
       * TaskAbortException("Failed to fetch index " + indexuri, e, true); // can retry
       * 
       * } catch (MetadataParseException e) { throw new TaskAbortException("Failed to parse index  "
       * + indexuri, e);
       */
    } catch (UnsupportedOperationException e) {
  ...
  /**
   ** Create a {@link FreenetArchiver} connected to the core of the singleton's
   * {@link PluginRespirator}.
   **
   ** @throws IllegalStateException if the singleton has not been initialised or if it does not have
   *         a respirator.
   */
  public static <T> FreenetArchiver<T> makeArchiver(ObjectStreamReader r, ObjectStreamWriter w,
      String mime, int size, short priorityClass) {
    if (lib == null || lib.pr == null) {
      throw new IllegalStateException(
          "Cannot archive to freenet without a fully live Library plugin connected to a freenet node.");
    } else {
   ...
   *         a respirator.
   */
  public static <T, S extends ObjectStreamWriter & ObjectStreamReader> FreenetArchiver<T> makeArchiver(
      S rw, String mime, int size, short priorityClass) {
