...
final public class Library implements URLUpdateHook {
    ...
    public static final String DEFAULT_INDEX_SITE = BOOKMARK_PREFIX + "liberty-of-information" +
                                                    " " + BOOKMARK_PREFIX +
                                                    "free-market-free-people" + " " +
                                                    BOOKMARK_PREFIX + "gotcha" + " " +
                                                    BOOKMARK_PREFIX + "wanna" + " " +
                                                    BOOKMARK_PREFIX + "wanna.old" + " " +
                                                    BOOKMARK_PREFIX + "gogo";
    ...
    private Library(PluginRespirator pr) {
        ...
        if ((store != null) && store.subStores.containsKey(STOREKEY)) {
            for (Map.Entry<String, String> entry :
                    store.subStores.get(STOREKEY).strings.entrySet()) {
            ...
            try {
                ObjectInputStream is =
                    new ObjectInputStream(
                        new FileInputStream(
                            persistentFile));    // These are annoying but it's better than nothing
...
//
    ...
    private Map<String, BookmarkCallback> bookmarkCallbacks = new HashMap<String,
                                                                  BookmarkCallback>();
...
//          if (tempURI.isUSK()) { tempURI = tempURI.sskForUSK(); }
        ...
        synchronized (this) {
            ...
            if (callback == null) {
                bookmarkCallbacks.put(name,
                                      callback = new BookmarkCallback(name, u.getAllMetaStrings(),
                                          edition));
                old = null;
            }
    ...
    private class BookmarkCallback implements USKRetrieverCallback, USKCallback {
        ...
        public void onFoundEdition(long l, USK key, ObjectContainer container,
                                   ClientContext context, boolean metadata, short codec,
            ...
            if (newKnownGood) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Bookmark " + bookmarkName + " new last known good edition " + l +
                                 " uri is now " + uri);
                }
...
//
        ...
        try {
            ...
            throw new TaskAbortException("Did not recognise index type in : \"" + indexuri + "\"",
                                         e);
        }
        ...
        try {
            ...
            if (indextype == ProtoIndex.class) {
                ...
                ProtoIndexSerialiser.forIndex(indexkey,
                                              RequestStarter.INTERACTIVE_PRIORITY_CLASS).pull(task);
                index = task.data;
            ...
            throw new TaskAbortException("Failed to fetch index " + indexuri + " : " + e, e,
                                         true);    // can retry
...
*/
     ...
     */
    public static <T,
            S extends ObjectStreamWriter & ObjectStreamReader> FreenetArchiver<T> makeArchiver(
