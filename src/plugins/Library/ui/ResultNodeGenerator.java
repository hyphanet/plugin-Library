...
/**
 * Class for parsing and formatting search results, once isDone() return true, the nodes are ready
 * to use
 *
 * @author MikeB
 */
public class ResultNodeGenerator implements Runnable {
  ...
  private void parseResult() {
    ...
    if (!groupusk) { // Move entries from the groupmap to the pageset, marking whether thaey are the
      ...
      for (Iterator<TermPageGroupEntry> it2 = groupmap.values().iterator(); it2.hasNext();) {
        ...
        SortedMap<Long, SortedSet<TermPageEntry>> editions = groupentry.getEditions(); // The
                                                                                       // editions
                                                                                       // of each
                                                                                       // site
        SortedSet<TermPageEntry> newest = editions.get(editions.lastKey()); // get the newest
                                                                            // edition
        for (Iterator<SortedSet<TermPageEntry>> editionIterator =
            groupentry.getEditions().values().iterator(); editionIterator.hasNext();) {
          SortedSet<TermPageEntry> edition = editionIterator.next(); // Iterate through all the
                                                                     // editions
          if (!showold && edition != newest) // If not showing old, skip all but newest
            continue;
          for (TermPageEntry termPageEntry : edition) {
            pageset.put(termPageEntry, edition == newest); // Add pages, marking whether they are
    ...
    if (groupmap != null) { // Produce grouped list of pages
      ...
      while (it2.hasNext()) {
        ...
        HTMLNode oldEditionContainer =
            siteBlockOldOuter.addChild("div", new String[] {"class", "style"},
        ...
        while (it3.hasNext()) {
          ...
          if (siteMap.get(version).size() > 1 || siteMap.size() > 1) {
            ...
            versionNode = oldEditionContainer.addChild("table", new String[] {"class"},
                new String[] {"librarian-result"});
            HTMLNode grouptitle = versionNode.addChild("tr").addChild("td",
                new String[] {"padding", "colspan"}, new String[] {"0", "3"});
            grouptitle.addChild("h4", "class",
      ...
      for (Iterator<Entry<TermPageEntry, Boolean>> it = pageset.entrySet().iterator(); it
          .hasNext();) {
  ...
  private HTMLNode termPageEntryNode(TermPageEntry entry, boolean newestVersion) {
    ...
    pageNode.addChild("a", new String[] {"href", "class", "title"},
        new String[] {realurl, (newestVersion ? "result-title-new" : "result-title-old"),
    ...
    if (uri.isSSKForUSK()) {
      ...
      pageNode.addChild("a", new String[] {"href", "class", "title"},
          new String[] {realuskurl,
              (newestVersion ? "result-uskbutton-new" : "result-uskbutton-old"), realuskurl},
          "[ USK ]");
    }
