...
public class MainPageToadlet extends Toadlet {
  ...
  public void handleMethodGET(URI uri, final HTTPRequest request, final ToadletContext ctx)
    ...
    try {
      ...
      p.headNode.addChild("link", new String[] {"rel", "href", "type"},
          new String[] {"stylesheet", path() + "static/style.css", "text/css"});
    ...
    } catch (RuntimeException e) { // this way isnt working particularly well, i think the ctx only
                                   // gives out one page maker
  ...
  public void handleMethodPOST(URI uri, HTTPRequest request, final ToadletContext ctx)
    ...
    try {
      ...
      MainPage page = MainPage.processPostRequest(request, contentNode, hasFormPassword,
          ctx.isAllowedFullAccess(), library, pr);
      if (page == null)
        page = new MainPage(library, pr);
      // Process the request
      MultiValueTable<String, String> headers = new MultiValueTable();
