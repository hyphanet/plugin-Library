...
public class MainPageToadlet extends Toadlet {
    ...
    public MainPageToadlet(HighLevelSimpleClient client, Library library, NodeClientCore core,
                           PluginRespirator pr) {
        super(client);
        this.core    = core;
    ...
    public void handleMethodGET(URI uri, final HTTPRequest request, final ToadletContext ctx)
        ...
        try {
            ...
            if (page == null) {
                ...
                if ((query != null) && (query.length() > 0)) {
                    title = query + " - " + L10nString.getString("title") + " (" +
                            Library.plugName + ")";
                }
            }
            ...
            // Style
            p.headNode.addChild("link", new String[] { "rel", "href", "type" },
                                new String[] { "stylesheet",
                    path() + "static/style.css", "text/css" });
        ...
        } catch (RuntimeException e) {    // this way isnt working particularly well,

            // i think the ctx only gives out one page maker
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, final ToadletContext ctx)
        ...
        try {
            ...
            HTMLNode pageNode    = p.outer;
            HTMLNode contentNode = p.content;
            MainPage page        = MainPage.processPostRequest(request, contentNode,
                                       hasFormPassword, ctx.isAllowedFullAccess(), library, pr);
