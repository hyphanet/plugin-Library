...
class MainPage {
  ...
  /**
   * FIXME ajaxy stuff isnt working, it doesnt seem to renew the timeout in the js, it stopped
   * working when the response type of the xml was changed to text/xml
   * 
   * @param library
   * @param pr
   */
  ...
  /**
   * Process a get request, the only parameter allowed for a get request is request id (for an
   * ongoing request)
   */
  public static MainPage processGetRequest(HTTPRequest request) {
    if (request.isParameterSet("request")
        && searchPages.containsKey(request.getIntParam("request"))) {
  ...
  private static enum Commands {
    ...
    addbookmark,
    /**
     * puts an index in the add bookmark box to be named, requires an integer parameter between 0
     * and "extraindexcount" to specify which index is being added
     */
    addindex,
    /**
     * deletes a bookmark from the Library, requires an integer parameter between 0 and the number
     * of bookmarks
     */
    removebookmark
  }
   ...
   */
  public static MainPage processPostRequest(HTTPRequest request, HTMLNode contentNode,
      boolean hasFormPassword, boolean userAccess, Library library, PluginRespirator pr) {
    cleanUpPages();
    ...
    if (request.isPartSet(Commands.find.toString())) {
      if (hasFormPassword) {
        ...
        try {
          if (logMINOR)
            Logger.minor(MainPage.class,
                "starting search for " + page.query + " on " + page.indexstring);
          page.search = Search.startSearch(page.query, page.indexstring);
          if (page.search == null)
            page.messages.append(
                "Stopwords too prominent in search term, try removing words like 'the', 'and' and 'that' and any words less than 3 characters");
          else {
            page.search.setMakeResultNode(page.groupusk, page.showold, true); // for the moment js
                                                                              // will always be on
                                                                              // for results, js
                                                                              // detecting isnt
                                                                              // being used
      ...
      try {
        ...
        if (userAccess && hasFormPassword && page.addindexname.length() > 0
            && URLEncoder.encode(page.addindexname, "UTF-8").equals(page.addindexname)) {
    ...
    } else { // check if one of the other indexes is being added as a bookmark, this doesnt actually
             // add it but moves it into the add box
      for (int i = 0; i < request.getIntPart("extraindexcount", 0); i++) { // TODO make sure name is
                                                                           // valid
  ...
  public void writeContent(HTMLNode contentNode, MultiValueTable<String, String> headers) {
    ...
    try {
      ...
      contentNode.addChild("script", new String[] {"type", "src"},
          new String[] {"text/javascript", "/library/static/script.js"}).addChild("%", " ");
        ...
        if (search.isDone()) {
          if (search.hasGeneratedResultNode()) {
            ...
            try {
              ...
              ResultNodeGenerator nodegenerator =
                  new ResultNodeGenerator(search.getResult(), groupusk, showold, true); // js is
                                                                                        // being
                                                                                        // switch on
                                                                                        // always
                                                                                        // currently
      ...
      // or js is enabled
      if (search != null && !"".equals(search.getQuery()) && !search.isDone()
          && exceptions.size() <= 0) {
        // refresh will GET so use a request id
        if (!js) {
          headers.put("Refresh", "2;url=" + refreshURL);
          // contentNode.addChild("script", new String[]{"type", "src"}, new
          // String[]{"text/javascript", path() + "static/" + (js ? "script.js" : "detect.js") +
          // "?request="+search.hashCode()+(showold?"&showold=on":"")}).addChild("%", " ");
          // contentNode.addChild("script", new String[]{"type", "src"}, new
          // String[]{"text/javascript", path() + "static/" + (js ? "script.js" : "detect.js") +
          // "?request="+search.hashCode()+(showold?"&showold=on":"")}).addChild("%", " ");
        }
   ...
   * Create search form
   *
   * // @param search already started // @param indexuri // @param js whether js has been detected
   * // @param showold
   *
   * @return an {@link HTMLNode} representing the search form
   */
  private HTMLNode searchBox() {
    HTMLNode searchDiv = new HTMLNode("div", new String[] {"id", "style"},
        new String[] {"searchbar", "text-align: center;"});
    if (pr != null) {
      HTMLNode searchForm = pr.addFormChild(searchDiv, path(), "searchform");
      HTMLNode searchBox = searchForm.addChild("div", "style",
          "display: inline-table; text-align: left; margin: 20px 20px 20px 0px;");
      searchBox.addChild("#", "Search query:");
      searchBox.addChild("br");
      searchBox.addChild("input", new String[] {"name", "size", "type", "value", "title"},
          new String[] {"search", "40", "text", query,
              "Enter a search query. You can use standard search syntax such as 'and', 'or', 'not' and \"\" double quotes around phrases"});
      searchBox.addChild("input", new String[] {"name", "type", "value", "tabindex"},
          new String[] {"find", "submit", "Find!", "1"});
      if (js)
      ...
      HTMLNode indexeslist =
          searchBox.addChild("ul", "class", "index-bookmark-list", "Select indexes");
      for (String bm : library.bookmarkKeys()) {
        // Logger.normal(this, "Checking for bm="+Library.BOOKMARK_PREFIX+bm+" in \""+indexuri + " =
        // " + selectedBMIndexes.contains(Library.BOOKMARK_PREFIX+bm)+"
        // "+indexuri.contains(Library.BOOKMARK_PREFIX+bm));
        HTMLNode bmItem = indexeslist.addChild("li");
        bmItem.addChild("input",
            new String[] {"name", "type", "value", "title",
                (selectedBMIndexes.contains(Library.BOOKMARK_PREFIX + bm) ? "checked" : "size")},
            new String[] {"~" + bm, "checkbox", Library.BOOKMARK_PREFIX + bm,
                "Index uri : " + library.getBookmark(bm), "1"},
            bm);
        bmItem.addChild("input", new String[] {"name", "type", "value", "title", "class"},
            new String[] {Commands.removebookmark + bm, "submit", "X", "Delete this bookmark",
      ...
      for (String uri : selectedOtherIndexes) {
        ...
        removeItem.addChild("input", new String[] {"type", "name", "value", "checked"},
            new String[] {"checkbox", "index" + i, uri, "checked",}, showuri);
        removeItem.addChild("input", new String[] {"name", "type", "value"},
            new String[] {Commands.addindex.toString() + i, "submit", "Add=>"});
        i++;
      }
      indexeslist.addChild("input", new String[] {"name", "type", "value"},
          new String[] {"extraindexcount", "hidden", "" + selectedOtherIndexes.size()});
      indexeslist.addChild("li").addChild("input",
          new String[] {"name", "type", "value", "class", "title"}, new String[] {"indexuris",
              "text", "", "index", "URI or path of other index(s) to search on"});


      HTMLNode optionsBox = searchForm.addChild("div", "style",
          "margin: 20px 0px 20px 20px; display: inline-table; text-align: left;", "Options");
      HTMLNode optionsList = optionsBox.addChild("ul", "class", "options-list");
      optionsList.addChild("li").addChild("input",
          new String[] {"name", "type", groupusk ? "checked" : "size", "title"},
          new String[] {"groupusk", "checkbox", "1",
              "If set, the results are returned grouped by site and edition, this makes the results quicker to scan through but will disrupt ordering on relevance, if applicable to the indexs you are using."},
          "Group sites and editions");
      optionsList.addChild("li").addChild("input",
          new String[] {"name", "type", showold ? "checked" : "size", "title"},
          new String[] {"showold", "checkbox", "1",
              "If set, older editions are shown in the results greyed out, otherwise only the most recent are shown."},
          "Show older editions");

      HTMLNode newIndexInput = optionsBox.addChild("div", new String[] {"class", "style"},
          new String[] {"index", "display: inline-table;"}, "Add an index:");
      newIndexInput.addChild("br");
      newIndexInput.addChild("div", "style", "display: inline-block; width: 50px;", "Name:");
      newIndexInput.addChild("input", new String[] {"name", "type", "class", "title", "value"},
          new String[] {"addindexname", "text", "index",
              "Name of the bookmark, this will appear in the list to the left", addindexname});
      newIndexInput.addChild("br");
      newIndexInput.addChild("div", "style", "display: inline-block; width: 50px;", "URI:");
      newIndexInput.addChild("input", new String[] {"name", "type", "class", "title", "value"},
          new String[] {"addindexuri", "text", "index",
              "URI or path of index to add to bookmarks, including the main index filename at the end of a Freenet uri will help Library not to block in order to discover the index type.",
              addindexuri});
      newIndexInput.addChild("br");
      newIndexInput.addChild("input", new String[] {"name", "type", "value"},
          new String[] {"addbookmark", "submit", "Add Bookmark"});
    } else
   ...
   *
   * // @param search // @param indexuri // @param request request to get progress from
   *
  ...
  private HTMLNode progressBox() throws TaskAbortException {
    ...
    searchingforCell.addChild("span", "class", "librarian-searching-for-target").addChild("b",
        query);
    ...
    statusRow.addChild("td").addChild("div", "id", "librarian-search-status")
        .addChild("table", new String[] {"id", "class"},
            new String[] {"progress-base", "progress-table"})
        .addChild("tbody").addChild(progressBar(search, true));
    return progressDiv;
  }

  /**
   * Put an error on the page, under node, also draws a big grey box around the error, unless it is
   * an InvalidSearchException in which case it just shows the description
   */
  public static void addError(HTMLNode node, Throwable error, StringBuilder messages) {
    if (messages != null
        && (error instanceof InvalidSearchException || error instanceof TaskAbortException)
        && !logMINOR) { // Print description
      ...
      HTMLNode error1 = node.addChild("div", "style",
          "padding:10px;border:5px solid gray;margin:10px", error.toString());
  ...
  /**
   * Draw progress bars and describe progress, CompositeProgess are drawn as a table with each row
   * containing a subProgress
   * 
  ...
  public static HTMLNode progressBar(Progress progress, boolean canFail) throws TaskAbortException {
    synchronized (progress) {
      if (progress instanceof CompositeProgress
          && ((CompositeProgress) progress).getSubProgress() != null
          && ((CompositeProgress) progress).getSubProgress().iterator().hasNext()) {
        // Put together progress bars for all the subProgress
        HTMLNode block = new HTMLNode("#");
        block.addChild("tr").addChild("td", "colspan", "6",
            progress.getSubject() + " : " + progress.getStatus());
        ...
        if (((CompositeProgress) progress).getSubProgress() != null)
          for (Progress progress1 : ((CompositeProgress) progress).getSubProgress()) {
            try {
              ...
              if (firstError == null)
                firstError = e;
              block.addChild("tr").addChild("td", "colspan", "6", progress1.getSubject() + " : "
                  + L10nString.getString("failed") + " : " + e.getMessage());
            }
          }
        ...
        if (progress instanceof ChainedProgress
            && ((ChainedProgress) progress).getCurrentProgress() != null)
        ...
        if (progress.isDone() || progress.getParts().known == 0) {
          ...
          int percentage = (int) (((float) 100) * fractiondone); // TODO cater for all data and
                                                                 // invalid (negative) values
          boolean fetchFinalized = parts.finalizedTotal();

          bar.addChild("td", new String[] {"class", "style"},
              new String[] {"progress-bar-outline", "padding: 0px 3px;"}).addChild("div",
                  new String[] {"class", "style"},
                  new String[] {
                      fetchFinalized ? "progress-bar-inner-final" : "progress-bar-inner-nonfinal",
                      "z-index : -1; width:" + percentage + "%;"});
