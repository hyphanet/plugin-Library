...
class MainPage {
    ...
    public static MainPage processPostRequest(HTTPRequest request, HTMLNode contentNode,
        ...
        String[] etcIndexes = request.getPartAsStringFailsafe("indexuris",
                                  256).trim().split("[ ;]");
        ...
        if (request.isPartSet(Commands.find.toString())) {
                ...
                try {
                    if (logMINOR) {
                        Logger.minor(MainPage.class,
                                     "starting search for " + page.query + " on " +
                                     page.indexstring);
                    }
                    ...
                    if (page.search == null) {
                        ...
                        page.search.setMakeResultNode(
                            page.groupusk, page.showold,
                            true);  // for the moment js will always be on for results, js detecting isnt being used
            ...
            for (int i = 0; i < request.getIntPart("extraindexcount", 0);
                    i++) {  // TODO make sure name is valid
    ...
    public void writeContent(HTMLNode contentNode, MultiValueTable<String, String> headers) {
        ...
        try {
            ...
            contentNode.addChild("script", new String[] { "type", "src" },
                                 new String[] { "text/javascript",
                    "/library/static/script.js" }).addChild("%", " ");
                    ...
                    if (search.hasGeneratedResultNode()) {
                        ...
                        try {
                            ...
                            ResultNodeGenerator nodegenerator =
                                new ResultNodeGenerator(
                                    search.getResult(), groupusk, showold,
                                    true);  // js is being switch on always currently due to detection being off
    ...
    private HTMLNode searchBox() {
        ...
        if (pr != null) {
            ...
            if (js) {
                searchBox.addChild("input", new String[] { "type", "name" },
                                   new String[] { "hidden",
                        "js" });
            }

            // Shows the list of bookmarked indexes TODO show descriptions on mouseover ??
            HTMLNode indexeslist = searchBox.addChild("ul", "class", "index-bookmark-list",
                                       "Select indexes");
            ...
            for (String bm : library.bookmarkKeys()) {
                ...
                bmItem.addChild("input", new String[] { "name", "type", "value", "title",
                        (selectedBMIndexes.contains(Library.BOOKMARK_PREFIX + bm)
                         ? "checked" : "size") }, new String[] { "~" + bm,
                        "checkbox", Library.BOOKMARK_PREFIX + bm,
                        "Index uri : " + library.getBookmark(bm), "1" }, bm);
                bmItem.addChild("input",
                                new String[] { "name", "type", "value", "title", "class" },
                                new String[] { Commands.removebookmark + bm,
                        "submit", "X", "Delete this bookmark", "index-bookmark-delete" });
            }
            ...
            for (String uri : selectedOtherIndexes) {
                ...
                removeItem.addChild("input", new String[] { "type", "name", "value", "checked" },
                                    new String[] { "checkbox",
                        "index" + i, uri, "checked", }, showuri);
                removeItem.addChild("input", new String[] { "name", "type", "value" },
                                    new String[] { Commands.addindex.toString() + i,
                        "submit", "Add=>" });
                i++;
            }

            indexeslist.addChild("input", new String[] { "name", "type", "value" },
                                 new String[] { "extraindexcount",
                    "hidden", "" + selectedOtherIndexes.size() });
            indexeslist.addChild("li").addChild("input", new String[] { "name", "type", "value",
                    "class", "title" }, new String[] { "indexuris", "text", "", "index",
                    "URI or path of other index(s) to search on" });

            HTMLNode optionsBox =
                searchForm.addChild(
                    "div", "style",
                    "margin: 20px 0px 20px 20px; display: inline-table; text-align: left;",
                    "Options");
            HTMLNode optionsList = optionsBox.addChild("ul", "class", "options-list");

            optionsList.addChild("li").addChild("input", new String[] { "name", "type",
                    groupusk ? "checked" : "size", "title" }, new String[] { "groupusk", "checkbox",
                    "1",
                    "If set, the results are returned grouped by site and edition, this makes the results quicker to scan through but will disrupt ordering on relevance, if applicable to the indexs you are using." }, "Group sites and editions");
            optionsList.addChild("li").addChild("input", new String[] { "name", "type",
                    showold ? "checked" : "size", "title" }, new String[] { "showold", "checkbox",
                    "1",
                    "If set, older editions are shown in the results greyed out, otherwise only the most recent are shown." }, "Show older editions");

            HTMLNode newIndexInput = optionsBox.addChild("div", new String[] { "class", "style" },
                                         new String[] { "index",
                    "display: inline-table;" }, "Add an index:");

            newIndexInput.addChild("br");
            newIndexInput.addChild("div", "style", "display: inline-block; width: 50px;", "Name:");
            newIndexInput.addChild("input", new String[] { "name", "type", "class", "title",
                    "value" }, new String[] { "addindexname", "text", "index",
                    "Name of the bookmark, this will appear in the list to the left",
                    addindexname });
            newIndexInput.addChild("br");
            newIndexInput.addChild("div", "style", "display: inline-block; width: 50px;", "URI:");
            newIndexInput.addChild("input", new String[] { "name", "type", "class", "title",
                    "value" }, new String[] { "addindexuri", "text", "index",
    ...
    private HTMLNode progressBox() throws TaskAbortException {
        ...
        statusRow.addChild("td").addChild("div", "id", "librarian-search-status").addChild("table",
                           new String[] { "id",
                "class" }, new String[] { "progress-base",
                "progress-table" }).addChild("tbody").addChild(progressBar(search, true));
    ...
    public static void addError(HTMLNode node, Throwable error, StringBuilder messages) {
        if ((messages != null) &&
                ((error instanceof InvalidSearchException) ||
                 (error instanceof TaskAbortException)) && !logMINOR) {  // Print description
     ...
     */
    public static HTMLNode progressBar(Progress progress, boolean canFail)
        ...
        synchronized (progress) {
            if ((progress instanceof CompositeProgress) &&
                ...
                if (progress.isDone() || (progress.getParts().known == 0)) {
                    ...
                    int percentage =
                        (int) (((float) 100) *
                            fractiondone);  // TODO cater for all data and invalid (negative) values
                    boolean fetchFinalized = parts.finalizedTotal();

                    bar.addChild("td", new String[] { "class", "style" },
                                 new String[] { "progress-bar-outline",
                                                "padding: 0px 3px;" }).addChild("div",
                                                new String[] { "class",
                            "style" }, new String[] {
                                fetchFinalized
                                ? "progress-bar-inner-final" : "progress-bar-inner-nonfinal",
                                "z-index : -1; width:" + percentage + "%;" });
                    bar.addChild("td",
                                 fetchFinalized ? percentage + "%" : "Operation length unknown");
                }
