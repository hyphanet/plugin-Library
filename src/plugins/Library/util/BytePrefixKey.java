 ...
 ** @author infinity0
 */
abstract public class BytePrefixKey<K extends BytePrefixKey<K>> extends AbstractPrefixKey<K>
  ...
  public static byte[] hexToBytes(String hex) {
    ...
    for (int i = 0, j = 0; i < bs.length; ++i) {
      ...
      char l = hex.charAt(j++);
      int hi = ('0' <= h && h <= '9') ? h - '0'
          : ('a' <= h && h <= 'f') ? h - 'a' + 10 : ('A' <= h && h <= 'F') ? h - 'A' + 10 : err(h);
      int li = ('0' <= l && l <= '9') ? l - '0'
          : ('a' <= l && l <= 'f') ? l - 'a' + 10 : ('A' <= l && l <= 'F') ? l - 'A' + 10 : err(l);
      bs[i] = (byte) ((hi << 4) | li);
    }
    return bs;
  }
  ...
  /*
   * Bean getter and setter for this PrefixKey public String getString() { return toString(); }
   * 
   * public void setString(String s) { if (s.length != hash.length<<1) { throw new
   * IllegalArgumentException("Incorrect length of string for this BytePrefixKey."); }
