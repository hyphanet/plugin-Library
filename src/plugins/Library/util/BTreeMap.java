...
public class BTreeMap<K, V> extends AbstractMap<K, V>
    ...
    protected class Node implements Comparable<Node> {
        ...
        public Iterable<X3<K, Node, K>> iterNodesK() {
            if (_iterNodesK == null) {
                _iterNodesK = new CompositeIterable<X2<K, K>, X3<K, Node, K>>(iterKeyPairs(),
                        true) {
                    @Override
    ...
    final public static <K> int compare(K key1, K key2, Comparator<? super K> comparator) {
        return (comparator != null)
               ? comparator.compare(key1, key2) : ((Comparable<K>) key1).compareTo(key2);
    }
    ...
    final public static <K> boolean compare0(K key1, K key2, Comparator<? super K> comparator) {
        return (key1 == null)
               ...
               : (key2 == null)
                 ? false
                 : ((comparator != null)
                    ? comparator.compare(key1, key2) : ((Comparable<K>) key1).compareTo(key2)) == 0;
    }
    ...
    private K split(Node parent, Node child) {
        ...
        assert((parent == null)
               ? ((child.lkey == null) && (child.rkey == null))
               : ((parent.nodeSize() < ENT_MAX) && !parent.isLeaf() &&
    ...
    public V remove(Object k) {
        ...
        for (;;) {
            ...
            if ((node != root) && (node.nodeSize() == ENT_MIN)) {
                ...
                // lnode doesn't exist
                (L < 0) ? ((R == ENT_MIN)
                           ? merge(parent, node, rnode) : rotateL(parent, node, rnode)) :

                // rnode doesn't exist
                (R < 0) ? ((L == ENT_MIN)
                           ? merge(parent, lnode, node) : rotateR(parent, lnode, node)) :

                // pick the node with more entries
                (R > L) ? rotateL(parent, node, rnode) : (L > R) ? rotateR(parent, lnode, node) :

                // otherwise pick one at "random"
                (size & 1) == 1 ? ((R == ENT_MIN)
                                   ? merge(parent, node, rnode)
                                   : rotateL(parent, node, rnode)) : ((L == ENT_MIN)
                                       ? merge(parent, lnode, node) : rotateR(parent, lnode, node));
            ...
            if (nextnode == null) {    // key is already in the node
                ...
                // otherwise pick one at "random"
                (size & 1) == 1 ? ((R == ENT_MIN)
                                   ? merge(node, lnode, rnode)
                                   : rotateL(node, lnode, rnode)) : ((L == ENT_MIN)
                                       ? merge(node, lnode, rnode) : rotateR(node, lnode, rnode));
                        ...
                        public Map.Entry<K, V> next() {
                            if (cnode.isLeaf()) {
                                ...
                                while ( !cnode.isLeaf()) {
                                    ...
                                    testnode
                                        .isLeaf();    // trigger DataNotLoadedException if this is a GhostNode
