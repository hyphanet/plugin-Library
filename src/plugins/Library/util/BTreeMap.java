 ...
 * for that use {@link TreeMap}.
 **
 ** From [http://en.wikipedia.org/wiki/B-tree wikipedia]: A B-tree is a tree data structure that
 * keeps data sorted and allows searches, insertions, and deletions in logarithmic amortized time.
 * Unlike self-balancing binary search trees, it is optimized for systems that read and write large
 * blocks of data. It is most commonly used in databases and filesystems.
 **
 ...
 * increase slowly as elements are added to the tree, but an increase in the overall depth is
 * infrequent, and results in all leaf nodes being one more node further away from the root.
 **
 ** A B-tree of order m (the maximum number of children for each node) is a tree which satisfies the
 * following properties:
 **
 ** * Every node has between m/2-1 and m-1 entries, except for the root node, which has between 1 and
 * m-1 entries. * All leaves are the same distance from the root. * Every non-leaf node with k
 * entries has k+1 subnodes arranged in sorted order between the entries (for details, see
 * {@link Node}).
 **
 ...
 * storage.
 **
 ** If {@link #NODE_MIN} is {@linkplain #BTreeMap(int) set to} 2, the resulting tree is structurally
 * and algorithmically equivalent to a 2-3-4 tree, which is itself equivalent to a red-black tree,
 * which is the implementation of {@link TreeMap} in the standard Java Class Library. This class has
 * extra overheads due to the B-tree generalisation of the structures and algorithms involved, and
 * is therefore only meant to be used to represent a B-tree stored in secondary storage, as the
 * above paragraph describes.
 **
 ** NOTE: at the moment there is a slight bug in this implementation that causes '''indeterminate
 * behaviour''' when used with a comparator that admits {@code
** null} keys. This is due to {@code null} having special semantics, meaning "end of map", for the
 * {@link Node#lkey} and {@link Node#rkey} fields. This is NOT an urgent priority to fix, but might
...
public class BTreeMap<K, V> extends AbstractMap<K, V>
   ...
   **
   ** TODO HIGH clean up the use of Node._size TODO HIGH make Node's fields private/immutable TODO
   * LOW use Node.{merge|split} instead of the long code in BTreeMap.{merge|split} FIXME LOW {@link
   * ConcurrentModificationException} for the entrySet iterator
   **
   ***************************************************************************
   ** IMPORTANT NOTE FOR MAINTAINERS:
   ***************************************************************************
   **
   ** When navigating the tree, either Node.isLeaf() or Node.nodeSize() MUST be called at least once
   * **for each node visited**, before accessing or modifying its contents. This is to give
   * SkeletonBTreeMap the chance to throw a DataNotLoadedException in case the node has not been
   * loaded.
   **
   ** Additionally, each node has a Node._size field which is a cache for Node.totalSize(), which
   * calculates the number of entries in it and all its children. To invalidate the cache, set
   * Node._size to -1. This must be done whenever an operation has >0 chance of changing the total
   * size.
   **
   ** FIXME LOW atm there is a slight bug in this implementation that causes indeterminate behaviour
   * when used with a comparator that admits null keys. This is due to null having special semantics
   * ("end of map"), for the lkey and rkey fields. This is NOT an urgent priority to fix, but might
   * be done in the future.
   **
   ** NULLNOTICE marks the parts of the code which are affected by this logic, as well as all
  ...
  /**
   ** Root node of the tree. The only node that can have less than ENT_MIN entries.
   */
  ...
  /**
   ** Creates a new empty map, sorted according to the given comparator, and with each non-root node
   * having the given minimum number of subnodes.
   **
   ** @param cmp The comparator for the tree, or {@code null} to use the keys' {@link Comparable
   *        natural} ordering.
  ...
  public BTreeMap(Comparator<? super K> cmp, int node_min) {
    if (node_min < 2) {
      throw new IllegalArgumentException(
          "The minimum number of subnodes must be set to at least 2");
    }
    comparator = cmp;
    NODE_MIN = node_min;
    NODE_MAX = node_min << 1;
  ...
  /**
   ** Creates a new empty map, sorted according to the keys' {@link Comparable natural} ordering, and
  ...
  /**
   ** Returns the subset of the given {@link SortedSet}, exclusively between the given keys. If
   * either key is {@code null}, this will be treated to mean the relevant edge of the set.
   **
   ** OPT NORM
   */
  public static <K> SortedSet<K> subSet(SortedSet<K> set, K lk, K rk) {
    ...
    if (lk == null) {
      ...
      if (ss.isEmpty() || !compare0(ss.first(), lk, set.comparator())) {
        return ss;
      }
   ...
   **
   ** * Every entry in a non-leaf node has two adjacent subnodes, holding entries which are all
   * strictly greater and smaller than the entry. * Conversely, every subnode has two adjacent
   * entries, which are strictly greater and smaller than every entry in the subnode. Note: edge
   ** subnodes' "adjacent" keys are not part of the node's entries, but inherited from some ancestor
   * parent.)
   **
   ** .....^.........................Node........................^
   ** ......|.....................................................|
   ...
   **
   ** Note: before anyone gets any ideas about making this class static (believe me, I tried), note
   * that {@link TreeMap} '''requires''' the comparator to be passed at construction. The neatest
   * way to do this is to pass either the tree or the comparator to the node's constructor.
   **
   ** This is fine in the context of {@link BTreeMap}, but serialisation makes it extremely awkard.
   * Passing the tree requires us to extend {@link plugins.Library.io.serial.Translator} to take a
   * context paramater, and passing the comparator requires us to add checks to ensure that the
   ** comparators are equal when the node is re-attached to the tree.
   **
   ** And even if this is implemented, nodes still need to know their parent tree (to set
   * {@link plugins.Library.io.serial.Serialiser}s etc) and so you would need to code a secondary
   * initialisation scheme to be called after object construction, and after execution returns from
   * {@link plugins.Library.io.serial.Translator#rev(Object)}. All in all, this is more trouble than
   * it's worth.
   **
   ** So I've gone with the non-static class, which means a new {@code
  ** Translator} needs to be constructed for each new B-tree. This is only two fields above an
   * {@link Object} (if you re-use the same objects for the
   * {@link SkeletonBTreeMap.NodeTranslator#ktr} and {@code
  ** SkeletonBTreeMap.NodeTranslator#mtr} fields) and so should be negligible compared to the memory
   * size of the rest of the B-tree.
   **
   ** @author infinity0
   */
  protected class Node implements Comparable<Node> {
    ...
    /**
     ** Map of entries to their immediate smaller nodes. The greatest node is mapped to by
     * {@link #rkey}.
     */
    final /* private */ protected Map<K, Node> lnodes;

    /**
     ** Map of entries to their immediate greater nodes. The smallest node is mapped to by
     * {@link #lkey}.
     */
    final /* private */ protected Map<K, Node> rnodes;

    /**
     ** Greatest key smaller than all keys in this node and subnodes. This is {@code null} when the
     * node is at the minimum-key end of the tree.
     */
    /* final */ protected K lkey;

    /**
     ** Smallest key greater than all keys in this node and subnodes. This is {@code null} when the
     * node is at the maximum-key end of the tree.
     */
    /* final */ protected K rkey;

    /**
     ** Cache for the total number of entries in this node and all subnodes. A negative value means
     * the cache was invalidated.
     */
    transient int _size = -1;

    /**
     ** Creates a new node for the BTree, with a custom map to store the entries.
     **
     ** It is '''assumed''' that the input map is empty; it is up to the caller to ensure that this
     * holds, or if not, then at least put the node into a consistent state before releasing it for
     * use.
     **
    ...
    /**
     ** Add the given entries and subnodes to this node. The subnodes are added with
     * {@link #addChildNode(BTreeMap.Node)}.
     **
     ** It is '''assumed''' that the entries are located exactly exclusively between the nodes (ie.
     * like in an actual node); it is up to the caller to ensure that this holds.
     **
    ...
    /**
     ** @return Whether the node is a ghost. This cannot be {@code true} for normal maps, but is used
     *         by {@link SkeletonBTreeMap}.
     */
     ...
     **
     ** It is '''assumed''' that the input is an actual subnode of this node; it is up to the caller
    ...
    public Node nodeL(Node node) {
      assert (lnodes.get(node.rkey) == node && rnodes.get(node.lkey) == node);
    ...
    /**
     ** Returns the subnode a that given key belongs in. The key must not be local to the node.
     **
     ** It is '''assumed''' that the input key is in the correct range; it is up to the caller to
     * ensure that this holds.
     **
     ** @param key The key to select the node for
     ** @return The subnode for the key, or null if the key is local to the node
     ** @throws NullPointerException if this is a leaf node; or if the key is {@code null} and the
     *         entries map uses natural order, or its comparator does not tolerate {@code null} keys
     */
     ...
     **
     ** It is '''assumed''' that both keys are in the local entries map, and that there is no node
     * between these keys. It is up to the caller to ensure that this holds.
     **
    ...
    /**
     ** A view of all subnodes with their lkey and rkey, as an {@link Iterable}. Iteration occurs in
     * '''sorted''' order.
     **
     ** TODO LOW this method does not check that this node actually has subnodes (ie. non-leaf). This
    ...
    public Iterable<X3<K, Node, K>> iterNodesK() {
      if (_iterNodesK == null) {
        _iterNodesK = new CompositeIterable<X2<K, K>, X3<K, Node, K>>(iterKeyPairs(), true) {
          @Override
          protected X3<K, Node, K> nextFor(X2<K, K> kp) {
    ...
    /**
     ** A view of all subnodes as an {@link Iterable}. Iteration occurs in '''no particular order'''.
     **
     ** TODO LOW this method does not check that this node actually has subnodes (ie. non-leaf). This
     * may or may not change in the future.
     */
    ...
    /**
     ** A view of all local keys as an {@link Iterable}. Iteration occurs in '''sorted''' order.
     */
    ...
    /**
     ** A view of all keys (including {@code lkey}, {@code rkey}) as an {@link Iterable}. Iteration
     * occurs in '''sorted''' order.
     */
    ...
    /**
     ** A view of adjacent key pairs (including {@code lkey}, {@code rkey}) as an {@link Iterable}.
    ...
    /**
     ** A view of child nodes exclusively between {@code lk}, {@code rk}, as an {@link Iterable}.
     * Iteration occurs in '''sorted''' order.
     **
     ** It is '''assumed''' that the input keys are all local to the node (or its lkey/rkey), and in
     * the correct order; it is up to the caller to ensure that this holds.
     **
    ...
    /**
     ** A view of local entries exclusively between {@code lk}, {@code rk}, as a {@link SortedMap}.
     **
     ** It is '''assumed''' that the input keys are all local to the node (or its lkey/rkey), and in
     * the correct order; it is up to the caller to ensure that this holds.
     **
    ...
    /**
     ** Merge child nodes between the given keys. The merged node is added to this node, and the old
     * children are discarded.
     **
     ** It is '''assumed''' that the input keys are all local to the node, and in the correct order;
    ...
    /**
     ** Split a child node at the given keys. The split children are added to this node, and the old
     * child is discarded.
     **
     ** It is '''assumed''' that the input keys are all local to the node, and in the correct order;
     * it is up to the caller to ensure that this holds.
    ...
    /**
     ** Defines a total ordering on the nodes. This is useful for eg. deciding which nodes to visit
     * first in a breadth-first search algorithm that uses a priority queue
     * ({@link java.util.concurrent.PriorityBlockingQueue}) instead of a FIFO
     ...
     **
     ** * Its right endpoint is strictly smaller than the other's, or * Its right endpoint is
     * compare-equal to the other's, and its left endpoint is strictly greater than the other's.
     * (Note that this case would never be reached in a breadth-first search.)
     **
     ** Or, equivalently, and perhaps more naturally (but less obvious why it is a total order):
     **
     ** * Both its endpoints are strictly smaller than the other's corresponding endpoints, or * Both
     * of its endpoints are contained within the other's endpoints, and one of them strictly so.
     **
  ...
  /**
   ** Compares two keys using the comparator for this tree, or the keys' {@link Comparable natural}
   * ordering if no comparator was given.
   **
   ** @throws ClassCastException if the keys cannot be compared by the given comparator, or if they
   *         cannot be compared naturally (ie. they don't implement {@link Comparable})
  ...
  /**
   ** Tests for compare-equality. Two values are compare-equal if they are both {@code null} or they
   * {@link #compare(Object, Object)} to {@code 0}. We use this rather than {@link #equals(Object)}
   * to maintain sorting order consistency in case the comparator is inconsistent with equals.
   **
   ** @throws ClassCastException if the keys cannot be compared by the given comparator, or if they
   *         cannot be compared naturally (ie. they don't implement {@link Comparable})
   */
  final public static <K> boolean compare0(K key1, K key2, Comparator<? super K> comparator) {
    return key1 == null ? key2 == null
        : key2 == null ? false
            : ((comparator != null) ? comparator.compare(key1, key2)
                : ((Comparable<K>) key1).compareTo(key2)) == 0;
  }
  ...
  /**
   ** Comparison between {@link Node#lkey} values, where {@code null} means "a value smaller than any
   * other". Use this when one of the operands is the {@code lkey} of some node.
   **
   ** @throws ClassCastException if the keys cannot be compared by the given comparator, or if they
   *         cannot be compared naturally (ie. they don't implement {@link Comparable})
   */
  ...
  /**
   ** Comparison between {@link Node#rkey} values, where {@code null} means "a value greater than any
   * other". Use this when one of the operands is the {@code rkey} of some node.
   **
   ** @throws ClassCastException if the keys cannot be compared by the given comparator, or if they
   *         cannot be compared naturally (ie. they don't implement {@link Comparable})
   */
   ...
   ** For non-leaf nodes:
   **
   ** * for each pair of adjacent keys (including two null keys either side of the list of keys), it
   * is possible to traverse between the keys, and the single node between them, using the lnodes,
   * rnodes maps and the lkey, rkey pointers. * lnodes' and rnodes' size is 1 greater than that of
   * entries. Ie. there is nothing else in the node beyond what we visited in the previous step.
   **
   ** It also checks the BTree node constraints:
   **
   ** * The node has at most ENT_MAX entries. * The node has at least ENT_MIN entries, except the
   * root. * The root has at least 1 entry.
   **
  ...
  final void verifyNodeIntegrity(Node node) {
    ...
    /*
     * DEBUG if (node._size > 0 && node._size != s) { System.out.println(node._size + " vs " + s);
  ...
  /**
   ** Package-private debugging method. This one checks BTree constraints for the subtree rooted at a
   * given node:
   **
   ** * All nodes follow the node constraints, listed above. * All leaves appear in the same level
   **
  ...
  /**
   ** Package-private debugging method. This one checks BTree constraints for the entire tree.
   **
   ** @throws IllegalStateException if the constraints are not satisfied
   */
  ...
  /**
   ** Move the given key from the given source node to the given target node. This method should be
   * used whenever entries need to be moved, so that eg. we can proceed even when the values have
   * not been loaded (such as in a {@link SkeletonBTreeMap}).
   **
   ** FIXME HIGH do this for everything in the class. ATM only Node.split() uses it.
   */
  ...
  /**
   ** Split a maximal child node into two minimal nodes, using the median key as the separator
   * between these new nodes in the parent. If the parent is {@code null}, creates a new
   * {@link Node} and points {@link #root} to it.
   **
   ** It is '''assumed''' that the child is an actual subnode of the parent, and that it is full. It
   * is up to the caller to ensure that this holds.
   **
   ** The exact implementation of this may change from time to time, so the calling code should
   * regard the input subnode as effectively destroyed, and re-get the desired result subnode from
   * calling the appropriate get methods on the parent node.
   **
  ...
  private K split(Node parent, Node child) {
    ...
    assert (parent == null ? (child.lkey == null && child.rkey == null)
        : (parent.nodeSize() < ENT_MAX && !parent.isLeaf() && parent.rnodes.get(child.lkey) == child
  ...
  /**
   ** Merge two minimal child nodes into a maximal node, using the key that separates them in the
   * parent as the key that joins the halfnodes in the merged node. If the parent is the
   * {@link #root} and the merge makes it empty, point {@code root} to the new merged node.
   **
   ** It is '''assumed''' that both childs are actual subnodes of the parent, that they are adjacent
   * in the parent, and that they are minimally full. It is up to the caller to ensure that this
   * holds.
   **
   ** The exact implementation of this may change from time to time, so the calling code should
   * regard both input subnodes as effectively destroyed, and re-get the desired result subnode from
   * calling the appropriate get methods on the parent node.
   **
  ...
  private K merge(Node parent, Node lnode, Node rnode) {
    ...
    assert (parent == root && parent.nodeSize() > 0 || parent.nodeSize() > ENT_MIN);
    ...
    if (parent == root && parent.entries.isEmpty()) {
      assert (parent.lkey == null && parent.rkey == null && rnode.lkey == null
          && rnode.rkey == null);
      root = rnode;
    }
  ...
  /**
   ** Performs a rotate operation towards the smaller node. A rotate operation is where:
   **
   ** * an entry M from the parent node is moved to a subnode * an entry S from an adjacent subnode
   * is moved to fill gap left by M * a subnode N associated with S is cut from M and attached to S
   **
   ** with the operands chosen appropriately to maintain tree constraints.
   **
   ** Here, M is the key that separates {@code lnode} and {@code rnode}, S is the smallest key of
   * {@code rnode}, and N its smallest subnode.
   **
   ** It is '''assumed''' that both childs are actual subnodes of the parent, that they are adjacent
   * in the parent, and that the child losing an entry has more than {@link #ENT_MIN} entries. It is
  ...
  /**
   ** Performs a rotate operation towards the greater node. A rotate operation is where:
   **
   ** * an entry M from the parent node is moved to a subnode * an entry S from an adjacent subnode
   * is moved to fill gap left by M * a subnode N associated with S is cut from M and attached to S
   **
   ** with the operands chosen appropriately to maintain tree constraints.
   **
   ** Here, M is the key that separates {@code lnode} and {@code rnode}, S is the greatest key of
   * {@code rnode}, and N its greatest subnode.
   **
   ** It is '''assumed''' that both childs are actual subnodes of the parent, that they are adjacent
   * in the parent, and that the child losing an entry has more than {@link #ENT_MIN} entries. It is
   * up to the caller to ensure that this holds.
  ...
  /**
   ** Gives a quick estimate of the height of the tree. This method will tend to over-estimate rather
   * than underestimate.
   **
   ** The exact formula is {@code 1 + floor(log(size)/log(NODE_MIN))}. I couldn't be bothered trying
   * to prove that this is a firm upper bound; feel free.
   */
  ...
  /**
   ** Returns the minimum number of nodes (at a single level) needed for the given number of entries.
   */
  ...
  /**
   ** Returns the minimum number of separator keys (at a single level) needed for the given number of
   * entries.
   */
  ...
  public Map.Entry<K, V> getEntry(int index) {
    ...
    for (;;) {
      ...
      // node.entries.tailMap(cache.get(cache.tailMap(index).firstKey())) // firstEntry().getValue()
      // is Java 6 only :(
   ...
   ** {@inheritDoc}
   **
   ** This implementation just descends the tree, returning {@code true} if it finds the given key.
   **
   ** @throws ClassCastException key cannot be compared with the keys currently in the map
   ** @throws NullPointerException key is {@code null} and this map uses natural order, or its
   *         comparator does not tolerate {@code null} keys
   */
  @Override
   ...
   * 
   * @Override public boolean containsValue(Object key) { throw new
   ...
   ** {@inheritDoc}
   **
   ** This implementation just descends the tree, returning the value for the given key if it can be
   * found.
   **
   ** @throws ClassCastException key cannot be compared with the keys currently in the map
   ** @throws NullPointerException key is {@code null} and this map uses natural order, or its
   ...
   ** {@inheritDoc}
   **
   ** This implementation uses the BTree single-pass insertion algorithm. We descend the tree,
   * restructuring it as needed so that we can insert an entry into a leaf.
   **
   ** Start at the root node. At each stage of the algorithm, we restructure the tree so that the
   * next node we reach has less than {@link #ENT_MAX} entries, and the insertion can occur without
   * breaking constraints.
   **
   ** (*) If the number of entries is {@link #ENT_MAX}, then perform {@link #split(Node, Node) split}
   * on the node. Both the newly created nodes now have less than {@link #ENT_MAX} entries; pick the
   * appropriate halfnode to for the rest of the operation, depending on the original input key.
   **
   ** If the node is a leaf, insert the value and stop. Otherwise, if the key is not already in the
   * node, select the appropriate subnode and repeat from (*).
   **
  ...
  public V put(K key, V value) {
    if (key == null) {
      throw new UnsupportedOperationException(
          "Sorry, this BTreeMap implementation can't handle null keys, even if the comparator supports it.");
    }
   ...
   ** {@inheritDoc}
   **
   ** This implementation uses the BTree single-pass deletion algorithm. We descend the tree,
   * restructuring it as needed so that we can delete the entry from a leaf.
   **
   ** Start at the root node. At each stage of the algorithm, we restructure the tree so that the
   * next node we reach has more than {@link #ENT_MIN} entries, and the deletion can occur without
   * breaking constraints.
   **
   ** (*) If the node is not the root, and if the number of entries is equal to {@link #ENT_MIN},
   * select its two siblings (L and R). Perform one of the following operations (ties being broken
   * arbitrarily):
   **
   ** * {@link #merge(Node, Node, Node) merge} with X, if X also has {@link #ENT_MIN} entries *
   * {@link #rotateL(Node, Node, Node) rotateL} with R, if the R subnode has more entries than L (or
   * equal) * {@link #rotateR(Node, Node, Node) rotateR} with L, if the L subnode has more entries
   * than R (or equal)
   **
   ** The selected node now has more than {@link #ENT_MIN} entries.
   **
   ** If the node is a leaf, remove the value and stop. Otherwise, if the key is not already in the
   * node, select the appropriate subnode and repeat from (*).
   **
   ** Otherwise, select the two subnodes (L and R) that this key separates. Perform one of the
   * following operations (ties being broken arbitrarily):
   **
   ** * {@link #merge(Node, Node, Node) merge}, if both subnodes have {@link #ENT_MIN} entries. *
   * {@link #rotateL(Node, Node, Node) rotateL}, if the R subnode has more entries than L (or equal)
   ** * {@link #rotateR(Node, Node, Node) rotateR}, if the L subnode has more entries than R (or
   * equal)
   **
   ** The node that the key ended up in now has more than {@link #ENT_MIN} entries (and will be
    ...
    for (;;) {
      ...
      if (node != root && node.nodeSize() == ENT_MIN) {
        ...
        K kk = // in java, ?: must be used in a statement :|
            // lnode doesn't exist
            (L < 0) ? ((R == ENT_MIN) ? merge(parent, node, rnode) : rotateL(parent, node, rnode)) :
            // rnode doesn't exist
                (R < 0)
                    ? ((L == ENT_MIN) ? merge(parent, lnode, node) : rotateR(parent, lnode, node))
                    :
                    ...
                    (R > L) ? rotateL(parent, node, rnode)
                        ...
                        // otherwise pick one at "random"
                            (size & 1) == 1
                                ? (R == ENT_MIN ? merge(parent, node, rnode)
      ...
      if (nextnode == null) { // key is already in the node
        ...
        K kk =
            ...
            (R > L) ? rotateL(node, lnode, rnode) : (L > R) ? rotateR(node, lnode, rnode) :
            // otherwise pick one at "random"
                (size & 1) == 1
                    ? (R == ENT_MIN ? merge(node, lnode, rnode) : rotateL(node, lnode, rnode))
                    : (L == ENT_MIN ? merge(node, lnode, rnode) : rotateR(node, lnode, rnode));
   ...
   ** {@inheritDoc}
   **
   ** This implementation iterates over the given map's {@code entrySet}, adding each mapping in
   * turn, except for when {@code this} map is empty, and the input map is a non-empty
   * {@link SortedMap}. In this case, it uses the BTree bulk-loading algorithm:
   **
   ** * distribute all the entries of the map across the least number of nodes possible, excluding
   * the entries that will act as separators between these nodes * repeat for the separator entries,
   * and use them to join the nodes from the previous level appropriately to form a node at this
   * level * repeat until there are no more entries on a level, at which point use the (single) node
   * from the previous level as the root
   **
  ...
  public void putAll(Map<? extends K, ? extends V> t) {
    ...
    if (t.isEmpty()) {
      ...
      if (!(comparator == null && map.comparator() == null
          || comparator.equals(map.comparator()))) {
        super.putAll(map);
        return;
      }
      ...
      while (map.size() > 0) {
        ...
        for (Integer n : Integers.allocateEvenly(map.size() - k + 1, k)) {
          ...
          if (en != null) {
            nextmap.put(prevkey = en.getKey(), en.getValue());
          }
        }
   ...
   */
  private Map.Entry<K, V> makeNode(Iterator<Map.Entry<K, V>> it, int n, K prevkey,
      Map<K, Node> lnodes, Map<K, Node> nextlnodes) {
    Node node;
  ...
  /**
   ** Restructures the tree, distributing the entries evenly between the leaf nodes. This method
   * merely calls {@link #putAll(Map)} with {@code this}.
   */
      ...
      entrySet = new AbstractSet<Map.Entry<K, V>>() {
        ...
        public Iterator<Map.Entry<K, V>> iterator() {
          ...
          return new Iterator<Map.Entry<K, V>>() {
            ...
            /*
             * public String toString() { StringBuilder s = new StringBuilder(); for (Node n:
             * nodestack) { s.append(n.getRange()).append(", "); } return "nodestack: [" + s +
             * "]; cnode: " + cnode.getRange() + "; lastkey: " + lastkey; }
             */
            ...
            /* @Override **/ public Map.Entry<K, V> next() {
              ...
              Map.Entry<K, V> next = centit.next();
              lastkey = next.getKey();
              removeok = true;
              return next;
            }
            ...
            /* @Override **/ public void remove() {
              if (!removeok) {
                throw new IllegalStateException(
                    "Iteration has not yet begun, or the element has already been removed.");
              }
  ...
  /*
   * provided by AbstractMap OPT LOW - the default clear() method is inefficient
   * 
   * @Override public Set<K> keySet() { throw new UnsupportedOperationException("not implemented");
   * }
   */
   ...
   * provided by AbstractMap OPT LOW - the default clear() method is inefficient
   * 
   * @Override public Collection<V> values() { throw new
   * UnsupportedOperationException("not implemented"); }
   */
