...
/**
 ** A {@link SortedSet} backed by a {@link SortedMap}, with one additional bonus method which I felt
 * was lacking from the {@link Set} interface, namely {@link #get(Object)}.
 **
 ** This implementation assumes that for the backing map, looking up items by key is more efficient
 * than by value. It is up to the programmer to ensure that this holds; most maps are designed to
...
public class SortedMapSet<E, M extends SortedMap<E, E>> extends AbstractSet<E>
   ...
   **
   ** It is '''assumed''' that every key already in this map, is mapped to itself. It is up to the
   ...
   **
   ** Note: this test is disabled (will always return true) since it is impossible to test this for
   * partially-loaded data structures. Code kept here to remind future programmers of this.
   **
   ** @throws UnsupportedOperationException
   */
  final static <E> boolean verifyBackingMapIntegrity(SortedMap<E, E> m) {
    /*
     * for (Map.Entry<E, E> en: m.entrySet()) { assert(en.getKey() == en.getValue()); }
     */
   ...
   ** @throws ClassCastException object cannot be compared with the objects currently in the map
   ** @throws NullPointerException o is {@code null} and this map uses natural order, or its
