 ...
 ** DOCUMENT
 **
 ** OPT LOW make a SortedArrayMap out of this, and make the BTreeMap node use that (or a
 * SortedArrayTripleMap<K, V, L, R>) instead of TreeMap
 **
 ** @author infinity0
 */
public class SortedArraySet<E> extends AbstractSet<E>
  ...
  /**
   ** Construct a new set from the given array, sorted according to the given comparator. The new set
   * will be '''backed''' by the array, so it must not be changed elsewhere in a program. Ideally,
   * all references to it should be discarded, and all access to the array should occur through this
   * wrapper class.
   **
   ** @param arr The backing array
   ** @param cmp The comparator used for sorting. A {@code null} value means {@linkplain Comparable
   *        natural ordering} is used.
   ** @param sorted Whether the array is already sorted. If this is {@code
  **        true}, the input array will not be sorted again.
   ** @throws IllegalArgumentException if the array contains duplicate elements, or if {@code sorted}
   *         is {@code true} but the array is not actually sorted.
   */
