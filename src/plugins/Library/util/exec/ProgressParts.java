...
public class ProgressParts {
    ...
    final public float getEstimatedFractionStarted() {
        return (totalest == 0)
               ? 0.0f
               : (totalest == TOTAL_FINALIZED)
                 ? getKnownFractionStarted() : (float) started / totalest;
    }
    ...
    final public float getEstimatedFractionKnown() {
        return (totalest == 0)
               ? 0.0f : (totalest == TOTAL_FINALIZED) ? 1.0f : (float) known / totalest;
    }
    ...
    final public String toPercentageString() {
        ...
        if (finalizedTotal()) {
            ...
            f.format("%.2f (%.2f??)", getKnownFractionDone() * 100,
                     getEstimatedFractionDone() * 100);
        }
    ...
    public static ProgressParts getSubParts(Iterable<? extends Progress> subprogress,
        ...
        if (num == unknown) {
            ...
            t = (try_to_be_smart)
                ? (int) Math.round(t * num / (float) (num - unknown)) : ESTIMATE_UNKNOWN;
        }
