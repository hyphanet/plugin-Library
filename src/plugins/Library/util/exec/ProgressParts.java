...
public class ProgressParts {
   ...
   **
   ** ;{@code -1} : The final total is known and is equal to {@link #known}. ;{@code -2} : No
  ...
  /**
   ** Constructs a new {@code ProgressPart} from the given arguments, fixing any values which are
   * greater than their next neighbour (For example, 5/4/3/2 -> 2/2/2/2, and 3/7/4/? -> 3/4/4/?).
   **
   ...
   ** @param k Parts {@linkplain #known}
   ** @param t {@linkplain #totalest Estimated} total parts
   ** @throws IllegalArgumentException if the constraints for the fields are not met.
   */
  public ProgressParts(int d, int s, int k, int t) {
    if (0 > d || d > s || s > k || (t >= 0 && k > t)
        || (t < 0 && t != ESTIMATE_UNKNOWN && t != TOTAL_FINALIZED)) {
      throw new IllegalArgumentException("ProgressParts (" + d + "/" + s + "/" + k + "/" + t
  ...
  /**
   ** Whether the task is done, ie. whether {@code done == staretd == known} and the total is
   * {@linkplain #totalest finalized}.
   */
  ...
  /**
   ** Returns the parts done as a fraction of the estimated total parts. Note: this will return
   * negative (ie. invalid) if there is no estimate.
   */
  final public float getEstimatedFractionDone() {
    return totalest == 0 ? 0.0f
        : totalest == TOTAL_FINALIZED ? getKnownFractionDone() : (float) done / totalest;
  }

  /**
   ** Returns the parts started as a fraction of the estimated total parts. Note: this will return
   * negative (ie. invalid) if there is no estimate.
   */
  final public float getEstimatedFractionStarted() {
    return totalest == 0 ? 0.0f
        : totalest == TOTAL_FINALIZED ? getKnownFractionStarted() : (float) started / totalest;
  }

  /**
   ** Returns the parts known as a fraction of the estimated total parts. Note: this will return
   * negative (ie. invalid) if there is no estimate.
   */
   ...
   **
   ** * If {@code done == started}, only one will be included. * If the total is
   * {@linkplain #finalizedTotal() finalized}, the estimate will be ommited. * If there is no
   * estimate, only the {@code ??} will be included.
   */
  @Override
  ...
  ** (estimated_fraction_done)}.
   **
   ** * If the total is {@linkplain #finalizedTotal() finalized}, the estimate will be ommited. * If
   * there is no estimate, only the {@code ??} will be included.
   */
  ...
  ** (estimated_percent_done)}.
  ...
  /**
   ** Constructs a new {@code ProgressParts} based on the total number of parts (done, started,
   * known) in the given iterable. Total estimated parts is calculated based on the value of
   * {@code try_to_be_smart}:
   **
   ** ;{@code true} : try to extrapolate an estimated total parts based on the number of
   * subprogresses having estimates, the sum of these estimates, and the total number of
   * subprogresses.
   **
   ** ;{@code false} : an estimate will only be supplied if all subprogresses have estimates.
   */
  public static ProgressParts getSubParts(Iterable<? extends Progress> subprogress,
      boolean try_to_be_smart) throws TaskAbortException {
    int d = 0, s = 0, k = 0, t = 0;
    int num = 0, unknown = 0;
    ...
    if (num == unknown) {
      ...
      t = (try_to_be_smart) ? (int) Math.round(t * num / (float) (num - unknown))
          : ESTIMATE_UNKNOWN;
    }
  ...
  /**
   ** Constructs a new {@code ProgressParts} based on the number of progresses (done, started, known)
   * in the given iterable, with the given total estimate.
   */
  public static ProgressParts getParts(Iterable<? extends Progress> subprogress, int estimate)
