...
public class TaskAbortException extends Exception {
  ...
  ** true}. A {@code false} value generally means that the task was itself aborted, but that the
   * intended effects of the task can be treated as having been accomplished, eg.
   * {@link TaskCompleteException}.
   **
   ** Multiple-task serialisers should handle abortions as follows: non-error abortions result in the
   * task being removed from the taskgroup; error abortions result are rethrown as-is.
   */
  final protected boolean error;

  /**
   ** Whether the failure is temporary, and the client should try again at a later time. Eg. this
   * should be {@code true} for abortions caused by {@link java.io.IOException}, and {@code false}
   * for abortions caused by {@link plugins.Library.io.DataFormatException}. Defaults to {@code
  ** false}.
   */
  ...
  /**
   ** Constructs a new exception with the specified paramaters, marked as error.
   **
   ** @param s The detail message
   ** @param t The cause
   ** @param r Whether a {@link #retry} is likely to succeed.
  ...
  /**
   ** Constructs a new exception with the specified paramaters, marked as error and non-retry.
   **
   ** @param s The detail message
   ** @param t The cause
   */
