...
public class BaseCompositeProgress implements CompositeProgress {
  ...
  /**
   ** Sets the iterable to accumulate progress information from. There are several static helper
   * methods you can use to create progress iterables that are backed by a collection of tracker
   * ids; see the rest of the documentation for this class.
   **
  ...
  /* @Override **/ public String getStatus() {
    try {
      ...
      assert (e.isError()); // in-progress / complete tasks should have been rm'd from the
                            // underlying iterable
  ...
  /* @Override **/ public void join() throws InterruptedException, TaskAbortException {
    ...
    for (;;) {
      ...
      if (s == 1 || Math.random() < 1 / Math.log(s) / 4) {
        ++s;
      } else {
        s = 1;
      }
  ...
  /*
   * // moved here from ParallelSerialiser. might be useful. protected void joinAll(Iterable<P>
   * plist) throws InterruptedException, TaskAbortException { Iterator<P> it = plist.iterator();
   * while (it.hasNext()) { P p = it.next(); try { p.join(); } catch (TaskAbortException e) { if
   * (e.isError()) { throw e; } else { // TODO LOW perhaps have a handleNonErrorAbort() that can be
   * overridden it.remove(); } } } }
   */
