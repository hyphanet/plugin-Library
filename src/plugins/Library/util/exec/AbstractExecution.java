...
/**
 ** A partial implementation of {@link Execution}, defining some higher-level functionality in terms
 * of lower-level ones.
 **
 ** To implement {@link Execution} fully from this class, the programmer needs to implement the
 * following methods:
 **
 ** * {@link Progress#getParts()}
 **
 ** The programmer must call {@link #setResult(Object)} and {@link #setError(TaskAbortException)} to
 * set the error and result fields correctly. If these methods are overridden, they '''must''' call
 * the overriden ones using {@code super}.
 **
...
public abstract class AbstractExecution<V> implements Execution<V> {
  ...
  /**
   ** Holds the error that caused the operation to abort, if any. If not {@code null}, then thrown by
   * {@link #getResult()}.
   */
  private TaskAbortException error;

  /**
   ** Holds the result of the operation once it completes. If {@link #error} is {@code null}, then
  ...
  protected synchronized void setStartDate() {
    ...
    start = new Date();
    for (ExecutionAcceptor<V> acc : accept) {
      offerStarted(acc);
    }
  }
