...
/**
 ** A class that wraps around an {@link Executor}, for processing any given type of object, not just
 * {@link Runnable}. Each object must be accompanied by a secondary "deposit" object, which is
 * returned with the object when it has been processed. Any exceptions thrown are also returned.
 **
...
public class ObjectProcessor<T, E, X extends Exception> implements Scheduler {
  ...
  // Allows a group of ObjectProcessor's to all notify the same object so that the caller doesn't
  // have to poll.
  ...
  /**
   ** Constructs a new processor. The processor itself will be thread-safe as long as the queues and
   * deposit map are not exposed to other threads, and the closure's invoke method is also
   * thread-safe.
   **
   ...
   */
  public ObjectProcessor(BlockingQueue<T> input, BlockingQueue<X2<T, X>> output, Map<T, E> deposit,
  ...
  /**
   ** Safely submits the given item and deposit to the given processer. Only use this when the input
   * queue's {@link BlockingQueue#put(Object)} method does not throw {@link InterruptedException},
   * such as that of {@link java.util.concurrent.PriorityBlockingQueue}.
   */
  public static <T, E, X extends Exception> void submitSafe(ObjectProcessor<T, E, X> proc, T item,
      E deposit) {
    try {
   ...
   **
   ** @throws IllegalStateException if the processor has already been {@link #close() closed}
  ...
  public void submit(T item, E deposit) throws InterruptedException {
    synchronized (this) {
      ...
      if (dep.containsKey(item)) {
        throw new IllegalArgumentException(
            "ObjectProcessor: object " + item + " already submitted");
      }

      dep.put(item, deposit);
    }
    // in.put() can block. Don't hold the outer lock during pushing.
    // Note that this can result in more stuff being in dep than is in in. This is okay, assuming
    // that
  ...
  /**
   ** Retrieved a processed item, along with its deposit and any exception that caused processing to
   * abort.
   */
  ...
  /**
   ** Whether there are any unprocessed items (including completed tasks not yet retrieved by the
   * submitter).
   */
  ...
  /**
   ** Retrieves an item by calling {@link BlockingQueue#take()} on the input queue. If this succeeds,
   * a job is {@linkplain #createJobFor(Object) created} for it, and sent to {@link #exec} to be
   * executed.
   **
   ** This method is provided for completeness, in case anyone needs it; {@link #auto()} should be
  ...
  public synchronized void dispatchTake() throws InterruptedException {
    ...
    /*
     * TODO NORM first needs a way of obeying maxconc T item = in.take();
     * exec.execute(createJobFor(item)); ++dispatched;
     */
  }

  /**
   ** Retrieves an item by calling {@link BlockingQueue#poll()} on the input queue. If this succeeds,
   * a job is {@linkplain #createJobFor(Object) created} for it, and sent to {@link #exec} to be
   * executed.
   **
   ** This method is provided for completeness, in case anyone needs it; {@link #auto()} should be
  ...
  /**
   ** Creates a {@link Runnable} to process the item and push it onto the output queue, along with
   * any exception that aborted the process.
   **
   ** The default implementation invokes {@link #clo} on the item, and then adds the appropriate data
  ...
  /**
   ** Start a new thread to run the {@link #pending} processors, if one is not already running.
   */
    ...
    auto = new Thread() {
      ...
      public void run() {
        ...
        while (!pending.isEmpty() && (t = timeout) == timeout || t-- > 0) {
          for (Iterator<WeakReference<ObjectProcessor>> it = pending.keySet().iterator(); it
              .hasNext();) {
          ...
          try {
            ...
            // FIXME the executors are perfectly capable of limiting the number of threads
            // running...
            Thread.sleep(100);
          } catch (InterruptedException e) {
  ...
  /**
   ** Add this processor to the collection of {@link #pending} processes, and makes sure there is a
  ...
  /**
   ** Stop accepting new submissions or deposit updates. Held items can still be processed and
   * retrieved, and if an {@linkplain #auto() auto-handler} is running, it will run until all such
