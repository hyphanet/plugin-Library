 ...
 **
 ** TODO NORM "issub" should not be necessary, but need to check bounds on submaps before trying to
 * remove an item.
 **
 ** @author infinity0
 */
public class SkeletonTreeMap<K, V> extends TreeMap<K, V>
  ...
  /**
   ** The backing map. This map is only ever modified structurally - ie. the {@link SkeletonValue}
   * for a given key is never overwritten; only its contents are. This ensures correct behaviour for
   * the {@link UnwrappingIterator} class.
   */
  ...
  public Object putGhost(K key, Object o) {
    ...
    if (o == null) {
      throw new IllegalArgumentException(
          "Cannot put a null dummy into the map. Use put(K, V) to mark an object as loaded.");
    }
  ...
  public static <K, V> void swapKey(K key, SkeletonTreeMap<K, V> src, SkeletonTreeMap<K, V> dst) {
    if (dst.containsKey(key)) {
      throw new IllegalArgumentException(
          "SkeletonTreeMap.swapKey: key " + key + " already exists in target map");
    }
    if (!src.containsKey(key)) {
      throw new IllegalArgumentException(
          "SkeletonTreeMap.swapKey: key " + key + " does not exist in source map");
    }
  ...
  /* @Override **/ public void deflate() throws TaskAbortException {
    ...
    for (Map.Entry<K, SkeletonValue<V>> en : skmap.entrySet()) {
      ...
      if (skel.data() == null) {
        if (skel.isLoaded())
          // FIXME is a null loaded value legal? Should it be? Disallowing nulls is not an
          // unreasonable policy IMHO...
          // Currently nulls are used in SkeletonBTreeMap.update() as placeholders, but we do not
          // want them to be serialised...
          throw new TaskAbortException("Skeleton value in " + this
              + " : isLoaded() = true, data = null, meta = " + skel.meta() + " for " + en.getKey(),
              new NullPointerException());
        else if (skel.meta() == null)
          throw new TaskAbortException("Skeleton value in " + this
              + " : isLoaded() = false, data = null, meta = null for " + en.getKey(),
              new NullPointerException());
        else {
  ...
  /* @Override **/ public void inflate(K key) throws TaskAbortException {
    ...
    try {
      ...
      for (Map.Entry<K, PullTask<V>> en : tasks.entrySet()) {
        ...
        if (t.data == null)
          throw new DataFormatException("Inflate got null from PullTask for " + key + " on " + this,
              null, null, tasks, en.getKey());
        SkeletonValue<V> sk = skmap.get(en.getKey());
        if (sk == null) {
          throw new DataFormatException(
              "SkeletonTreeMap got unexpected extra data from the serialiser.", null, sk, tasks,
              en.getKey());
        }
   ...
   **
   ** This implementation provides static methods to translate between this class and a map from
   * ({@link String} forms of the key) to ({@link Task#meta metadata} of the values).
   **
  ...
  abstract public static class TreeMapTranslator<K, V>
    ...
    /**
     ** Forward translation. If the translator is given is {@code null}, it will use
     ...
     */
    public static <K, V> Map<String, Object> app(SkeletonTreeMap<K, V> map,
        Map<String, Object> intm, Translator<K, String> ktr) {
      if (!map.isBare()) {
        throw new IllegalArgumentException(
    ...
    /**
     ** Backward translation. If no translator is given, a direct cast will be attempted.
     **
     ...
     */
    public static <K, V> SkeletonTreeMap<K, V> rev(Map<String, Object> intm,
        SkeletonTreeMap<K, V> map, Translator<K, String> ktr) throws DataFormatException {
      if (ktr == null) {
        try {
  ...
  public Comparator<? super K> comparator() {
    return skmap.comparator();
  }

  @Override
  public boolean containsKey(Object key) {
    return skmap.containsKey(key);
  }
  ...
  public V get(Object key) {
    ...
    if (!sk.isLoaded()) {
      throw new DataNotLoadedException("Data not loaded for key " + key + ": " + sk.meta(), this,
          key, sk.meta());
    }
    return sk.data();
  }
   ...
   ** {@inheritDoc}
   **
   ** NOTE: if the value for the key hasn't been loaded yet, then this method will return **null**
   * instead of returning the actual previous value (that hasn't been loaded yet).
   **
   ** TODO LOW could code a setStrictChecksMode() or something to have this method throw
   * {@link DataNotLoadedException} in such circumstances, at the user's discretion. This applies
   * for CombinedEntry.setValue() too.
   */
  @Override
   ...
   **
   ** TODO LOW could code a setStrictChecksMode() or something to have this method throw
   * {@link DataNotLoadedException} in such circumstances, at the user's discretion.
   */
  @Override
   ...
   ** {@inheritDoc}
   **
   ** NOTE: the sorted map returned by this method is limited in its functionality, and will throw
   * {@link UnsupportedOperationException} for most of its methods. For details, see
   * {@link UnwrappingSortedSubMap}.
   */
  @Override
   ...
   * 
   * @Override public int hashCode() { throw new UnsupportedOperationException("not implemented"); }
   */
  ...
  /************************************************************************
   ** {@link Iterator} that will throw {@link DataNotLoadedException} when it encounters such data.
   * Precise behaviour:
   **
   ** * The key iterator will never throw an exception * The entry iterator will never throw an
   * exception, but the {@link UnwrappingEntry#getValue()} method of the entry will throw one if
   ** data is not loaded at the time of the method call. This allows you to load ''and retrieve'' the
   * data during the middle of an iteration, by polling the {@code getValue()} method of the entry.
   ** * The value iterator will throw an exception if the value is not loaded, and keep throwing this
   * exception for subsequent method calls, pausing the iteration until the value is loaded.
   **
   ** The {@link #remove()} method will set the {@link #ghosts} field correctly as long as (and only
   * if) access to (the {@link SkeletonValue} corresponding to the item being removed) is either
   * synchronized, or contained within one thread.
   **
   ** @author infinity0
   */
  protected class UnwrappingIterator<T> implements Iterator<T> {
    ...
    /**
     ** Last entry returned by the backing iterator. Used by {@link #remove()} to update the
     * {@link #ghosts} counter correctly.
     */
    ...
    /**
     ** Whether this is a view of a submap {@link SkeletonTreeMap}. Currently, this determines
     * whether attempts to modify the view will fail, since it's a bitch to keep track of the ghost
     * counter.
     */
     ...
     ** Type of iterator.
     **
     ** ;{@link #KEY} : Key iterator ;{@link #VALUE} : Value iterator ;{@link #ENTRY} : Entry
     * iterator
     */
    final protected int type;

    /**
     ** Construct an iterator backed by the given iterator over the entries of the backing map.
     **
    ...
    public T next() {
      switch (type) {
        ...
        case VALUE:
          if (last == null || gotvalue) {
            last = iter.next();
          }
          SkeletonValue<V> skel = last.getValue();
          if (!(gotvalue = skel.isLoaded())) {
            throw new DataNotLoadedException(
                "SkeletonTreeMap: Data not loaded for key " + last.getKey() + ": " + skel.meta(),
                SkeletonTreeMap.this, last.getKey(), skel.meta());
          }
          return (T) skel.data();
        case ENTRY:
          last = iter.next();
    ...
    /************************************************************************
     ** {@link java.util.Map.Entry} backed by one whose value is wrapped inside a
     * {@link SkeletonValue}. This class will unwrap the value when it is required, throwing
     * {@link DataNotLoadedException} as necessary.
     **
     ** @author infinity0
     */
    protected class UnwrappingEntry implements Map.Entry<K, V> {
      ...
      protected void verifyLoaded() {
        if (!skel.isLoaded()) {
          throw new DataNotLoadedException(
              "SkeletonTreeMap: Data not loaded for key " + key + ": " + skel.meta(),
              SkeletonTreeMap.this, key, skel.meta());
        }
      }
  ...
  /************************************************************************
   ** Submap of a {@link SkeletonTreeMap}. Currently, this only implements the {@link #firstKey()},
   * {@link #lastKey()} and {@link #isEmpty()} methods. This is because it's a bitch to implement an
   * entire {@link SortedMap}, and I only needed those three methods for {@link SkeletonBTreeMap} to
  ...
  protected class UnwrappingSortedSubMap extends AbstractMap<K, V> implements SortedMap<K, V> {
     ...
     ** {@inheritDoc}
     **
     ** NOTE: the sorted map returned by this method has the {@link UnwrappingSortedSubMap same
     * limitations} as this class.
     */
  ...
  protected class UnwrappingEntrySet extends AbstractSet<Map.Entry<K, V>> {
    ...
    public Iterator<Map.Entry<K, V>> iterator() {
      return new UnwrappingIterator<Map.Entry<K, V>>(bkmap.entrySet().iterator(),
          UnwrappingIterator.ENTRY, issub);
    }
    ...
    public boolean contains(Object o) {
      ...
      if (!sk.isLoaded()) {
        throw new DataNotLoadedException("Data not loaded for key " + key + ": " + sk.meta(),
            SkeletonTreeMap.this, key, sk.meta());
      }
...
/************************************************************************
 ** A class that wraps the associated value (which may not be loaded) of a key in a
 * {@link SkeletonTreeMap}, along with metadata and its loaded status.
 **
 ** meta and data are private with accessors to ensure proper encapsulation i.e. the only way data
 * can be != null is if isLoaded(), and the only way meta can be != null is if !isLoaded(). Note
 * that with the methods and the class final, there should be no performance penalty.
 **
 ** @author infinity0
 */
