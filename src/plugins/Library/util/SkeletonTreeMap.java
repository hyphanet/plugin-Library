...
public class SkeletonTreeMap<K, V> extends TreeMap<K, V>
    ...
    public static <K, V> void swapKey(K key, SkeletonTreeMap<K, V> src, SkeletonTreeMap<K, V> dst) {
        if (dst.containsKey(key)) {
            throw new IllegalArgumentException("SkeletonTreeMap.swapKey: key " + key +
                    " already exists in target map");
        }

        if ( !src.containsKey(key)) {
            throw new IllegalArgumentException("SkeletonTreeMap.swapKey: key " + key +
                    " does not exist in source map");
        }
        ...
        for (Map.Entry<K, SkeletonValue<V>> en : skmap.entrySet()) {
            ...
            if (skel.data() == null) {
                if (skel.isLoaded()) {
                    ...
                    // FIXME is a null loaded value legal? Should it be? Disallowing nulls is not
                    // an unreasonable policy IMHO...
                    // Currently nulls are used in SkeletonBTreeMap.update() as placeholders, but
                    // we do not want them to be serialised...
                    throw new TaskAbortException("Skeleton value in " + this +
                                                 " : isLoaded() = true, data = null, meta = " +
                                                 skel.meta() + " for " +
                                                 en.getKey(), new NullPointerException());
                } else if (skel.meta() == null) {
                    throw new TaskAbortException(
                        "Skeleton value in " + this +
                        " : isLoaded() = false, data = null, meta = null for " +
                        en.getKey(), new NullPointerException());
                } else {
    ...
    public void inflate(K key) throws TaskAbortException {
        ...
        try {
            ...
            for (Map.Entry<K, PullTask<V>> en : tasks.entrySet()) {
                ...
                if (t.data == null) {
                    throw new DataFormatException("Inflate got null from PullTask for " + key +
                                                  " on " + this, null, null, tasks, en.getKey());
                }
                ...
                if (sk == null) {
                    throw new DataFormatException(
                        "SkeletonTreeMap got unexpected extra data from the serialiser.", null, sk,
                        tasks, en.getKey());
                }
    ...
    abstract public static class TreeMapTranslator<K, V>
        ...
        public static <K, V> SkeletonTreeMap<K, V> rev(Map<String, Object> intm,
                SkeletonTreeMap<K, V> map, Translator<K, String> ktr)
        ...
        if (keys == null) {
            ...
            keys = new AbstractSet<K>() {
                ...
                public Iterator<K> iterator() {
                    return new UnwrappingIterator<K>(skmap.entrySet().iterator(),
                                                  UnwrappingIterator.KEY, false);
                }
                @Override
                public void clear() {
        ...
        if (values == null) {
            values = new AbstractCollection<V>() {
                ...
                public Iterator<V> iterator() {
                    return new UnwrappingIterator<V>(skmap.entrySet().iterator(),
                                                  UnwrappingIterator.VALUE, false);
                }
                @Override
                public void clear() {
                    SkeletonTreeMap.this.clear();
    ...
    */
    ...
    protected class UnwrappingIterator<T> implements Iterator<T> {
                ...
                case VALUE :
                    ...
                    if ( !(gotvalue = skel.isLoaded())) {
                        throw new DataNotLoadedException(
                            "SkeletonTreeMap: Data not loaded for key " + last.getKey() + ": " +
                            skel.meta(), SkeletonTreeMap.this, last.getKey(), skel.meta());
                    }
        ...
        protected class UnwrappingEntry implements Map.Entry<K, V> {
            ...
            protected void verifyLoaded() {
                if ( !skel.isLoaded()) {
                    throw new DataNotLoadedException("SkeletonTreeMap: Data not loaded for key " +
                            key + ": " + skel.meta(), SkeletonTreeMap.this, key, skel.meta());
                }
            }
            ...
            public boolean equals(Object o) {
                ...
                return ((key == null) ? en.getKey() == null : key.equals(en.getKey())) &&
                       ((skel.data() == null)
                        ? en.getValue() == null : skel.data().equals(en.getValue()));
            }
        }
    }
    ...
    protected class UnwrappingEntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            ...
            if ( !sk.isLoaded()) {
                throw new DataNotLoadedException("Data not loaded for key " + key + ": " +
                        sk.meta(), SkeletonTreeMap.this, key, sk.meta());
            }
