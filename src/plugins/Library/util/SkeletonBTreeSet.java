 ...
 **
 ** TODO LOW make this implement Skeleton<Pair<Integer, Integer>, ?> ? - ie. deflate(int, int) to get
 * the relevant subset?
 **
 ** @author infinity0
 */
public class SkeletonBTreeSet<E>
    extends BTreeSet<E> /* implements Skeleton<E, MapSerialiser<E, E>> */ {

  // TODO NORM when we implement internal_entries in SkeletonBTreeMap, then
  // we can have it automatically set to TRUE here.

  public void setSerialiser(IterableSerialiser<SkeletonBTreeMap<E, E>.SkeletonNode> n,
      MapSerialiser<E, E> v) {
  ...
  // TODO NORM actually implement this...
  // ========================================================================
  ...
  public void update(SortedSet<E> putset, SortedSet<E> remset) throws TaskAbortException {
    ((SkeletonBTreeMap<E, E>) bkmap).update(null, remset, new SortedSetMap<E, SortedSet<E>>(putset),
        null, new TaskAbortExceptionConvertor());
  }

  /**
   ** Creates a translator for the nodes of the B-tree. This method just calls the
   * {@link SkeletonBTreeMap#makeNodeTranslator(Translator, Translator) corresponding method} for
   * the map backing this set.
   **
   ** @param ktr Translator for the keys
   ** @param mtr Translator for each node's local entries map
   */
  public <Q, R> SkeletonBTreeMap<E, E>.NodeTranslator<Q, R> makeNodeTranslator(Translator<E, Q> ktr,
      Translator<SkeletonTreeMap<E, E>, R> mtr) {
    return ((SkeletonBTreeMap<E, E>) bkmap).makeNodeTranslator(ktr, mtr);
  }


  /************************************************************************
   ** {@link Translator} between a {@link Collection} and a {@link Map} of keys to themselves. Used
   * by {@link TreeSetTranslator}.
   **
  ...
  abstract public static class MapCollectionTranslator<E, M extends SkeletonTreeMap<E, E>, C extends Collection<E>>
    ...
    public static <E, M extends SkeletonTreeMap<E, E>, C extends Collection<E>> C app(M src,
        C dst) {
      for (E k : src.keySet()) {
        dst.add(k);
      }
    ...
    public static <E, M extends SkeletonTreeMap<E, E>, C extends Collection<E>> M rev(C src,
        M dst) {
      for (E e : src) {
        dst.putGhost(e, e);
      }
  ...
  /************************************************************************
   ** Translator for the {@link java.util.TreeMap} backing a {@link java.util.TreeSet}. This will
   * turn a map into a list and vice versa, so that the serialised representation doesn't store
   * duplicates.
   **
  ...
  public static class TreeTranslator<E, T>
    ...
    public TreeTranslator(Translator<E, T> k,
        Translator<SkeletonTreeMap<E, E>, ? extends Collection<T>> m) {
