...
/**
 ** Trie-like map or map-like structure (eg. multimap) with each node mapping keys that match a given
 * prefix. Each node in the tree will admit a certain maximum number of mappings before it starts
 * creating subtrees.
 **
 ** @deprecated
 ** @author infinity0
...
abstract public class PrefixTree<K extends PrefixTree.PrefixKey, V> {
  ...
  /**
   ** Array holding the child trees. There is one cell in the array for each symbol in the alphabet
   * of the key. For all i: {@link #child}[i] != null, or {@link #child}[i].{@link #lastIndex()} ==
   * i.
   */
  ...
  /**
   ** Maximum size of ({@link #sizeLocal()} + {@link #child}) before we start to create subtrees.
   */
  final public int capacityLocal;

  /**
   ** Number of subtrees. At all times, this should equal the number of non-null members of
   * {@link #child}.
   */
  protected int subtrees = 0;

  /**
   ** Number of elements contained in the tree. At all times, it should equal the sum of the size of
   * the subtrees, plus the {@link #sizeLocal()}.
   */
  protected int size = 0;

  /**
   ** Counts the number of mappings in each prefix group, meaning the set of all mappings whose keys
   * give the same key.get(preflen). At all times, the sum the array elements should equal the size
   * field. Additionally, for all i: ({@link #child}[i] != null) implies ({@link #sizePrefix}[i] ==
   * {@link #child}[i].size()); and {@link #sizeLocal()} == sum{ {@link #sizePrefix}[j] :
   * {@link #child}[j] == null }
   */
  protected int sizePrefix[];

  /**
   ** Cache for {@link #smallestChild()}. At all times, this should either be -1, or be the index of
   * the subtree with the smallest size. If -1, then either there are no subtrees (check
   * {@link #subtrees} == 0) or the cache has been invalidated.
   */
  ...
  /**
   ** Build a subtree from the largest subset of the local map consisting of (keys that share a
   * common prefix of length one greater than that of this tree).
   **
   ** For efficiency, this method *assumes* that such a set exists and is non-empty; it is up to the
   * calling code to make sure this is true.
   **
   ** TODO URGENT consider, in the case of Multimap, when the size of local map goes above
   * capacityLocal and cannot be converted into a subtree (eg. when there are capacityLocal+1 values
   * for the same key).
   **
  ...
  /**
   ** Put all entries of a subtree into this node, if there is enough space. (If there is enough
   * space, then this implies that the subtree itself has no child trees, due to the size
  ...
  /**
   ** After a put operation, move some entries into new subtrees, with big subtrees taking priority,
   * until there is enough space to fit all the remaining entries. That is, ensure that there exists
   * sz such that: - {@link #sizePrefix}[{@link #smallestChild()}] == sz - sz > {@link #sizeLeft()}
   ** - and that for all i: - ({@link #child}[i] == null) implies {@link #sizePrefix}[i] <= sz -
  ...
  /**
   ** Reshuffle after a put operation of many elements, keeping the same constraints. The elements
   * must belong to the same subgroup.
   **
  ...
  /**
   ** After a remove operation, merge some subtrees into this node, with small subtrees taking
   * priority, until there is no more space to fit any more subtrees. That is, ensure that there
   * exists sz such that: - {@link #sizePrefix}[{@link #smallestChild()}] == sz - sz >
   * {@link #sizeLeft()} - and that for all i: - ({@link #child}[i] == null) implies
   * {@link #sizePrefix}[i] <= sz - ({@link #child}[i] != null) implies {@link #sizePrefix}[i] >= sz
   **
  ...
  /**
   ** Reshuffle after a remove operation of many elements, keeping the same constraints. The elements
   * must belong to the same subgroup.
   **
   ** @param i Subgroup of elements
   ** @param n Number of elements
  ...
  /**
   ** Make a subtree with the appropriate prefix. For effeciency, this method assumes that
   * {@link #child}[i] does not already have a tree attached; it is up to the calling code to ensure
   * that this holds.
   **
  ...
  /**
   ** Transfer the value for a key to the appropriate subtree. For efficiency, this method assumes
   * that {@link #child}[i] already exists and that its prefix matches the key; it is up to the
   * calling code to ensure that this holds.
   */
  abstract protected void transferLocalToSubtree(int i, K key);

  /**
   ** Transfer all mappings of a subtree to the local map. For efficiency, the subtree is assumed to
   * be an actual direct subtree of this map, ie. the same object as {@link #child}[i] for some i;
   * it is up to the calling code to ensure that this holds.
   */
  abstract protected void transferSubtreeToLocal(PrefixTree<K, V> ch);

  /**
   ** Select the object which handles keys with i as the next prefix element. Should return
   * {@link #child}[i] if and only if {@link #child}[i] != null and a pointer to the local map
   * otherwise.
   */
  ...
  public boolean equals(Object o) {
    ...
    if (!prefix.equals(tr.prefix) || preflen != tr.preflen || capacityLocal != tr.capacityLocal
  ...
  /**
   ** Defines an interface that provides prefix-related operations, such as returning the next
   * component of a key,
   **
   ** @author infinity0
   */
  public interface PrefixKey<K extends PrefixKey<K>> extends Cloneable, Comparable<K> {
    ...
    /**
     ** Returns the number of possible symbols at each cell of the key. This should return the same
     * value for any instance.
     */
    public int symbols();

    /**
     ** Returns the size of the key. This should return the same value for any instance.
     */
    public int size();
  ...
  /**
   ** Provides implementations of various higher-level functionalities of PrefixKey in terms of
   * lower-level ones.
   **
   ** @author infinity0
   */
