 ...
 **
 ** TODO HIGH get rid of uses of rnode.get(K). All other uses of rnode, as well as lnode and entries,
 * have already been removed. This will allow us to re-implement the Node class.
 **
 ** To the maintainer: this class is very very unstructured and a lot of the functionality should be
 * split off elsewhere. Feel free to move stuff around.
 **
 ** @author infinity0
 */
public class SkeletonBTreeMap<K, V> extends BTreeMap<K, V> implements SkeletonMap<K, V> {
  ...
  /*
   ** Whether entries are "internal to" or "contained within" nodes, ie. are the entries for a node
   * completely stored (including values) with that node in the serialised representation, or do
   * they refer to other serialised data that is external to the node?
   **
   ** This determines whether a {@link TreeMap} or a {@link SkeletonTreeMap} is used to back the
   * entries in a node.
   **
   ** Eg. a {@code BTreeMap<String, BTreeSet<TermEntry>>} would have this {@code true} for the map,
   * and {@code false} for the map backing the set.
   */
  // final protected boolean internal_entries;
  /*
   ** OPT HIGH disable for now, since I can't think of a good way to implement this tidily.
   **
   ** three options:
   **
   ** 0 have SkeletonNode use TreeMap when int_ent is true rather than SkeletonTreeMap but this will
   * either break the Skeleton contract of deflate(), which expects isBare() to be true afterwards,
   * or it will break the contract of isBare(), if we modifiy that method to return true for
   * TreeMaps instead.
   **
   ** - pros: uses an existing class, efficient - cons: breaks contracts (no foreseeable way to
   * avoid), complicated to implement
   **
   ** 1 have another class that extends SkeletonTreeMap which has one single boolean value
   * isDeflated, alias *flate(K) to *flate(), and all those functions do is set that boolean. then
   * override get() etc to throw DNLEx depending on the value of that boolean; and have SkeletonNode
   ** use this class when int_ent is true.
   **
   ** - pros: simple, efficient OPT HIGH - cons: requires YetAnotherClass
   **
   ** 2 don't have the internal_entries, and just use a dummy serialiser that copies task.data to
   * task.meta for push tasks, and vice versa for pull tasks.
   **
   ** - pros: simple to implement - cons: a hack, inefficient
   **
  ...
  public void setSerialiser(IterableSerialiser<SkeletonNode> n, MapSerialiser<K, V> v) {
    if ((nsrl != null || vsrl != null) && !isLive()) {
      throw new IllegalStateException(
          "Cannot change the serialiser when the structure is not live.");
    }
    nsrl = n;
    vsrl = v;
  ...
  final public Comparator<PullTask<SkeletonNode>> CMP_PULL =
  ...
  final public Comparator<PushTask<SkeletonNode>> CMP_PUSH =
      new Comparator<PushTask<SkeletonNode>>() {
  ...
  public class SkeletonNode extends Node implements Skeleton<K, IterableSerialiser<SkeletonNode>> {
    ...
    protected SkeletonNode(K lk, K rk, boolean lf, SkeletonTreeMap<K, V> map,
        Collection<GhostNode> gh) {
      this(lk, rk, lf, map);
      _size = map.size();
      if (!lf) {
        if (map.size() + 1 != gh.size()) {
          throw new IllegalArgumentException("SkeletonNode: in constructing " + getName()
              + ", got size mismatch: map:" + map.size() + "; gh:" + gh.size());
        }
    ...
    /**
     ** Set the value-serialiser for this node and all subnodes to match the one assigned for the
     * entire tree.
     */
    ...
    /* @Override **/ public IterableSerialiser<SkeletonNode> getSerialiser() {
      return nsrl;
    }

    /* @Override **/ public boolean isLive() {
      if (ghosts > 0 || !((SkeletonTreeMap<K, V>) entries).isLive()) {
        return false;
      }
      if (!isLeaf()) {
        for (Node n : iterNodes()) {
     ...
     **
     ** It is '''assumed''' that there is already a {@link SkeletonNode} in its place; the ghost will
     * replace it. It is up to the caller to ensure that this holds.
     **
     ...
     **
     ** It is '''assumed''' that there is already a {@link GhostNode} in its place; the skeleton will
     * replace it. It is up to the caller to ensure that this holds.
     **
    ...
    /* @Override **/ public void deflate() throws TaskAbortException {
      if (!isLeaf()) {
        List<PushTask<SkeletonNode>> tasks =
            new ArrayList<PushTask<SkeletonNode>>(childCount() - ghosts);
  ...
  public class GhostNode extends Node {
     ...
     **
     ** Maintaining this field's value is a bitch, so I've tried to remove uses of this from the
     * code. Currently, it is only used for the parent field a {@link DataFormatException}, which
     * lets us retrieve the serialiser, progress, etc etc etc. TODO NORM somehow find another way of
     * doing that so we can get rid of it completely.
     */
    ...
    @Override
    public int nodeSize() {
      throw new DataNotLoadedException("BTreeMap Node not loaded: " + getRange(), parent, lkey,
          this);
    }
  ...
  /**
   ** Post-processes a {@link PullTask} and returns the {@link SkeletonNode} pulled.
   **
   ** The tree will be in a consistent state after the operation, if it was in a consistent state
   * before it.
   */
  protected SkeletonNode postPullTask(PullTask<SkeletonNode> task, SkeletonNode parent)
  ...
  /**
   ** Post-processes a {@link PushTask} and returns the {@link GhostNode} pushed.
   **
   ** The tree will be in a consistent state after the operation, if it was in a consistent state
   * before it.
   */
  ...
  protected void swapKey(K key, Node src, Node dst) {
    SkeletonTreeMap.swapKey(key, (SkeletonTreeMap<K, V>) src.entries,
        (SkeletonTreeMap<K, V>) dst.entries);
  }
  ...
  /**
   ** Parallel bulk-inflate. At the moment, this will inflate all the values of each nodes too.
   **
    ...
    try {
      ...
      do {
        ...
        while (proc_pull.hasCompleted()) {
          ...
          if (ex != null) {
            assert (!(ex instanceof plugins.Library.util.exec.TaskInProgressException)); // by
                                                                                         // contract
                                                                                         // of
                                                                                         // ScheduledSerialiser
   ...
   ** @see #update(SortedSet, SortedSet, SortedMap, Closure)
   */
  public <X extends Exception> void update(SortedSet<K> putkey, SortedSet<K> remkey,
      Closure<Map.Entry<K, V>, X> value_handler, ExceptionConvertor<X> conv)
      throws TaskAbortException {
  ...
  /**
   * Separate executor for value handlers. Value handlers can themselves call update() and end up
   * polling, so we need to keep them separate from the threads that do the actual work!
   */
  final public static Executor VALUE_EXECUTOR = new Executor() {
    /* @Override **/ public void execute(Runnable r) {
      synchronized (Executors.class) {
        if (value_exec == null) {
          value_exec = new ThreadPoolExecutor(0, 0x40, 60, TimeUnit.SECONDS,
  ...
  /**
   * Separate executor for deflating. We don't want update()'s to prevent actual pushes.
   */
  final public static Executor DEFLATE_EXECUTOR = new Executor() {
    /* @Override **/ public void execute(Runnable r) {
      synchronized (Executors.class) {
        if (deflate_exec == null) {
          deflate_exec = new ThreadPoolExecutor(0, 0x40, 60, TimeUnit.SECONDS,
              new LinkedBlockingQueue<Runnable>(), new ThreadPoolExecutor.CallerRunsPolicy());
        }
      }
  ...
  /**
   ** Asynchronously updates a remote B-tree. This uses two-pass merge/split algorithms (as opposed
   * to the one-pass algorithms of the standard {@link BTreeMap}) since it assumes a copy-on-write
   * backing data store, where the advantages (concurrency, etc) of one-pass algorithms disappear,
   * and the disadvantages (unneeded merges/splits) remain.
   **
   ** Unlike the (ideal design goal of the) inflate/deflate methods, this is designed to support
   * accept only one concurrent update. It is up to the caller to ensure that this holds. TODO NORM
   * enforce this somehow
   **
   ** Currently, this method assumes that the root.isBare(). TODO NORM enforce this..
   **
   ** This is a wrapper method to deal with the occasional key rejection due to trying to add too
   * many keys to a single node. It also checks parameters.
   **
   ** Note: {@code remkey} is not implemented yet.
   **
   ** @throws UnsupportedOperationException if {@code remkey} is not empty
   */
  protected <X extends Exception> void update(SortedSet<K> putkey, SortedSet<K> remkey,
      final SortedMap<K, V> putmap, Closure<Map.Entry<K, V>, X> value_handler,
      ExceptionConvertor<X> conv) throws TaskAbortException {
    ...
    if (remkey != null && !remkey.isEmpty()) {
      throw new UnsupportedOperationException(
          "SkeletonBTreeMap: update() currently only supports merge operations");
    }
    ...
    while (true) {
      ...
      if (rejected == null || rejected.isEmpty())
        return;
      System.err
          .println("Rejected keys: " + rejected.size() + " - re-running merge with rejected keys.");
      putkey = rejected;
    }
  }

  protected <X extends Exception> void update(SortedSet<K> putkey, final SortedMap<K, V> putmap,
      Closure<Map.Entry<K, V>, X> value_handler, ExceptionConvertor<X> conv,
      final SortedSet<K> rejected) throws TaskAbortException {

    // Avoid polling.
    final Notifier notifier = new Notifier();

    /*
     ** The code below might seem confusing at first, because the action of the algorithm on a single
     * node is split up into several asynchronous parts, which are not visually adjacent. Here is a
     * more contiguous description of what happens to a single node between being inflated and then
     * eventually deflated.
     **
     ** Life cycle of a node:
     **
     ** - node gets popped from proc_pull - enter InflateChildNodes - subnodes get pushed into
     * proc_pull - (recurse for each subnode) - subnode gets popped from proc_pull - etc -
     * split-subnodes get pushed into proc_push - wait for all: split-subnodes get popped from
     * proc_push - enter SplitNode - for each item in the original node's DeflateNode - release the
     * item and acquire it on - the parent's DeflateNode if the item is a separator - a new
     * DeflateNode if the item is now in a split-node - for each split-node: - wait for all: values
     * get popped from proc_val; SplitNode to close() - enter DeflateNode - split-node gets pushed
     * into proc_push (except for root)
     **
     */
     ...
     */
    class DeflateNode extends TrackingSweeper<K, SortedSet<K>>
        implements Runnable, SafeClosure<Map.Entry<K, V>> {

      /** The node to be pushed, when run() is called. */
      final SkeletonNode node;
      /**
       * A closure for the parent node. This is called when all of its children (of which this.node
       * is one) have been pushed.
       */
      ...
      /**
       ** Push this node. This is run when this sweeper is cleared, which happens after all the
       * node's local values have been obtained, '''and''' the node has passed through SplitNode
       * (which closes this sweeper).
       */
      public void run() {
        try {
          deflate();
      ...
      /**
       ** Update the key's value in the node. Runs whenever an entry is popped from proc_val.
       */
      public void invoke(Map.Entry<K, V> en) {
        assert (node.entries.containsKey(en.getKey()));
        node.entries.put(en.getKey(), en.getValue());
        if (logMINOR)
          Logger.minor(this, "New value for key " + en.getKey() + " : " + en.getValue() + " in "
              + node + " parent = " + parNClo);
      }
    ...
    // The proc_val output queue must comfortably cover everything generated by a single invocation
    // of InflateChildNodes, or else we will deadlock.
    // Note that this is all deflated sub-trees, so they are relatively small - some of the other
    // queues
    ...
    final ObjectProcessor<Map.Entry<K, V>, DeflateNode, X> proc_val = (value_handler == null) ? null
        : new ObjectProcessor<Map.Entry<K, V>, DeflateNode, X>(
            ...
            new HashMap<Map.Entry<K, V>, DeflateNode>(), value_handler, VALUE_EXECUTOR, conv,
            notifier // These can block so pool them separately.
        ).autostart();
    ...
    class SplitNode extends CountingSweeper<SkeletonNode> implements Runnable {
      ...
      /**
       * The value-closure for this node. This keeps track of all unhandled values in this node. If
       * this node is split, and the values have not been handled, they are passed onto the
       * value-closures of the relevant split nodes (or that of the parent node, if the key turns
       * out to be a separator key).
       */
      final DeflateNode nodeVClo;
      /**
       * The closure for the parent node, also a CountingSweeper. We keep a reference to this so we
       * can add the new split nodes to it.
       */
      /* final */ SplitNode parNClo;
      /**
       * The value-closure for the parent node. The comment for nodeVClo explains why we need a
       * reference to this.
       */
      /* final */ DeflateNode parVClo;

      protected SplitNode(SkeletonNode n, SkeletonNode p, DeflateNode vc, SplitNode pnc,
          DeflateNode pvc) {
        super(true, false);
        node = n;
        parent = p;
      ...
      /**
       ** Closes the node's DeflateNode, and (if appropriate) splits the node and updates the
       * deposits for each key moved. This is run after all its children have been deflated.
       */
      public void run() {
        ...
        for (Node nn : nodes) {
          ...
          // try {
          assert (subheld.isEmpty()
              || compareL(n.lkey, subheld.first()) < 0 && compareR(subheld.last(), n.rkey) < 0);
          // } catch (AssertionError e) {
          // System.out.println(n.lkey + " " + subheld.first() + " " + subheld.last() + " " +
          // n.rkey);
          // throw e;
          // }
          for (K key : subheld) {
      ...
      /**
       ** When we move a key to another node (eg. to the parent, or to a new node resulting from the
       * split), we must deassign it from the original node's sweeper and reassign it to the sweeper
       * for the new node.
       **
       ** NOTE: if the overall co-ordinator algorithm is ever made concurrent, this section MUST be
       * made atomic
       **
       ** @param key The key
      ...
      private void reassignKeyToSweeper(K key, DeflateNode clo) {
        ...
        if (logMINOR)
          Logger.minor(this, "Reassigning key " + key + " to " + clo + " on " + this + " parent="
              + parent + " parent split node " + parNClo + " parent deflate node " + parVClo);
    ...
    class InflateChildNodes implements SafeClosure<SkeletonNode> {
      ...
      /**
       ** Merge the relevant parts of the map into the node, and inflate its children. Runs whenever
       * a node is popped from proc_pull.
       */
      public void invoke(SkeletonNode node) {
        ...
        try {
          ((SkeletonTreeMap<K, V>) node.entries).inflate();
        } catch (TaskAbortException e) {
          throw new RuntimeException(e);
        }
        ...
        if (node.isLeaf()) {
          ...
          if (proc_val == null) {
            ...
            if (putkey.size() < proc_val.outputCapacity()) {
              ...
              TreeSet<K> accepted =
                  new TreeSet<K>(Sorted.select(putkey, proc_val.outputCapacity() / 2));
              List<SortedSet<K>> notAccepted = Sorted.split(putkey, accepted, new TreeSet<K>()); // FIXME
                                                                                                 // NullSet
                                                                                                 // ???
              System.err.println("Too many keys to add to a single node: We can add "
                  + proc_val.outputCapacity() + " but are being asked to add " + putkey.size()
                  + " - adding " + accepted.size() + " and rejecting the rest.");
              for (K key : accepted) {
                handleLocalPut(node, key, vClo);
              }
          ...
          for (SortedSet<K> rng : range) {
            ...
            ObjectProcessor.submitSafe(proc_pull, task,
                (SafeClosure<SkeletonNode>) new InflateChildNodes(node, rng, nClo, vClo));
          }
        }
        ...
        if (nClo.isCleared()) {
          nClo.run();
        } // eg. if no child nodes need to be modified
      }

      /**
       ** Handle a planned local put to the node.
       **
       ** Keys added locally have their values set to null. These will be updated with the correct
       * values via UpdateValue, when the value-getter completes its operation on the key. We add
       * the keys now, **before** the value is obtained, so that SplitNode can work out how to split
      ...
      private void handleLocalPut(SkeletonNode n, K key, DeflateNode vClo) {
        ...
        vClo.acquire(key);
        if (logMINOR)
          Logger.minor(this, "handleLocalPut for key " + key + " old value " + oldval
              + " for deflate node " + vClo + " - passing to proc_val");
    ...
    try {
      ...
      do {
        ...
        // System.out.println(System.identityHashCode(this) + " " + proc_pull + " " + proc_push + "
        // " + ((proc_val == null)? "": proc_val));

        // Only sleep if we run out of jobs.
        if ((!progress) && (count++ > 10)) {
          count = 0;
          // if(ccount++ > 10) {
          System.out.println(/* System.identityHashCode(this) + " " + */proc_val + " " + proc_pull
              + " " + proc_push + " " + proc_deflate);
          // ccount = 0;
          // }
        ...
        while (proc_push.hasCompleted()) {
          X3<PushTask<SkeletonNode>, CountingSweeper<SkeletonNode>, TaskAbortException> res =
              proc_push.accept();
          ...
          if (ex != null) {
            // FIXME HIGH
            throw new UnsupportedOperationException(
                "SkeletonBTreeMap.update(): PushTask aborted; handler not implemented yet", ex);
          }
        ...
        // System.out.println(System.identityHashCode(this) + " " + proc_push + " " + ((proc_val ==
        // null)? "": proc_val+ " ") + proc_pull);
        ...
        if (proc_val != null && proc_val.hasCompleted()) {
          ...
          if (ex != null) {
            // FIXME HIGH
            throw new UnsupportedOperationException(
                "SkeletonBTreeMap.update(): value-retrieval aborted; handler not implemented yet",
                ex);
          }
        ...
        if (proc_pull.hasCompleted()) {
          X3<PullTask<SkeletonNode>, SafeClosure<SkeletonNode>, TaskAbortException> res =
              proc_pull.accept();
          PullTask<SkeletonNode> task = res._0;
          ...
          if (ex != null) {
            // FIXME HIGH
            throw new UnsupportedOperationException(
                "SkeletonBTreeMap.update(): PullTask aborted; handler not implemented yet", ex);
          }
      ...
      } while (proc_pull.hasPending() || proc_push.hasPending()
          || (proc_val != null && proc_val.hasPending()) || proc_deflate.hasPending());
  ...
  /**
   ** Creates a translator for the nodes of the B-tree. This method is necessary because
   * {@link NodeTranslator} is a non-static class.
   **
   ** For an in-depth discussion on why that class is not static, see the class description for
   * {@link BTreeMap.Node}.
   **
   ** TODO LOW maybe store these in a WeakHashSet or something... will need to code equals() and
   * hashCode() for that
   **
   ** @param ktr Translator for the keys
   ** @param mtr Translator for each node's local entries map
   */
  public <Q, R> NodeTranslator<Q, R> makeNodeTranslator(Translator<K, Q> ktr,
      Translator<SkeletonTreeMap<K, V>, R> mtr) {
   ...
   ** DOCUMENT.
   **
   ** For an in-depth discussion on why this class is not static, see the class description for
   * {@link BTreeMap.Node}.
   **
  ...
  public class NodeTranslator<Q, R> implements Translator<SkeletonNode, Map<String, Object>> {
    ...
    /* @Override **/ public Map<String, Object> app(SkeletonNode node) {
      if (!node.isBare()) {
        throw new IllegalStateException("Cannot translate non-bare node " + node.getRange()
            + "; size:" + node.nodeSize() + "; ghosts:" + node.ghosts + "; root:" + (root == node));
      }
      Map<String, Object> map = new LinkedHashMap<String, Object>();
      map.put("lkey", (ktr == null) ? node.lkey : ktr.app(node.lkey));
      map.put("rkey", (ktr == null) ? node.rkey : ktr.app(node.rkey));
      map.put("entries",
          (mtr == null) ? node.entries : mtr.app((SkeletonTreeMap<K, V>) node.entries));
    ...
    /* @Override **/ public SkeletonNode rev(Map<String, Object> map) throws DataFormatException {
      try {
        ...
        SkeletonNode node =
            new SkeletonNode((ktr == null) ? (K) map.get("lkey") : ktr.rev((Q) map.get("lkey")),
                (ktr == null) ? (K) map.get("rkey") : ktr.rev((Q) map.get("rkey")), !notleaf,
                (mtr == null) ? (SkeletonTreeMap<K, V>) map.get("entries")
                    : mtr.rev((R) map.get("entries")),
                gh);
        if (!node.isBare()) {
          throw new IllegalStateException(
              "map-translator did not produce a bare SkeletonTreeMap: " + mtr);
        }
  ...
  /************************************************************************
   ** {@link Translator} with access to the members of {@link BTreeMap}. DOCUMENT.
   **
   ** @author infinity0
   */
  public static class TreeTranslator<K, V>
    ...
    /* @Override **/ public Map<String, Object> app(SkeletonBTreeMap<K, V> tree) {
      if (tree.comparator() != null) {
        throw new UnsupportedOperationException(
            "Sorry, this translator does not (yet) support comparators");
      }
      Map<String, Object> map = new LinkedHashMap<String, Object>();
      map.put("node_min", tree.NODE_MIN);
      map.put("size", tree.size);
      Map<String, Object> rmap =
          tree.makeNodeTranslator(ktr, mtr).app((SkeletonBTreeMap.SkeletonNode) tree.root);
      ...
      try {
        ...
        if (tree.size != tree.root.totalSize()) {
          throw new DataFormatException(
              "Mismatched sizes - tree: " + tree.size + "; root: " + tree.root.totalSize(), null,
              null);
        }
        return tree;
      } catch (ClassCastException e) {
        throw new DataFormatException("Could not build SkeletonBTreeMap from data", e, map, null,
            null);
      }
    }
        ...
        public Iterator<K> iterator() {
          ...
          return new Iterator<K>() {
            ...
            /* @Override **/ public void remove() {
              ...
              // FIXME
              // if (!removeok) {
              // throw new IllegalStateException("Iteration has not yet begun, or the element has
