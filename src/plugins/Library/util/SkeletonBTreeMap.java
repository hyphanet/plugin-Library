...
public class SkeletonBTreeMap<K, V> extends BTreeMap<K, V> implements SkeletonMap<K, V> {
    ...
    final public Comparator<PullTask<SkeletonNode>> CMP_PULL =
        ...
        /* @Override* */
        public int compare(PullTask<SkeletonNode> t1, PullTask<SkeletonNode> t2) {
    ...
    final public Comparator<PushTask<SkeletonNode>> CMP_PUSH =
        ...
        /* @Override* */
        public int compare(PushTask<SkeletonNode> t1, PushTask<SkeletonNode> t2) {
        ...
        protected SkeletonNode(K lk, K rk, boolean lf, SkeletonTreeMap<K, V> map,
            ...
            if ( !lf) {
                if (map.size() + 1 != gh.size()) {
                    throw new IllegalArgumentException("SkeletonNode: in constructing " +
                            getName() + ", got size mismatch: map:" + map.size() + "; gh:" +
                            gh.size());
                }
        ...
        */
            ...
            if ( !isLeaf()) {
                ...
                for (PushTask<SkeletonNode> task : tasks) {
                    try {
                        ...
                        throw new TaskAbortException("Could not deflate BTreeMap Node " +
                                                     getRange(), e);
                    }
                }
            }
        ...
        public void deflate(K key) throws TaskAbortException {
            ...
            try {
                ...
                throw new TaskAbortException("Could not deflate BTreeMap Node " + node.getRange(),
                                             e);
            }
        }
        ...
        public void inflate(K key, boolean auto) throws TaskAbortException {
            ...
            try {
                ...
                throw new TaskAbortException("Could not inflate BTreeMap Node " + node.getRange(),
                                             e);
            } catch (RuntimeException e) {
                throw new TaskAbortException("Could not inflate BTreeMap Node " + node.getRange(),
                                             e);
    ...
    public class GhostNode extends Node {
        ...
        @Override
        public int nodeSize() {
            throw new DataNotLoadedException("BTreeMap Node not loaded: " + getRange(), parent,
                    lkey, this);
        }
    ...
    public void inflate() throws TaskAbortException {
        ...
        final Queue<SkeletonNode>                                     nodequeue =
            new PriorityQueue<SkeletonNode>();
        ...
        if (nsrl instanceof Serialiser.Trackable) {
            ids      = new LinkedHashMap<PullTask<SkeletonNode>,
                                         ProgressTracker<SkeletonNode, ?>>();
        ...
        try {
            ...
            do {
                ...
                while (proc_pull.hasCompleted()) {
                    X3<PullTask<SkeletonNode>, SkeletonNode, TaskAbortException> res =
                        proc_pull.accept();
                    ...
                    if (ex != null) {
                        assert( !(ex instanceof plugins.Library.util.exec
                            .TaskInProgressException));    // by contract of ScheduledSerialiser
    ...
    */
    ...
    protected <X extends Exception> void update(SortedSet<K> putkey, SortedSet<K> remkey,
        ...
        while (true) {
            ...
            System.err.println("Rejected keys: " + rejected.size() +
                               " - re-running merge with rejected keys.");
            putkey = rejected;
        }
    }
    ...
    protected <X extends Exception> void update(SortedSet<K> putkey, final SortedMap<K, V> putmap,
        ...
        class DeflateNode extends TrackingSweeper<K, SortedSet<K>>
            ...
            public void invoke(Map.Entry<K, V> en) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "New value for key " + en.getKey() + " : " + en.getValue() +
                                 " in " + node + " parent = " + parNClo);
                }
            }
            public void deflate() throws TaskAbortException {
                synchronized (this) {
        ...
        class SplitNode extends CountingSweeper<SkeletonNode> implements Runnable {
            ...
            public void run() {
                ...
                for (Node nn : nodes) {
                    ...
                    assert(subheld.isEmpty() ||
                           ((compareL(n.lkey, subheld.first()) < 0) &&
                            (compareR(subheld.last(), n.rkey) < 0)));
            ...
            private void reassignKeyToSweeper(K key, DeflateNode clo) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Reassigning key " + key + " to " + clo + " on " + this +
                                 " parent=" + parent + " parent split node " + parNClo +
                                 " parent deflate node " + parVClo);
                }
        ...
        class InflateChildNodes implements SafeClosure<SkeletonNode> {
            ...
            protected InflateChildNodes(SkeletonNode p, SortedSet<K> ki, SplitNode pnc,
                                        DeflateNode pvc) {
                parent  = p;
                putkey  = ki;
                parNClo = pnc;
                ...
                if (node.isLeaf()) {
                    ...
                    if (proc_val == null) {
                        ...
                        if (putkey.size() < proc_val.outputCapacity()) {
                            ...
                            TreeSet<K> accepted = new TreeSet<K>(Sorted.select(putkey,
                                                      proc_val.outputCapacity() / 2));
                            List<SortedSet<K>> notAccepted =
                                Sorted.split(putkey, accepted,
                                             new TreeSet<K>());    // FIXME NullSet ???

                            System.err.println(
                                "Too many keys to add to a single node: We can add " +
                                proc_val.outputCapacity() + " but are being asked to add " +
                                putkey.size() + " - adding " + accepted.size() +
                    ...
                    Iterable<SortedSet<K>> range = Sorted.split(putkey,
                                                       Sorted.keySet(node.entries), fkey);
                    ...
                    for (SortedSet<K> rng : range) {
                        ...
                        // WORKAROUND unnecessary cast here - bug in SunJDK6; works fine on OpenJDK6
                        ObjectProcessor.submitSafe(
                            proc_pull, task,
                            (SafeClosure<SkeletonNode>) new InflateChildNodes(
                                node, rng, nClo, vClo));
                    }
                }
            ...
            private void handleLocalPut(SkeletonNode n, K key, DeflateNode vClo) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "handleLocalPut for key " + key + " old value " + oldval +
                                 " for deflate node " + vClo + " - passing to proc_val");
                }

                ObjectProcessor.submitSafe(proc_val, $K(key, oldval), vClo);
            }
            private void handleLocalRemove(SkeletonNode n, K key,
                    TrackingSweeper<K, SortedSet<K>> vClo) {
                throw new UnsupportedOperationException("not implemented");
            }
        }
...
//                  if(ccount++ > 10) {
                    System.out.println( /* System.identityHashCode(this) + " " + */proc_val + " " +
                            proc_pull + " " + proc_push + " " + proc_deflate);
...
//                  }
                ...
                while (proc_push.hasCompleted()) {
                    ...
                    if (ex != null) {
                        ...
                        // FIXME HIGH
                        throw new UnsupportedOperationException(
                            "SkeletonBTreeMap.update(): PushTask aborted; handler not implemented yet",
                            ex);
                    }
                ...
                if (proc_pull.hasCompleted()) {
                    ...
                    if (ex != null) {
                        ...
                        // FIXME HIGH
                        throw new UnsupportedOperationException(
                            "SkeletonBTreeMap.update(): PullTask aborted; handler not implemented yet",
                            ex);
                    }
    ...
    public class NodeTranslator<Q, R> implements Translator<SkeletonNode, Map<String, Object>> {
        ...
        public Map<String, Object> app(SkeletonNode node) {
            if ( !node.isBare()) {
                throw new IllegalStateException("Cannot translate non-bare node " +
                        node.getRange() + "; size:" + node.nodeSize() + "; ghosts:" + node.ghosts +
                        "; root:" + (root == node));
            }
        ...
        public SkeletonNode rev(Map<String, Object> map) throws DataFormatException {
            try {
                ...
                SkeletonNode node = new SkeletonNode((ktr == null)
                                        ? (K) map.get("lkey")
                                        : ktr.rev((Q) map.get("lkey")), (ktr == null)
                                            ? (K) map.get("rkey")
                                            : ktr.rev((Q) map.get("rkey")), !notleaf, (mtr == null)
                                                ? (SkeletonTreeMap<K, V>) map.get("entries")
                                                : mtr.rev((R) map.get("entries")), gh);
                ...
                throw new DataFormatException("Could not build SkeletonNode from data", e, map,
                                              null, null);
    ...
    public static class TreeTranslator<K, V>
        ...
        public Map<String, Object> app(SkeletonBTreeMap<K, V> tree) {
            ...
            Map<String, Object> rmap = tree.makeNodeTranslator(ktr,
                                           mtr).app((SkeletonBTreeMap.SkeletonNode) tree.root);
        ...
        public SkeletonBTreeMap<K, V> rev(Map<String, Object> map) throws DataFormatException {
            try {
                SkeletonBTreeMap<K, V> tree = new SkeletonBTreeMap<K,
                                                  V>((Integer) map.get("node_min"));
                ...
                if (tree.size != tree.root.totalSize()) {
                    throw new DataFormatException("Mismatched sizes - tree: " + tree.size +
                                                  "; root: " + tree.root.totalSize(), null, null);
                }

                return tree;
            } catch (ClassCastException e) {
                throw new DataFormatException("Could not build SkeletonBTreeMap from data", e, map,
                                              null, null);
            }
        }
    }
            ...
            keySet = new AbstractSet<K>() {
                ...
                public Iterator<K> iterator() {
                    ...
                    return new Iterator<K>() {
                        ...
                        Stack<Iterator<Map.Entry<K, V>>> itstack   =
                            new Stack<Iterator<Map.Entry<K, V>>>();
                        SkeletonNode              cnode    =
                            (SkeletonNode) (SkeletonBTreeMap.this.root);
