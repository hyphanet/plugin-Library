...
/**
 ** Thrown when data hasn't been loaded yet, eg. when a {@link Skeleton} hasn't been fully loaded
 * into memory. Exceptions should be constructed such that a method call to
 * {@link Skeleton#inflate()} on {@link #getParent()} with {@link #getKey()} as its argument will
 * load the data and prevent an exception from being thrown the next time round the data is
 * accessed.
 **
 ** Ideally, this would be {@code DataNotLoadedException<K>}, where {@code K} is the type parameter
 * for {@link Skeleton}, but java does not currently allow parametrised classes to extend
 * {@link Throwable}.
 **
 ** @author infinity0
 ** @see Skeleton
