...
/**
 ** Defines an interface for an extension of a data structure which is only partially loaded into
 * memory. Operations on the missing data should throw {@link DataNotLoadedException}.
 **
 ** @author infinity0
 ** @see Serialiser
...
public interface Skeleton<K, S extends Serialiser<?>> {
  ...
  /**
   ** Inflate the entire skeleton so that after the method call, {@link #isLive()} returns true.
   */
  public void inflate() throws TaskAbortException;

  /**
   ** Deflate the entire skeleton so that after the method call, {@link #isBare()} returns true.
   */
  public void deflate() throws TaskAbortException;

  /**
   ** Partially inflate the skeleton based on some parameter object. This method may or may not also
  ...
  /**
   ** Partially deflate the skeleton based on some parameter object. This method may or may not also
