...
final public class Sorted {
    ...
    public static <E> List<SortedSet<E>> split(SortedSet<E> subj, SortedSet<E> sep,
        ...
        do {
            ...
            nsep = (csep == null)
                   ? sep.first() : second(sep.tailSet(csep));  // JDK6 sep.higher(csep);
    ...
    public static <E> List<E> select(SortedSet<E> subj, int num, Inclusivity inc) {
        if (num >= 2) {
            ...
            throw new IllegalArgumentException(
                "select(): cannot select a negative number of items");
        }

        if (subj.size() < num) {
            throw new IllegalArgumentException("select(): cannot select " + num +
                    " elements from a set of size " + subj.size());
        ...
        for (Integer s : Integers.allocateEvenly(subj.size() - num, n)) {
            ...
            if (it.hasNext()) {
                ...
                assert((sel.size() == num) &&
                       ((inc == Inclusivity.NONE) || (inc == Inclusivity.LEFT)));
            }
        }
