 ...
 **
 ** Most of the stuff here is completely fucking pointless and only because we need to support Java
 * 5. Otherwise, we can just use standard Navigable* methods. :|
 **
 ** @author infinity0
 */
final public class Sorted {
  ...
  private static class SortedKeySet<K> extends AbstractSet<K> implements SortedSet<K> {
    ...
    /* @Override **/ public SortedSet<K> headSet(K r) {
      return new SortedKeySet<K>(map.headMap(r));
    }

    /* @Override **/ public SortedSet<K> tailSet(K l) {
      return new SortedKeySet<K>(map.tailMap(l));
    }

    /* @Override **/ public SortedSet<K> subSet(K l, K r) {
      return new SortedKeySet<K>(map.subMap(l, r));
    }

  }

  /**
   ** Returns the second element in an iterable, or {@code null} if there is no such element. The
   * iterable is assumed to have at least one element.
   */
  ...
  /**
   ** Returns the least element (in the given set) >= the given element, or {@code null} if there is
   * no such element.
   **
   ** This is identical to Java 6's {@code NavigableSet.ceiling(E)}, but works for {@link SortedSet}.
   */
  ...
  /**
   ** Returns the greatest element (in the given set) <= the given element, or {@code null} if there
   * is no such element.
   **
   ** This is identical to Java 6's {@code NavigableSet.floor(E)}, but works for {@link SortedSet}.
   */
   ...
   * no such element.
   **
   ** This is identical to Java 6's {@code NavigableSet.higher(E)}, but works for {@link SortedSet}.
   */
   ...
   * is no such element.
   **
   ** This is identical to Java 6's {@code NavigableSet.lower(E)}, but works for {@link SortedSet}.
   */
  ...
  /**
   ** Returns a {@link SortedSet} view of the given {@link SortedMap}'s keys. You would think that
   * the designers of the latter would have overridden the {@link SortedMap#keySet()} method to do
   * this, but noooo...
   **
   ...
   **
   ** FIXME LOW currently this method assumes that the comparator of the set is consistent with
   * equals(). we should use the SortedSet's comparator() instead, or "natural" ordering.
   **
   ** JDK6 use a NavigableSet instead of a SortedSet.
   **
   ** @param subj Subject of the split
   ** @param sep Separators to split at
   ** @param foundsep An empty set which will be filled with the separators that were also contained
   *        in the subject set.
   ** @return A list of subsets; each subset contains all entries between two adjacent separators, or
   *         an edge separator and the corresponding edge of the set. The list is in sorted order.
   ** @throws NullPointerException if any of the inputs are {@code null}
   */
  public static <E> List<SortedSet<E>> split(SortedSet<E> subj, SortedSet<E> sep,
      SortedSet<E> foundsep) {
    if (!foundsep.isEmpty()) {
      throw new IllegalArgumentException(
          "split(): Must provide an empty set to add found separators to");
    }
    ...
    do {
      ...
      res.add((csep == null) ? subj.headSet(nsep)
          : (nsep == null) ? subj.tailSet(csub) : subj.subSet(csub, nsep));
      if (nsep == null) {
        break;
      }
  ...
  /**
   ** Select {@code n} separator elements from the subject sorted set. The elements are distributed
   * evenly amongst the set.
   **
   ** @param subj The set to select elements from
   ** @param num Number of elements to select
   ** @param inc Which sides of the set to select. For example, if this is {@code BOTH}, then the
   *        first and last elements will be selected.
  ...
  public static <E> List<E> select(SortedSet<E> subj, int num, Inclusivity inc) {
    ...
    if (subj.size() < num) {
      throw new IllegalArgumentException(
          "select(): cannot select " + num + " elements from a set of size " + subj.size());
  ...
  /**
   ** Select {@code n} separator elements from the subject sorted set. The inclusivity is given as
   * {@code NONE}.
   **
