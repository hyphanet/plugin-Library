...
/**
 ** A {@link Sweeper} which uses a given {@link Collection} to keep track of the objects added. It
 * also allows iteration through these objects, and supports calls to {@link Iterator#remove()}.
 **
 ** @author infinity0
 */
public class TrackingSweeper<T, C extends Collection<T>> extends AbstractSweeper<T>
   ...
   **
   ** If {@code coll_immute} is {@code null}, then {@code coll} will be used in its place. This has a
   * slight performance advantage over a wrapping "immutable" view; but only use it when trusted
   * code is going to access the sweeper.
   **
  ...
  /**
   ** Attempts to infer an immutable view of the given collection, using the available static
   * creators given in {@link Collections}.
   **
   ** Note that this method will always return an immutable {@link Collection} if the input is not a
   * {@link SortedSet}, {@link Set}, or {@link List}; this may or may not be what you want.
   */
