...
/**
 ** An (immutable) blueprint for an object. This class provides methods to construct new objects from
 * a list or map of property parameters, and to present (currently immutable) map-views of existing
 * objects.
 **
 ** @author infinity0
 */
public class ObjectBlueprint<T> {
  ...
  /**
   ** Map of properties to their method names. The keys are the property names; the values are the
   * names of the nullary methods to invoke to retrieve the property-values. If any map-value is
   * null, then the field with the same name as its map-key will be used instead.
   */
  final protected Map<String, String> properties;

  /**
   ** Map of properties to their corresponding fields. This is automatically generated from the
   * {@link #properties} map.
   */
  final protected Map<String, Field> prop_fields = new HashMap<String, Field>();

  /**
   ** Map of properties to their corresponding getter methods. This is automatically generated from
   * the {@link #properties} map.
   */
  final protected Map<String, Method> prop_methods = new HashMap<String, Method>();

  /**
   ** Map of properties to their corresponding types. This is automatically generated from the
   * {@link #properties} map.
   */
  final protected Map<String, Class<?>> param_type = new LinkedHashMap<String, Class<?>>();

  /**
   ** A constructor for the class, whose parameter types are the values of {@link #param_type} in the
   * same iteration order. This is automatically from the class object, and must exist (otherwise
   * construction of the blueprint will throw {@link NoSuchMethodException}).
   */
  final protected Constructor<T> constructor;

  /**
   ** Constructs a blueprint from the given {@link #properties} map. If you don't have the map ready
   * at hand, and it's a mixture of methods and fields, consider using {@link #init(Class)} instead.
   **
   ** The constructor's parameter types are given by {@code ctor_params}. This '''must be a subset'''
   * of {@code blueprint}; otherwise the blueprint constructor will not be able to determine the
   * types of those parameters. For example, if your class has two fields {@code int field_a} and
   ** {@code float field_b}, and {@code blueprint} maps <code>{"field_b":null,
  ** "field_a":null}</code>, and {@code ctor_params} lists <code>["field_b",
  ** "field_a"]</code>, then this method will search for the constructor with parameters
   * {@code (float, int)}. If it doesn't exist, {@link NoSuchMethodException} will be thrown.
   **
   ** If {@code ctor_params} is {@code null}, the parameter types will instead be inferred from the
   * keys of the properties map; the end result will be the same as if it was equal to
   * {@code blueprint.keySet()}.
   **
   ** @param c The class to represent
   ** @param blueprint The map of properties
   ** @param ctor_params Properties corresponding to the input parametrs of the object constructor.
   ** @throws NoSuchFieldException if Java reflection can't find an inferred field
   ** @throws NoSuchMethodException if Java reflection can't find a given method or the inferred
   *         constructor
  ...
  **         null}
  ...
  public ObjectBlueprint(Class<T> c, Map<String, String> blueprint, Iterable<String> ctor_params)
    ...
    if (ctor_params == null) {
      constructor =
          cls.getConstructor(param_type.values().toArray(new Class<?>[param_type.size()]));
    } else {
      List<Class<?>> types = new ArrayList<Class<?>>();
      for (String property : ctor_params) {
        types.add(param_type.get(property));
      }
   ...
   * at hand, and it's a mixture of methods and fields, consider using {@link #init(Class)} instead.
   **
   ** This constructor just delegates to {@link #ObjectBlueprint(Class, Map, Iterable)
   * ObjectBlueprint(c, blueprint, null)}.
   **
  ...
  /**
   ** Constructs a blueprint from the given collection of field names. This constructor just creates
   * the appropriate blueprint map to delegate to {@link #ObjectBlueprint(Class, Map)}.
   **
  ...
  /**
   ** Constructs a blueprint from the given properties map and the given object constructor. If you
   * don't have the map ready at hand, and it's a mixture of methods and fields, consider using
   * {@link #init(Class)} instead.
   **
   ** If {@code ctor} is {@code null}, then any calls to the blueprint's {@link #objectFromMap(Map)}
   * and {@link #newInstance(Object[])} will throw {@link NullPointerException}.
   **
   ** @param c The class to represent
   ** @param blueprint The map of properties
  ...
  **         null}
  ...
  /**
   ** Helper method for the constructors. Sets {@link #prop_fields}, {@link #prop_methods} and
   * {@link #param_type} from {@link #properties}.
   */
    ...
    for (Map.Entry<String, String> en : properties.entrySet()) {
      ...
      if (method_name == null || method_name.length() == 0) {
        ...
        if (m.getParameterTypes().length > 0) {
          throw new IllegalArgumentException("Bad blueprint: getter method " + method_name
              + " for property " + property + " takes more than 0 arguments");
        }
  ...
  /**
   ** Returns a builder for an {@link ObjectBlueprint} for the given class. This should make
   * construction of blueprints neater in source code, eg:
   **
   ** ObjectBlueprint<MyClass> bprint = ObjectBlueprint.init(MyClass.class) .addFields("name",
   * "origin", "date") .addMethod("height", "getHeight") .addMethod("width", "getWidth") .build();
   **
   **
  ...
  /**
   ** Constructs a new object by invoking the inferred constructor with the given list of arguments.
   **
   ** @throws NullPointerException if no constructor was supplied to this class' own constructor.
  ...
  /**
   ** Constructs a new object from the given map of properties and their desired values.
   **
   ** @param map Map of properties to their desired values.
   ** @throws NullPointerException if no constructor has been set
   ** @see Constructor#newInstance(Object[])
   */
  public T objectFromMap(Map<?, ?> map)
      throws InstantiationException, IllegalAccessException, InvocationTargetException {
    ...
    for (Map.Entry<String, Class<?>> en : param_type.entrySet()) {
      ...
      try {
        ...
        throw new IllegalArgumentException("Parameter for property " + property
            + " is not of the correct type should be " + type + " but is " + value.getClass(), e);
      }
      initargs[i++] = value;
    }
    return constructor.newInstance(initargs);
  ...
  /**
   ** Returns a map-view of an object. The keys are its properties as defined by the blueprint, and
   * the values are the values of those properties. The map is backed by the object, so changes to
   * the object (if any) are reflected in the map. The map itself is immutable, however (for now;
   ** this might be changed later).
   **
   ** NOTE: the below implementation is technically not optimal since {@link AbstractMap}'s remove
   * methods (and that of its entry/key/value views) actually iterate over the map looking for the
   * key. However, this shouldn't be an issue since the map doesn't support remove (calling it will
   * only throw {@link UnsupportedOperationException} when the key is found), and most objects only
   * have a small (<20) number of properties. If you find that you need such functionality, feel
   * free to implement a more optimal solution.
   **
  ...
  public Map<String, Object> objectAsMap(final T object) {
    ...
    return new AbstractMap<String, Object>() {
      ...
      final private Set<Map.Entry<String, Object>> entrySet =
                ...
                /* @Override **/ public Map.Entry<String, Object> next() {
                  ...
                  return new Map.Entry<String, Object>() {
                    ...
                    /* @Override **/ public Object getValue() {
                      return get(property, method_name);
                    }

                    /* @Override **/ public Object setValue(Object o) {
                      throw new UnsupportedOperationException(
                          "Cannot modify an object in this way.");
                    }
                  };
                }
  ...
  protected static class Builder<T> {
    ...
    /**
     ** Properties map. This uses {@link LinkedHashMap}, which iterates through the properties in the
     * same order in which they were added to the builder.
     */
    ...
    /**
     ** Sets an object constructor, and tell {@link #build()} to use
     * {@link ObjectBlueprint#ObjectBlueprint(Class, Map, Constructor)}.
     **
     ** @return {@code this}
     */
    ...
    /**
     ** Sets the constructor parameters, and tell {@link #build()} to use
     * {@link ObjectBlueprint#ObjectBlueprint(Class, Map, Iterable)}. (This is the default).
     **
     ** @return {@code this}
     */
