...
public class YamlReaderWriter implements ObjectStreamReader, ObjectStreamWriter {
    ...
    final static int MAX_PARALLEL =
        1;  // Limited by memory mainly. If memory is no object it could be limited by threads.
    ...
    public Object readObject(InputStream is) throws IOException {
        ...
        try {
            ...
            throw new DataFormatException("Yaml could not process the stream: " + is, e, is, null,
                                          null);
        } finally {
        ...
        public ExtendedRepresenter() {
            ...
            this.representers.put(Packer.BinInfo.class, new Represent() {
                ...
                public Node representData(Object data) {
                    ...
                    Map<Object, Object> map = Collections.<Object, Object>singletonMap(inf.getID(),
                                                  inf.getWeight());
            ...
            this.yamlConstructors.put("!FreenetURI", new AbstractConstruct() {
                ...
                public Object construct(Node node) {
                    ...
                    try {
                        ...
                        throw new ConstructorException("while constructing a FreenetURI",
                                node.getStartMark(), "found malformed URI " + uri, null);
                    }
                }
            });
                ...
                public Object construct(Node node) {
                    ...
                    if (map.size() != 1) {
                        throw new ConstructorException("while constructing a Packer.BinInfo",
                                node.getStartMark(), "found incorrectly sized map data " + map,
                                null);
                    }
        ...
        public class ConstructTermEntry<T extends TermEntry> extends AbstractConstruct {
            ...
            public Object construct(Node node) {
                ...
                try {
                    ...
                    throw new ConstructorException("while constructing a " +
                            blueprint.getObjectClass().getSimpleName(), node.getStartMark(),
                                "could not instantiate map " + map, null, e);
                }
            }
        }
    }
