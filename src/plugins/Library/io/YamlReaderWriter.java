...
/**
 ** Converts between an object and a stream containing a YAML document. By default, this uses a
 * {@link Yaml} processor with additional object and tag definitions relevant to the Library plugin.
 **
 ** (Ideally this would implement {@link java.io.ObjectInput} and {@link java.io.ObjectOutput} but
 * they have too many methods to bother with...)
 **
 ** @see Yaml
 ** @see Loader
 ** @see Dumper
 ** @author infinity0
 */
public class YamlReaderWriter implements ObjectStreamReader, ObjectStreamWriter {

  final public static String MIME_TYPE = "text/yaml";
  final public static String FILE_EXTENSION = ".yml";

  final static int MAX_PARALLEL = 1; // Limited by memory mainly. If memory is no object it could be
                                     // limited by threads.
  ...
  /**
   * We do NOT keep this thread-local, because the Composer is only cleared after the next call to
   * load(), so it can persist with a lot of useless data if we then use a different thread. So lets
   * just construct them as needed.
   */
  ...
  static {
    try {
      tebp_term = new ObjectBlueprint<TermTermEntry>(TermTermEntry.class,
          Arrays.asList("subj", "rel", "term"));
      tebp_index = new ObjectBlueprint<TermIndexEntry>(TermIndexEntry.class,
          Arrays.asList("subj", "rel", "index"));
      tebp_page = new ObjectBlueprint<TermPageEntry>(TermPageEntry.class,
      ...
      this.representers.put(Packer.BinInfo.class, new Represent() {
        /* @Override **/ public Node representData(Object data) {
          ...
          Map<Object, Object> map =
              Collections.<Object, Object>singletonMap(inf.getID(), inf.getWeight());
    ...
    public ExtendedConstructor() {
      ...
      this.yamlConstructors.put("!BinInfo", new AbstractConstruct() {
        /* @Override **/ public Object construct(Node node) {
          ...
          if (map.size() != 1) {
            throw new ConstructorException("while constructing a Packer.BinInfo",
                node.getStartMark(), "found incorrectly sized map data " + map, null);
          }
      ...
      /* @Override **/ public Object construct(Node node) {
        ...
        try {
          ...
          throw new ConstructorException(
              "while constructing a " + blueprint.getObjectClass().getSimpleName(),
  ...
  public static class ConstructorException
    ...
    public ConstructorException(String context, Mark contextMark, String problem,
        Mark problemMark) {
