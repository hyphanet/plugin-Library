...
/**
 ** {@link MapSerialiser} that packs a map of weighable elements (eg. objects with a {@code size()}
 * method) into a group of fixed-capacity bins. There are two main modes of operation, depending on
 * the value of {@link #NO_TINY}; in addition to this, there is an {@link #aggression} attribute
 * that affects the speed vs. optimity of the packing.
 **
 ** The class requires that the weights of each element is never greater than BIN_CAP, so that an
 * element can always totally fit into an additional bin; {@link IllegalArgumentException} will be
 * thrown when elements that violate this are encountered.
 **
 ...
 ** {@link Scale#weigh(Object)}, and pass it into the constructor. The other methods can also be
 * optionally overridden if an alternate metadata format is desired.
 **
 ** The programmer might also like wish to override the following:
 **
 ** * {@link #generator()} * {@link #preprocessPullBins(Map, Collection)} *
 * {@link #preprocessPushBins(Map, Collection)}
 **
 ** Note that this class effectively '''disables''' the "duplicate" detection of
 ** {@link ProgressTracker}; pull/push tasks are deemed to be equal if their meta/data point to the
 * same object, but this class dynamically generates the meta/data for each bin on each pull/push
 * request. This should not intefere with correctness (as long as the relevant locks in place); just
 * be slightly inefficient when requests for the same bin are given to a child serialiser that uses
 * {@link ProgressTracker} to detect duplicates.
 **
 ** If you plan on using a child serialiser that uses some other method to detect duplicates, bear in
 * mind that {@link #pullUnloaded(Map, Object)} actually '''depends''' on said behaviour for
 * correctness; see its source code for more details.
 **
 ** @author infinity0
 */
public class Packer<K, T>
    implements MapSerialiser<K, T>, Serialiser.Composite<IterableSerialiser<Map<K, T>>> {
  ...
  /**
   ** Maximum weight of a bin (except one; see {@link #push(Map, Object)} for details).
   */
  ...
  /**
   ** Whether the "tiny" (weight less than {@code BIN_CAP/2}) bin will be kept as a separate bin, or
   * merged into the next-smallest.
   */
  ...
  /**
   ** How aggressive the bin-packing algorithm is. Eg. will it load bins that are not already loaded,
   * to perform better optimisation?
   **
   ** ;0 : discount all unloaded bins (unrecommended) ;1 (default) : discount all unchanged bins
   * after the BFD step ;2 : discount all unchaged bins after the redistribution step ;3 : load all
   * bins before doing any packing (ie. re-pack from scratch)
   */
   ...
   **
   ** If you are using a child {@link Serialiser} that detects and discards duplicate pull/push
   * requests using a mechanism '''other''' than the default {@link ProgressTracker}, please first
   * read the section relating to this in the class description.
   **
  ...
  /**
   ** Constructs a Packer with the given parameters and {@link #NO_TINY} set to {@code true}.
   **
   ** If you are using a child {@link Serialiser} that detects and discards duplicate pull/push
   * requests using a mechanism '''other''' than the default {@link ProgressTracker}, please first
   * read the section relating to this in the class description.
  ...
  /**
   ** Creates a {@link IDGenerator} for use by the rest of the algorithms of this class.
   **
   ** The default generator '''requires''' all keys of the backing map to be present in the input
   * task map, since the default {@link IDGenerator} does not have any other way of knowing what bin
   * IDs were handed out in previous push operations, and it must avoid giving out duplicate IDs.
   **
   ** You can bypass this requirement by extending {@link IDGenerator} so that it can work out that
   * information (eg. using an algorithm that generates a UUID), and overriding
   * {@link #generator()}. (ID generation based on bin ''content'' is ''not'' supported, and is not
  ...
  /**
   ** Any tasks that need to be done after the bin tasks have been formed, but before they have been
   * passed to the child serialiser. The default implementation does nothing.
   */
  protected void preprocessPullBins(Map<K, PullTask<T>> tasks,
      Collection<PullTask<Map<K, T>>> bintasks) {}

  /**
   ** Any tasks that need to be done after the bin tasks have been formed, but before they have been
   * passed to the child serialiser. The default implementation does nothing.
   */
  protected void preprocessPushBins(Map<K, PushTask<T>> tasks,
      Collection<PushTask<Map<K, T>>> bintasks) {}

  /**
   ** Looks through {@code elems} for {@link PushTask}s with null data and reads its metadata to
   * determine which bin to add it to.
   * 
   * @throws TaskAbortException
   */
  protected SortedSet<Bin<K>> initialiseBinSet(SortedSet<Bin<K>> bins, Map<K, PushTask<T>> elems,
      Inventory<K, T> inv, IDGenerator gen, Object mapmeta) throws TaskAbortException {
      ...
      if (task.data == null) {
        if (task.meta == null) {
          throw new IllegalArgumentException(
              "Packer error: null data and null meta for key" + en.getKey());
        }
    ...
    for (Map.Entry<Object, Set<K>> en : binincubator.entrySet()) {
      ...
      // And so that the assertions in the next method work, which assume that the data is packed
      // already (e.g. if we have 2 bins, they contain something).
      for (K k : keys) {
        if (bin.add(k)) {
          if (bin.weight > BIN_CAP) {
            ...
            bin.remove(k);
            if (overflow == null)
              overflow = new HashMap<K, PushTask<T>>();
    ...
    if (overflow != null) {
      ...
      System.err.println("Repacking: data has grown, " + overflow.size()
          + " items don't fit, pulling data to repack");
    ...
    Bin<K> lightest;
    while (bins.size() > 1
        && (second = bins.headSet(lightest = bins.last()).last()).filled() < BIN_CAPHF) {
  ...
  /**
   ** Looks through {@code elems} for {@link PushTask}s with non-null data and packs it into the
   * given collection of bins.
   **
   ** NOTE: this method assumes the conditions described in the description for this class. It is up
   * to the calling code to ensure that they hold.
   */
  protected void packBestFitDecreasing(SortedSet<Bin<K>> bins, Map<K, PushTask<T>> elems,
      Inventory<K, T> inv, IDGenerator gen) {
  ...
  /**
   ** Given a group of bins, attempts to redistribute the items already contained within them to make
   * the weights more even.
   */
  ...
  /**
   ** Dicards all the bins which remain unchanged from what they were initialised to contain. This
   * means that we don't re-push something that is already pushed.
   */
  ...
  /**
   ** Pulls all PushTasks with null data. This is used when we need to push a bin that has been
   * assigned to hold the (unloaded) data of these tasks.
   */
    ...
    for (Map.Entry<K, PushTask<T>> en : tasks.entrySet()) {
      ...
      if (newtask == null) {
        ...
        throw new IllegalStateException(
            "The child serialiser should *not* discard (what it perceives to be) duplicate requests for bins. See the class documentation for details.");
      } else if (newtask.data == null) {
        throw new IllegalStateException(
            "Packer did not get the expected data while pulling unloaded bins; the pull method (or the child serialiser) is buggy.");
      }
   ...
   ** {@inheritDoc}
   **
   ** This implementation loads all the data for each bin referred to in the task map, and will add
   * extra tasks to the map if those pulling those bins result in extra data being loaded.
   */
  /* @Override **/ public void pull(Map<K, PullTask<T>> tasks, Object mapmeta)
      throws TaskAbortException {
      ...
      for (Map.Entry<K, PullTask<T>> en : tasks.entrySet()) {
        ...
        if (bintask == null) {
          ...
          throw new TaskAbortException(
              "Packer did not find the element (" + en.getKey() + ") in the expected bin ("
                  + scale.readMetaID(task.meta)
                  + "). Either the data is corrupt, or the child serialiser is buggy.",
      ...
      for (Map.Entry<Object, PullTask<Map<K, T>>> en : bintasks.entrySet()) {
        ...
        for (Map.Entry<K, T> el : bintask.data.entrySet()) {
          if (tasks.containsKey(el.getKey())) {
            throw new TaskAbortException(
                "Packer found an extra unexpected element (" + el.getKey() + ") inside a bin ("
                    + en.getKey()
                    + "). Either the data is corrupt, or the child serialiser is buggy.",
                new Exception("internal error"));
          }
          PullTask<T> task =
              new PullTask<T>(scale.makeMeta(en.getKey(), scale.weigh(el.getValue())));
   ...
   ** {@inheritDoc}
   **
   ** This implementation will pack all tasks with non-null data into bins, taking into account bins
   * already allocated (as defined by the tasks with null data and non-null metadata).
   **
   ** There are two main modes of operation:
   **
   ** ; {@link #NO_TINY} is {@code false} : Uses the Best-Fit Decreasing bin-packing algorithm, plus
   * an additional redistribution algorithm that "evens out" the bins. Each bin's weight will be
   * between [BIN_CAP/2, BIN_CAP], except for at most one bin. ; {@link #NO_TINY} is {@code true} :
   ** As above, but if the exceptional bin exists, it will be merged with the next-smallest bin, if
   * that exists. As before, each bin will weigh between [BIN_CAP/2, BIN_CAP], except for the merged
   * element (if it exists), which will weigh between (BIN_CAP, BIN_CAP*3/2).
   **
   ** In addition to this, the {@link #aggression} attribute will affect when fully-unloaded
   * unchanged bins are discarded.
   **
   ** The default generator '''requires''' all keys of the backing map to be present in the input
   * task map, since the default {@link IDGenerator} does not have any other way of knowing what bin
   * IDs were handed out in previous push operations, and it must avoid giving out duplicate IDs.
   ...
   * a priority at present.)
   */
  /* @Override **/ public void push(Map<K, PushTask<T>> tasks, Object mapmeta)
      throws TaskAbortException {
    ...
    try {
      ...
      if (logDEBUG)
        Logger.debug(this, "Aggression = " + agg + " tasks size = " + tasks.size());
   ...
   **
   ** NOTE: this implementation is incomplete, since we don't override the remove() methods of the
   * sets and iterators returned by the subset and iterator methods, to also recalculate the weight,
   * but these are never used so it's OK.
   **
   ** @author infinity0
   */
  protected static class Bin<K> extends TreeSet<K> implements Comparable<Bin<K>> {
    ...
    /**
     ** Descending comparator for total bin load, ie. ascending comparator for remainding weight
     * capacity. {@link DummyBin}s are treated as the "heaviest", or "least empty" bin for its
     * weight.
     */
  ...
  /************************************************************************
   ** A class that pretends to be a bin with a certain capacity and weight. This is used when we want
   * a such a bin for some purpose (eg. as an argument to a comparator) but we don't want to have to
   * populate a real bin to get the desired weight (which might be massive).
   **
   ** @author infinity0
   */
  ...
  /************************************************************************
   ** A class that provides a "weight" assignment for each key in a given map, by reading either the
   * data or the metadata of its associated task.
   **
   ** @author infinity0
   */
  protected static class Inventory<K, T> extends IdentityComparator<K> {
    ...
    /**
     ** Keeps track of "the exceptional element" as defined in the description for
     * {@link #push(Map, Object)}.
     */
    protected K giant;
      ...
      if (i == null) {
        ...
        if (i > packer.BIN_CAP) {
          if (packer.NO_TINY && giant == null) {
            ...
            throw new IllegalArgumentException("Element " + key
                + " greater than the capacity allowed: " + i + "/" + packer.BIN_CAP);
          }
        }
        weights.put(key, i);
      }
    ...
    ** null} key is treated as the "lightest" key for its weight.
    ...
    /**
     ** Set the weight for the {@code null} key. This is useful for when you want to obtain a subset
     * using tailSet() or headSet(), but don't have a key with the desired weight at hand.
     */
  ...
  /************************************************************************
   ** Generates unique IDs for the bins. This generator assigns {@link Long}s in sequence;
   * registering any type of integer object reference will cause future automatically-assigned IDs
   * to be greater than that integer.
   **
   ** This implementation cannot ensure uniqueness of IDs generated (with respect to ones generated
   * in a previous session), unless they are explicitly registered using
   * {@link #registerID(Object)}.
   **
   ** @author infinity0
   */
  public static class IDGenerator {
    ...
    /**
     ** Register an ID that was assigned in a previous session, so the generator doesn't output that
     * ID again.
     */
   ...
   **
   ** To implement this class, the programmer needs to implement the {@link #weigh(Object)} method.
   **
   ** @author infinity0
   */
  abstract public static class Scale<T> {
    ...
    /**
     ** Constructs the metadata for a bin, from the bin ID and the map-wide metadata.
     */
