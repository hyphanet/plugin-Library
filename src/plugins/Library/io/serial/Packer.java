...
public class Packer<K, T>
    ...
    public void setAggression(int i) {
        if ((i < 0) || (i > 3)) {
            throw new IllegalArgumentException(
                "Invalid aggression value: only 0,1,2,3 is allowed.");
        }
    ...
    protected SortedSet<Bin<K>> initialiseBinSet(SortedSet<Bin<K>> bins, Map<K, PushTask<T>> elems,
            Inventory<K, T> inv, IDGenerator gen, Object mapmeta)
            throws TaskAbortException {
    ...
    public void pull(Map<K, PullTask<T>> tasks, Object mapmeta) throws TaskAbortException {
        try {
            ...
            for (Map.Entry<K, PullTask<T>> en : tasks.entrySet()) {
                ...
                } else if ((bintask.data == null) ||
                           (task.data = bintask.data.remove(en.getKey())) == null) {
                    throw new TaskAbortException(
                        "Packer did not find the element (" + en.getKey() +
                        ") in the expected bin (" + scale.readMetaID(task.meta) +
                        "). Either the data is corrupt, or the child serialiser is buggy.", new Exception(
                            "internal error"));
                }
            }
            ...
            for (Map.Entry<Object, PullTask<Map<K, T>>> en : bintasks.entrySet()) {
                ...
                for (Map.Entry<K, T> el : bintask.data.entrySet()) {
                    if (tasks.containsKey(el.getKey())) {
                        throw new TaskAbortException(
                            "Packer found an extra unexpected element (" + el.getKey() +
                            ") inside a bin (" + en.getKey() +
                            "). Either the data is corrupt, or the child serialiser is buggy.", new Exception(
                                "internal error"));
                    }

                    PullTask<T> task = new PullTask<T>(scale.makeMeta(en.getKey(),
                                           scale.weigh(el.getValue())));
    ...
    */
        ...
        try {
            ...
            for (PushTask<Map<K, T>> bintask : bintasks) {
                for (K k : bintask.data.keySet()) {
                    tasks.get(k).meta = scale.makeMeta(scale.readBinMetaID(bintask.meta),
                            inv.getWeight(k));
    ...
    protected static class Bin<K> extends TreeSet<K> implements Comparable<Bin<K>> {
        ...
        public int compareTo(Bin<K> bin) {
            ...
            return (bin instanceof DummyBin)
                   ? -1 : IdentityComparator.comparator.compare(this, bin);
        }
    }
    ...
    protected static class Inventory<K, T> extends IdentityComparator<K> {
        ...
        public int getWeight(K key) {
            ...
            if (i == null) {
                ...
                if (task == null) {
                    throw new IllegalArgumentException("This scale does not have a weight for " +
                            key);
