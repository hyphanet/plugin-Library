...
/**
 ** An interface for asynchronous task execution. The methods return objects for managing and
 * scheduling tasks.
 **
 ** @author infinity0
 */
public interface ScheduledSerialiser<T> extends IterableSerialiser<T> {
  ...
  /**
   ** Creates a {@link ObjectProcessor} for executing {@link PullTask}s, which should be
   * {@linkplain ObjectProcessor#auto()} automatically started.
   **
  ...
  /**
   ** Creates a {@link ObjectProcessor} for executing {@link PushTask}s, which should be
   * {@linkplain ObjectProcessor#auto()} automatically started.
   **
   ** @param input Queue to add task requests to
   ** @param output Queue to pop completed tasks from
