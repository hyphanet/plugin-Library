...
/**
 ** An interface that handles a single {@link Serialiser.Task} and sends live updates of its
 * {@link Progress}.
 **
 ** @author infinity0
 */
public interface LiveArchiver<T, P extends Progress> extends Archiver<T> {
   ...
   ** {@link TaskAbortException}s are stored in the progress object, rather than thrown.
   **
   ** Implementations must also modify the state of the progress object such that after the operation
   * completes, all threads blocked on {@link Progress#join()} will either return normally or throw
   * the exact {@link TaskAbortException} that caused it to abort.
   **
