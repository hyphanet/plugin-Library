...
/**
 ** An {@link IterableSerialiser} that uses threads to handle tasks given to it in parallel, and
 * keeps track of task progress.
 **
 ** To implement this class, the programmer must implement the
 * {@link LiveArchiver#pullLive(Serialiser.PullTask, Progress)} and
 * {@link LiveArchiver#pushLive(Serialiser.PushTask, Progress)} methods.
 **
 ** DOCUMENT (rewritten)
 **
 ** @author infinity0
 */
public abstract class ParallelSerialiser<T, P extends Progress> implements IterableSerialiser<T>,
    ScheduledSerialiser<T>, LiveArchiver<T, P>, Serialiser.Trackable<T> {

  final static protected ThreadPoolExecutor exec =
      new ThreadPoolExecutor(0, 0x40, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
          new ThreadPoolExecutor.CallerRunsPolicy() // easier than catching
                                                    // RejectedExecutionException, if it ever occurs
      );
  ...
  protected <K extends Task> Runnable createJoinRunnable(final K task,
      final TaskInProgressException e, final SafeClosure<X2<K, TaskAbortException>> post) {
    return new Runnable() {
      public void run() {
   ...
   ** DOCUMENT.
   **
   ** If {@code post} is {@code null}, it is assumed that {@link Progress} objects are created by the
   * thread that calls this method. Failure to do this '''will result in deadlock'''.
   **
   ** Otherwise, this method will create {@link Progress} objects for each task. If they are created
   * before this, '''deadlock will result''' as the handler waits for that progress to complete.
   */
  protected Runnable createPullJob(final PullTask<T> task,
      final SafeClosure<X2<PullTask<T>, TaskAbortException>> post) {
    try {
      ...
      return new Runnable() {
        public void run() {
          ...
          // Take semaphore (purpose is to limit the number of decoded stuff in memory which is not
          // yet wanted)
   ...
   * before this, '''deadlock will result''' as the handler waits for that progress to complete.
   */
  protected Runnable createPushJob(final PushTask<T> task,
      final SafeClosure<X2<PushTask<T>, TaskAbortException>> post) {
    try {
  ...
  /* @Override **/ public <E> ObjectProcessor<PullTask<T>, E, TaskAbortException> pullSchedule(
      BlockingQueue<PullTask<T>> input, BlockingQueue<X2<PullTask<T>, TaskAbortException>> output,
      Map<PullTask<T>, E> deposit) {
    return new ObjectProcessor<PullTask<T>, E, TaskAbortException>(input, output, deposit, null,
        Executors.DEFAULT_EXECUTOR, new TaskAbortExceptionConvertor()) {
      @Override
      protected Runnable createJobFor(PullTask<T> task) {
  ...
  /* @Override **/ public <E> ObjectProcessor<PushTask<T>, E, TaskAbortException> pushSchedule(
      BlockingQueue<PushTask<T>> input, BlockingQueue<X2<PushTask<T>, TaskAbortException>> output,
      Map<PushTask<T>, E> deposit) {
    return new ObjectProcessor<PushTask<T>, E, TaskAbortException>(input, output, deposit, null,
        Executors.DEFAULT_EXECUTOR, new TaskAbortExceptionConvertor()) {
      @Override
      protected Runnable createJobFor(PushTask<T> task) {
