...
/**
 * Performs asynchronous searches over many index or with many terms and search logic TODO review
 * documentation
 * 
 * @author MikeB
 */
public class Search extends AbstractExecution<Set<TermEntry>>
  ...
  /**
   * Settings for producing result nodes, if true a HTMLNode of the results will be generated after
   * the results are complete which can be accessed via getResultNode()
   */
  ...
  private enum SearchStatus {
    Unstarted, Busy, Combining_First, Combining_Last, Formatting, Done
  };
  ...
  public static Search startSearch(String search, String indexuri)
    ...
    if (indices.length < 1 || search.trim().length() < 1)
      ...
      ArrayList<Execution<Set<TermEntry>>> indexrequests =
          new ArrayList<Execution<Set<TermEntry>>>(indices.length);
   ...
   **/
  private Search(String query, String indexURI, List<? extends Execution<Set<TermEntry>>> requests,
      ResultOperation resultOperation) throws InvalidSearchException {
    super(makeString(query, indexURI));
    if (resultOperation == ResultOperation.SINGLE && requests.size() != 1)
      throw new InvalidSearchException(
          requests.size() + " requests supplied with SINGLE operation");
    ...
    if ((resultOperation == ResultOperation.PHRASE
        ...
        || resultOperation == ResultOperation.DIFFERENTINDEXES) && requests.size() < 2)
      throw new InvalidSearchException(
    ...
    for (Execution<Set<TermEntry>> request : requests) {
      if (request != null || resultOperation == ResultOperation.PHRASE)
        ...
        throw new NullPointerException(
            "Search cannot encapsulate nulls except in the case of a ResultOperation.PHRASE where they are treated as blanks");
    }
  ...
  private Search(String query, String indexURI, Execution<Set<TermEntry>> request) {
    ...
    if (request == null)
      throw new NullPointerException(
          "Search cannot encapsulate null (query=\"" + query + "\" indexURI=\"" + indexURI + "\")");
    query = query.toLowerCase(Locale.US).trim();
  ...
  private static Search splitQuery(String query, String indexuri)
    ...
    if (query.matches("\\A[\\S&&[^-\"]]*\\Z")) {
      ...
      if (request == null)
        throw new InvalidSearchException(
            "Something wrong with query=\"" + query + "\" or indexURI=\"" + indexuri
    ...
    if (logMINOR)
      Logger.minor(Search.class, "Phrases removed query: " + sb);

    // Remove the unmatched \" (if any)
    String formattedquery = sb.toString().replaceFirst("\"", "");
    if (logMINOR)
      Logger.minor(Search.class, "Removing the unmatched bracket: " + formattedquery);
    ...
    if (logMINOR)
      Logger.minor(Search.class, "OR-subst query : " + formattedquery);
    formattedquery = formattedquery.replaceAll("\\s+(?:not\\s*|-)(\\S+)", "^^($1)");
    if (logMINOR)
      Logger.minor(Search.class, "NOT-subst query : " + formattedquery);
    formattedquery = formattedquery.replaceAll("\\s+", "&&");
    if (logMINOR)
      Logger.minor(Search.class, "AND-subst query : " + formattedquery);
    ...
    for (int i = 1; i < phraseparts.length; i++) {
      ...
      formattedquery += "\"" + phrases.get(Integer.parseInt(string.replaceFirst("(\\d+).*", "$1")))
    ...
    if (formattedquery.contains("&&")) {
      ...
      for (String subquery : intersects) {
        ...
        if (subsearch != null) // We will assume that searching for 'the big apple' will near enough
                               // show the same results as 'big apple', so just ignore 'the' in
                               // interseaction
    ...
    if (formattedquery.contains("||")) {
      ...
      for (String subquery : unions) {
        ...
        if (add == null) // eg a search for 'the or cake' would be almost the same as a search for
                         // 'the' and so should be treated as such
          return null;
  ...
  /**
   * Creates a string which uniquly identifies this Search object for comparison and lookup, wont
   * make false positives but can make false negatives as search and indexuri aren't standardised
   *
   * @param search
   * @param indexuri
   */
  ...
  public List<? extends Progress> getSubProgress() {
    ...
    for (Execution<Set<TermEntry>> request : subsearches) {
      if (request == null)
        continue;
      if (request instanceof CompositeProgress
          && ((CompositeProgress) request).getSubProgress() != null
          && ((CompositeProgress) request).getSubProgress().iterator().hasNext()) {
        for (Iterator<? extends Progress> it =
            ((CompositeProgress) request).getSubProgress().iterator(); it.hasNext();) {
  ...
  /**
   * @return true if all are Finished and Result is ready, also stimulates the creation of the
   *         result if all subreqquests are complete and the result isn't made
   */
  @Override
  ...
  private synchronized void setStatus() throws TaskAbortException {
    switch (status) {
      ...
      case Busy:
        ...
        status = SearchStatus.Combining_First; // If Busy and waiting for subrequests to combine,
                                               // status -> Combining_First
  ...
  private boolean isSubRequestsComplete() throws TaskAbortException {
    for (Execution<Set<TermEntry>> r : subsearches) {
      try {
        ...
        if (innerCanFailAndStillComplete())
          continue;
        throw e;
      }
    }
  ...
  public void remove() {
    ...
    // FIXME in fact this shouldn't be necessary, the TaskAbortException should be propagated
    // upwards???
    // FIXME really we'd want to convert it into a failed status so we could show that one failed,
    // and still show partial results
    if (subsearches != null) {
