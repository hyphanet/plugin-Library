...
public class Search extends AbstractExecution<Set<TermEntry>>
    ...
    public static Search startSearch(String search, String indexuri)
        ...
        if ((indices.length < 1) || (search.trim().length() < 1)) {
            ...
            Search newSearch = new Search(search, indexuri, indexrequests,
                                          ResultOperation.DIFFERENTINDEXES);
     ...
     */
    private Search(String query, String indexURI,
                   List<? extends Execution<Set<TermEntry>>> requests,
                   ResultOperation resultOperation)
            throws InvalidSearchException {
        super(makeString(query, indexURI));

        if ((resultOperation == ResultOperation.SINGLE) && (requests.size() != 1)) {
            throw new InvalidSearchException(requests.size() +
                    " requests supplied with SINGLE operation");
        }
        ...
        if (((resultOperation == ResultOperation.PHRASE) ||
            ...
            throw new InvalidSearchException(resultOperation.toString() +
                    " operations need more than one term");
        }

        query = query.toLowerCase(Locale.US).trim();

        // Create a temporary list of sub searches then make it unmodifiable
        List<Execution<Set<TermEntry>>> tempsubsearches =
    ...
    private Search(String query, String indexURI, Execution<Set<TermEntry>> request) {
        ...
        if (request == null) {
            throw new NullPointerException("Search cannot encapsulate null (query=\"" + query +
                    "\" indexURI=\"" + indexURI + "\")");
        }
    ...
    private static Search splitQuery(String query, String indexuri)
        ...
        if (query.matches("\\A[\\S&&[^-\"]]*\\Z")) {
            ...
            if (request == null) {
                throw new InvalidSearchException("Something wrong with query=\"" + query +
                        "\" or indexURI=\"" + indexuri +
        ...
        if (query.matches("\\A\"[^\"]*\"\\Z") ||
            ...
            while ((phrasesearches.size() > 0) &&
                    (phrasesearches.get(phrasesearches.size() - 1) == null)) {
        ...
        for (int i = 1; i < phraseparts.length; i++) {
            ...
            if (logMINOR) {
                Logger.minor(Search.class,
                             "replacing phrase " + string.replaceFirst("(\\d+).*", "$1"));
            }

            formattedquery += "\"" +
                              phrases.get(Integer.parseInt(string.replaceFirst("(\\d+).*",
                                  "$1"))) + "\"" + string.replaceFirst("\\d+€(.*)", "$1");
        }
        ...
        if (formattedquery.contains("&&")) {
            ...
            for (String subquery : intersects) {
                ...
                if (subsearch !=
                        null) {  // We will assume that searching for 'the big apple' will near enough show the same results as 'big apple', so just ignore 'the' in interseaction
            ...
            switch (intersectsearches.size()) {
                ...
                default :
                    return new Search(query, indexuri, intersectsearches,
    ...
    public String toString() {
        return "Search: " + resultOperation + " - " + status + " : " + subject + " : " +
               subsearches;
    }
        ...
        for (Execution<Set<TermEntry>> request : subsearches) {
            ...
            if ((request instanceof CompositeProgress) &&
                ...
                for (Iterator<? extends Progress> it =
                        ((CompositeProgress) request).getSubProgress().iterator();
                        it.hasNext(); ) {
        ...
        switch (status) {
            ...
            case Busy :
                if ( !isSubRequestsComplete()) {
                    for (Execution<Set<TermEntry>> request : subsearches) {
                        if ((request != null) &&
                                ( !(request instanceof Search) ||
                                  ((Search) request).status == SearchStatus.Busy)) {
                ...
                status = SearchStatus
                    .Combining_First;  // If Busy and waiting for subrequests to combine, status -> Combining_First
            ...
            case Combining_Last :  // for when this is combining
                ...
                if (formatResult) {
                    ...
                    resultNodeGenerator = new ResultNodeGenerator(resultset, htmlgroupusk,
                            htmlshowold, htmljs);

                    if (executor != null) {
                        executor.execute(resultNodeGenerator,
                                         "Library.Search : formatting results");
                    } else {
                        (new Thread(resultNodeGenerator,
                                    "Library.Search : formatting results")).start();
                    }
