...
public class ResultSet implements Set<TermEntry>, Runnable {
    ...
    ResultSet(String subject, ResultOperation resultOperation,
              List<Execution<Set<TermEntry>>> subRequests, boolean ignoreTAEs)
            throws TaskAbortException {
        if ((resultOperation == ResultOperation.SINGLE) && (subRequests.size() != 1)) {
            throw new IllegalArgumentException(subRequests.size() +
        ...
        if (((resultOperation == ResultOperation.PHRASE) ||
                ...
                (resultOperation == ResultOperation.DIFFERENTINDEXES)) && (subRequests.size() <
                2)) {
            throw new IllegalArgumentException(resultOperation.toString() +
        ...
        for (Iterator<? extends TermEntry> it = firstCollection.iterator(); it.hasNext(); ) {
            ...
            if (i == collections.length) {
                TermEntry newEntry = convertEntry(termEntry,
                                         combinedrelevance / collections.length);
    ...
    private void phrase(Collection<? extends TermEntry>... collections) {
        ...
        for (TermEntry termEntry : firstCollection) {
            ...
            for (i = 1; (positions != null) && (i < collections.length) && (positions.size() > 0);
                    i++) {
                Collection<? extends TermEntry> collection = collections[i];
                ...
                TermPageEntry termPageEntry1 = (TermPageEntry) getIgnoreSubject(termPageEntry,
                                                   collection);

                if ((termPageEntry1 == null) || !termPageEntry1
                        .hasPositions()) {    // If collection doesnt contain this termpageentry or has not positions, it does not follow
                    positions = null;
                } else {
                    for (Iterator<Integer> it = positions.keySet().iterator(); it.hasNext(); ) {
                        ...
                        if ( !termPageEntry1.hasPosition(posi + i)) {
                            ...
                            Logger.minor(this,
                                         termPageEntry.page + "[" + positions.keySet() +
                                         "] is followed by " + termPageEntry1.page + "[" +
                                         termPageEntry1.positions() + "] +" + i);
                        }
                    }
                }
            }
    ...
    private TermEntry convertEntry(TermEntry termEntry, float rel) {
        ...
        if (termEntry instanceof TermTermEntry) {
            ...
            entry = new TermPageEntry(subject, rel, ((TermPageEntry) termEntry).page,
                                      ((TermPageEntry) termEntry).title,
                                      ((TermPageEntry) termEntry).positionsMap());
        } else if (termEntry instanceof TermIndexEntry) {
            entry = new TermIndexEntry(subject, rel, ((TermIndexEntry) termEntry).index);
        } else {
            throw new UnsupportedOperationException("The TermEntry type " +
                    termEntry.getClass().getName() + " is not currently supported in ResultSet");
        }
    ...
    private TermEntry mergeEntries(TermEntry... entries) {
        for (int i = 1; i < entries.length; i++) {
            if ( !entries[0].equalsTarget(entries[i])) {
                throw new IllegalArgumentException("entries were not equal : " +
                        entries[0].toString() + " & " + entries[i].toString());
            }
        }
        ...
        if (combination instanceof TermIndexEntry) {
            combination = new TermIndexEntry(subject, entries[0].rel,
                    ((TermIndexEntry) combination).index);
        } else if (combination instanceof TermPageEntry) {
            combination = new TermPageEntry(subject, entries[0].rel,
                    ((TermPageEntry) combination).page, ((TermPageEntry) combination).title,
                    ((TermPageEntry) combination).positionsMap());
        } else if (combination instanceof TermTermEntry) {
            combination = new TermTermEntry(subject, entries[0].rel,
                    ((TermTermEntry) combination).term);
        } else {
            throw new IllegalArgumentException("Unknown type : " +
                    combination.getClass().toString());
        }
    ...
    private TermEntry combine(TermEntry entry1, TermEntry entry2) {
        ...
        float newRel = entry1.rel / ((entry2.rel == 0) ? 1 : 2) +
                       entry2.rel / ((entry1.rel == 0) ? 1 : 2);
        ...
        if (entry1 instanceof TermPageEntry) {
            ...
            return new TermPageEntry(pageentry1.subj, newRel, pageentry1.page,
                                     (pageentry1.title != null)
                                     ? pageentry1.title : pageentry2.title, newPos);
     ...
     * @param collection
     */
    private TermEntry getIgnoreSubject(TermEntry entry,
