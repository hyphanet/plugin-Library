...
/**
 * Unmodifiable Set which makes sure all data in results being combined is retained and is created
 * as the result of Set operations on other Sets. Does not contain references to the original Sets.
 * <br />
 * <br />
...
public class ResultSet implements Set<TermEntry>, Runnable {
  ...
  /**
   * What should be done with the results of subsearches\n INTERSECTION : ( >2 subRequests) result
   * is common results of subsearches\n UNION : ( >2 subRequests) result is all results of
   * subsearches\n REMOVE : ( 2 subRequests) result is the results of the first subsearch with the
   * second search removed PHRASE : ( >2 subRequests) For a phrase, the subsearches are in the order
   * of the words in a phrase SINGLE : ( 1 subRequests) To encapsulate a single request
   * DIFFERENTINDEXES : ( >2 subRequests) for a serch made up of requests on multiple indexes
   */
  public enum ResultOperation {
    INTERSECTION, UNION, REMOVE, PHRASE, SINGLE, DIFFERENTINDEXES
  };
   ...
   *
   *         TODO reevaluate relevance for all combinations, and find a way to calculate relevance
   *         of phrases
   */
  ResultSet(String subject, ResultOperation resultOperation,
      List<Execution<Set<TermEntry>>> subRequests, boolean ignoreTAEs) throws TaskAbortException {
    if (resultOperation == ResultOperation.SINGLE && subRequests.size() != 1)
      throw new IllegalArgumentException(
          subRequests.size() + " requests supplied with SINGLE operation");
    ...
    if ((resultOperation == ResultOperation.PHRASE
        ...
        || resultOperation == ResultOperation.DIFFERENTINDEXES) && subRequests.size() < 2)
      throw new IllegalArgumentException(
          resultOperation.toString() + " operations need more than one term");
  ...
  public synchronized void run() {
    ...
    try {
      ...
      exception = e; // Exeptions thrown here are stored in case this is being run in a thread, in
                     // this case it is thrown in isDone() or iterator()
      throw e;
    }
  ...
  /**
   * Overwrite add all entries in result to the internal Set converted to this ResultSet's subject.
   * If the Set already has anything inside it you should use addAllInternal below.
   * 
   * @param result
   */
   ...
   */
  private void exclude(Collection<? extends TermEntry> add,
      Collection<? extends TermEntry> subtract) {
   ...
   * 
   * @param collections to be merged into this collection TODO proper relevance calculating here,
   *        currently i think the relevance of the first one added will have less impact than the
   *        others, the other 3 types are more important i believe
   */
  ...
  /**
   * Iterate over the first collection adding those elements which exist in all the other
   * collections
   * 
  ...
  /**
   * Iterate over the first collection, and the termpositions of each entry, keeping those positions
   * which are followed in the other collections. Keeps those entries which have positions remaingin
   * after this process
   * 
   * @param collections
   */
  private void phrase(Collection<? extends TermEntry>... collections) {
    ...
    for (TermEntry termEntry : firstCollection) {
      ...
      for (i = 1; positions != null && i < collections.length && positions.size() > 0; i++) {
        ...
        if (termPageEntry1 == null || !termPageEntry1.hasPositions()) // If collection doesnt
                                                                      // contain this termpageentry
                                                                      // or has not positions, it
                                                                      // does not follow
          positions = null;
        else {
          for (Iterator<Integer> it = positions.keySet().iterator(); it.hasNext();) {
            ...
            if (!termPageEntry1.hasPosition(posi + i))
              ...
              Logger.minor(this, termPageEntry.page + "[" + positions.keySet() + "] is followed by "
                  + termPageEntry1.page + "[" + termPageEntry1.positions() + "] +" + i);
          }
        }
      }
      // if this termentry has any positions remaining, add it
      if (positions != null && positions.size() > 0)
        addInternal(new TermPageEntry(subject, termPageEntry.rel, termPageEntry.page,
            termPageEntry.title, positions));
    }
  }
  ...
  private TermEntry convertEntry(TermEntry termEntry, float rel) {
    ...
    if (termEntry instanceof TermTermEntry)
      ...
      entry = new TermPageEntry(subject, rel, ((TermPageEntry) termEntry).page,
          ((TermPageEntry) termEntry).title, ((TermPageEntry) termEntry).positionsMap());
    else if (termEntry instanceof TermIndexEntry)
      entry = new TermIndexEntry(subject, rel, ((TermIndexEntry) termEntry).index);
    else
      throw new UnsupportedOperationException("The TermEntry type " + termEntry.getClass().getName()
          + " is not currently supported in ResultSet");
    return entry;
  }
  ...
  /**
   * Merge a group of TermEntries each pair of which(a, b) must be a.equalsTarget The new TermEntry
   * created will have the subject of this ResultSet, it will try to combine all the optional fields
   * from the TermEntrys being merged, with priority being put on those earliest in the arguments
   *
   * @param entries
  ...
  private TermEntry mergeEntries(TermEntry... entries) {
    for (int i = 1; i < entries.length; i++)
      if (!entries[0].equalsTarget(entries[i]))
        throw new IllegalArgumentException(
            "entries were not equal : " + entries[0].toString() + " & " + entries[i].toString());

    TermEntry combination = entries[0];

    // This mess could be replaced by a method in the TermEntrys which returns a new TermEntry with
    // a changed subject
    if (combination instanceof TermIndexEntry) {
      combination =
          new TermIndexEntry(subject, entries[0].rel, ((TermIndexEntry) combination).index);
    } else if (combination instanceof TermPageEntry) {
      combination = new TermPageEntry(subject, entries[0].rel, ((TermPageEntry) combination).page,
          ((TermPageEntry) combination).title, ((TermPageEntry) combination).positionsMap());
  ...
  private TermEntry combine(TermEntry entry1, TermEntry entry2) {
    ...
    float newRel =
        entry1.rel / ((entry2.rel == 0) ? 1 : 2) + entry2.rel / ((entry1.rel == 0) ? 1 : 2);
    ...
    if (entry1 instanceof TermPageEntry) {
      ...
      return new TermPageEntry(pageentry1.subj, newRel, pageentry1.page,
          (pageentry1.title != null) ? pageentry1.title : pageentry2.title, newPos);
      ...
      throw new UnsupportedOperationException(
          "This type of TermEntry is not yet supported in the combine code : "
              + entry1.getClass().getName());
   ...
   */
  private Set<TermEntry>[] getResultSets(List<Execution<Set<TermEntry>>> subRequests,
      boolean ignoreTAEs) throws TaskAbortException {
      ...
      if (subRequests.get(i) == null) {
        if (resultOperation == ResultOperation.PHRASE)
          ...
          throw new NullPointerException(
              "Nulls not allowed in subRequests for operations other than phrase.");
      } else {
        try {
