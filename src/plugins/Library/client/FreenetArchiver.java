...
/**
 ** Converts between a map of {@link String} to {@link Object}, and a freenet key. An
 * {@link ObjectStreamReader} and an {@link ObjectStreamWriter} is used to do the hard work once the
 * relevant streams have been established, from temporary {@link Bucket}s.
 **
 ** Supports a local cache. FIXME: NOT GARBAGE COLLECTED! Eventually will fill up all available disk
 * space!
 **
 ** @author infinity0
 */
public class FreenetArchiver<T> implements LiveArchiver<T, SimpleProgress> {
  ...
  /**
   * If true, we will insert data semi-asynchronously. That is, we will start the insert, with
   * ForceEncode enabled, and return the URI as soon as possible. The inserts will continue in the
   * background, and before inserting the final USK, the caller should call waitForAsyncInserts().
   * 
   * FIXME SECURITY: This should be relatively safe, because it's all CHKs, as long as the content
   * isn't easily predictable.
   * 
   * FIXME PERFORMANCE: This dirty hack is unnecessary if we split up IterableSerialiser.push into a
   * start phase and a stop phase.
   * 
   * The main purpose of this mechanism is to minimise memory usage: Normally the data being pushed
   * remains in RAM while we do the insert, which can take a very long time.
   */
  ...
  public FreenetArchiver(NodeClientCore c, ObjectStreamReader r, ObjectStreamWriter w, String mime,
      int size, short priority) {
    if (c == null) {
      throw new IllegalArgumentException(
          "Can't create a FreenetArchiver with a null NodeClientCore!");
    }
  ...
  public <S extends ObjectStreamWriter & ObjectStreamReader> FreenetArchiver(NodeClientCore c, S rw,
   ...
   *
   * FIXME OPT NORM: Two-phase pull: First pull to a bucket, then construct the data. The reason for
   * this is memory usage: We can limit the number in the second phase according to downstream
   * demand (e.g. blocking queues in ObjectProcessor's), so that the amount of stuff kept in memory
   * is limited, while still allowing an (almost?) unlimited number in the fetching data phase. See
   * comments in ParallelSerialiser.createPullJob.
   */
  /* @Override **/ public void pullLive(PullTask<T> task, final SimpleProgress progress)
        ...
        try {
          ...
          if (tempB == null) {
            ...
            if (initialMetadata != null)
              Logger.debug(this,
                  "Fetching block for FreenetArchiver from metadata (" + cacheKey + ")");
            else
            ...
            if (progress != null) {
              ...
              if (prog_old.known - prog_old.done != prog_new.known - prog_new.done) {
                Logger.error(this,
                    "Inconsistency when tracking split file progress (pulling): " + prog_old.known
                        + " of " + prog_old.done + " -> " + prog_new.known + " of "
                        + prog_new.done);
                System.err.println("Inconsistency when tracking split file progress (pulling): "
                    + prog_old.known + " of " + prog_old.done + " -> " + prog_new.known + " of "
                    + prog_new.done);
              }
          ...
          Logger.debug(this,
              "Fetched block for FreenetArchiver in " + (endTime - startTime) + "ms.");
   ...
   **
   ** If the input metadata is an insert URI (SSK or USK), it will be replaced by its corresponding
   * request URI. Otherwise, the data will be inserted as a CHK. Note that since {@link FreenetURI}
   * is immutable, the {@link FreenetURI#suggestedEdition} of a USK is '''not''' automatically
   ** incremented.
   */
  /* @Override **/ public void pushLive(PushTask<T> task, final SimpleProgress progress)
      throws TaskAbortException {
    HighLevelSimpleClient hlsc = core.makeClient(priorityClass, false, false);
    ...
    try {
      ...
      try {
        ...
        putter = new ClientPutter(cb, ib.getData(), FreenetURI.EMPTY_CHK_URI, ib.clientMetadata,
            ctx, priorityClass, false, null, false, core.clientContext, null,
            insertAsMetadata ? CHKBlock.DATA_LENGTH : -1);
        cb.setPutter(putter);
        ...
        if (status == WAIT_STATUS.FAILED) {
          ...
          Logger.debug(this, "Got URI for asynchronous insert: " + uri + " size " + tempB.size()
          ...
          Logger.debug(this,
              "Got generated metadata (" + buf.length + " bytes) for asynchronous insert size "
                  + tempB.size() + " in " + (System.currentTimeMillis() - cb.startTime));
        }
        if (progress != null)
          progress.addPartDone();
        // }
        ...
        if (progress != null) {
          ...
          if (prog_old.known - prog_old.done != prog_new.known - prog_new.done) {
            Logger.error(this,
                "Inconsistency when tracking split file progress (pushing): " + prog_old.known
                    + " of " + prog_old.done + " -> " + prog_new.known + " of " + prog_new.done);
            System.err.println(
                "Inconsistency when tracking split file progress (pushing): " + prog_old.known
                    + " of " + prog_old.done + " -> " + prog_new.known + " of " + prog_new.done);
          }
  ...
  enum WAIT_STATUS {
    FAILED, GENERATED_URI, GENERATED_METADATA;
  }
  ...
  public class PushCallback implements ClientPutCallback {
    ...
    public synchronized void setPutter(ClientPutter put) {
      ...
      synchronized (FreenetArchiver.this) {
        ...
        Logger.debug(this,
            "Pushing " + totalBytesPushing + " bytes on " + semiAsyncPushes.size() + " inserters");
      }
    }
    ...
    public void onFailure(InsertException e, BaseClientPutter state) {
      Logger.error(this, "Failed background insert (" + generatedURI + "), now running: "
          + semiAsyncPushes.size() + " (" + SizeUtil.formatSize(totalBytesPushing) + ").");
      synchronized (this) {
        failed = e;
        notifyAll();
      }
    ...
    public void onSuccess(BaseClientPutter state) {
      synchronized (FreenetArchiver.this) {
        if (semiAsyncPushes.remove(this))
          totalBytesPushing -= size;
        Logger.debug(this,
            "Completed background insert (" + generatedURI + ") in "
                + (System.currentTimeMillis() - startTime) + "ms, now running: "
                + semiAsyncPushes.size() + " (" + SizeUtil.formatSize(totalBytesPushing) + ").");
        FreenetArchiver.this.notifyAll();
      }
      if (ib != null)
  ...
  public static class SimpleProgressUpdater implements ClientEventListener {
    ...
    public void receive(ClientEvent ce, ClientContext context) {
      ...
      if (new_succeeded > new_total) {
        Logger.normal(this,
            "Received SplitfileProgressEvent greater than 100%: " + evt.getDescription());
      ...
      synchronized (splitfile_blocks) {
        ...
        try {
          progress.addPartKnown(new_total - old_total, evt.finalizedTotal); // throws
          ...
          if (n == 1) {
            ...
            Logger.normal(this,
                "Received SplitfileProgressEvent out-of-order: " + evt.getDescription());
            for (int i = 0; i < n; ++i) {
              progress.addPartDone();
            }
          }
          ...
          Logger.normal(this,
              "Received SplitfileProgressEvent out-of-order: " + evt.getDescription(), e);
        }
