 ...
 * @author infinity0
 */
public class FreenetArchiver<T> implements LiveArchiver<T, SimpleProgress>, RequestClient {
    ...
    public FreenetArchiver(NodeClientCore c, ObjectStreamReader r, ObjectStreamWriter w,
    ...
    public <S extends ObjectStreamWriter & ObjectStreamReader>FreenetArchiver(NodeClientCore c,
            S rw, String mime, int size, short priority) {
                ...
                try {
                    if ((cacheDir != null) && cacheDir.exists() && cacheDir.canRead()) {
                        ...
                        if (cached.exists() && (cached.length() != 0)) {
                            ...
                            System.out.println(
                                "Fetching block for FreenetArchiver from disk cache: " + cacheKey);
                        }
                    }
                    ...
                    if (tempB == null) {
                        if (initialMetadata != null) {
                            ...
                            System.out.println(
                                "Fetching block for FreenetArchiver from network: " + u);
                        }
                        ...
                        if (progress != null) {
                            ...
                            if (initialMetadata != null) {
                                res = hlsc.fetchFromMetadata(
                                    new SimpleReadOnlyArrayBucket(initialMetadata));
                            } else {
                                res = hlsc.fetch(u);
                            }
                            ...
                            if (prog_old.known - prog_old.done != prog_new.known - prog_new.done) {
                                Logger.error(
                                    this,
                                    "Inconsistency when tracking split file progress (pulling): " +
                    ...
                    System.out.println("Fetched block for FreenetArchiver in " +
                    ...
                    throw new TaskAbortException("Failed to read content from local tempbucket", e,
                                                 true);
    ...
    */
    /* @Override* */
    public void pushLive(PushTask<T> task, final SimpleProgress progress)
            throws TaskAbortException {
        HighLevelSimpleClient hlsc = core.makeClient(priorityClass, false, false);
        Bucket tempB = null;
...
//              } else {
                ...
                putter = new ClientPutter(cb, ib.getData(), FreenetURI.EMPTY_CHK_URI,
                                          ib.clientMetadata, ctx, priorityClass, false, false,
                                          this, null, false, core.clientContext, null,
                ...
                if (status == WAIT_STATUS.FAILED) {
                    ...
                    System.out.println("Got URI for asynchronous insert: " + uri + " size " +
                                       tempB.size() + " in " +
                                       (System.currentTimeMillis() - cb.startTime));
                } else {
                    ...
                    System.out.println("Got generated metadata (" + buf.length +
                                       " bytes) for asynchronous insert size " + tempB.size() +
...
//              }
                if (progress != null) {
                    ...
                    if (prog_old.known - prog_old.done != prog_new.known - prog_new.done) {
                        Logger.error(
                            this,
                            "Inconsistency when tracking split file progress (pushing): " +
                            prog_old.known + " of " + prog_old.done + " -> " + prog_new.known +
                            " of " + prog_new.done);
                        System.err.println(
                            "Inconsistency when tracking split file progress (pushing): " +
                            prog_old.known + " of " + prog_old.done + " -> " + prog_new.known +
                ...
                if ((cacheKey != null) && (cacheDir != null) && cacheDir.exists() &&
                        cacheDir.canRead()) {
                    File cached = new File(cacheDir, cacheKey);
                ...
                throw new TaskAbortException("Failed to write content to local tempbucket", e,
                                             true);
            } catch (RuntimeException e) {
                throw new TaskAbortException("Failed to complete task: ", e);
            }
    ...
    enum WAIT_STATUS { FAILED, GENERATED_URI, GENERATED_METADATA; }
...
//          this.progress = progress;
        ...
        public synchronized void setPutter(ClientPutter put) {
            ...
            synchronized (FreenetArchiver.this) {
                ...
                System.out.println("Pushing " + totalBytesPushing + " bytes on " +
                                   semiAsyncPushes.size() + " inserters");
            }
        }
        ...
        public void onFailure(InsertException e, BaseClientPutter state,
            ...
            System.out.println("Failed background insert (" + generatedURI + "), now running: " +
    ...
    public static class SimpleProgressUpdater implements ClientEventListener {
        ...
        public void receive(ClientEvent ce, ObjectContainer maybeContainer, ClientContext context) {
            ...
            if (new_succeeded > new_total) {
                Logger.normal(this,
                              "Received SplitfileProgressEvent greater than 100%: " +
                              evt.getDescription());
            ...
            synchronized (splitfile_blocks) {
                ...
                try {
                    progress.addPartKnown(new_total - old_total,
                                          evt.finalizedTotal);  // throws IllegalArgumentException
                    ...
                    if (n == 1) {
                        ...
                        Logger.normal(this,
                                      "Received SplitfileProgressEvent out-of-order: " +
                                      evt.getDescription());
        ...
        synchronized (this) {
            while (true) {
                if ( !pushesFailed.isEmpty()) {
                    throw new TaskAbortException("Failed to insert content",
                                                 pushesFailed.remove(0), true);
                }
                ...
                System.out.println("Waiting for " + semiAsyncPushes.size() +
